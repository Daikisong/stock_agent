    ...
    *,
    ...
    return {
        ...
        "rules": [
            ...
            "If existing_evidence_summary.full_thesis_queue_context.planner_failure_feedback is non-empty, treat it as source-route repair guidance only. It is not score evidence. Use previous_claim_failure_primary_mode, source_route_repair_actions, previous_seed_materialization_primary_failure_axis, seed_materialization_repair_actions, previous_source_task_primary_failure_axis, previous_source_task_top_source_classes, previous_source_task_top_primitive_gaps, and source_task_repair_actions to avoid repeating the prior rejected claim/seed/source-task pattern and produce source_task_drafts/query_intents that can create a new source-backed claim for the primitive_gap.",
            ...
            "If previous_seed_materialization_primary_failure_axis is PRIMITIVE_GAP_UNSATISFIED, the prior seed found adjacent or generic context but not the original primitive. Do not reuse that generic context as gap closure; plan a source route or query_intent that directly states the requested primitive_gap.",
            "If previous_seed_materialization_primary_failure_axis is NO_FETCHED_DOCUMENT, first plan a fetchable original source route before claim extraction. If it is TEMPORAL_NOT_CURRENT, find a current or superseding source. If it is PROVIDER_ERROR_RECORDED or PROVIDER_FAILED, keep the result source-pending instead of converting it to a low score.",
            "If previous_source_task_primary_failure_axis is PRIMITIVE_GAP_UNSATISFIED, the prior executed source task reached sources but did not close the requested primitive. Do not repeat the same top source classes as generic context unless the new task/query directly targets the missing primitive. If it is NO_SCORE_ELIGIBLE_REAL_CLAIM, plan a current direct target-company source anchor before any score path can close. If it is PROVIDER_ERROR_RECORDED or PROVIDER_FAILED, keep the result source-pending or choose a different bounded source class rather than treating the gap as absent.",
