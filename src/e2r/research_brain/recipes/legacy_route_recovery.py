"""Compatibility route matrix implementation owned by Research Brain."""
...
def _compatibility_route_families(research_families: list[str]) -> list[tuple[str, str]]:
    """Return a neutral official-first compatibility route.

    This projection intentionally does not inspect a primitive identifier.
    Question-specific route selection is compiled from EvidenceRecipe in the
    reconstructed path; this legacy-shaped matrix only keeps old reports
    ...
    *,
    ...
    for contract in contracts:
        ...
        for primitive in primitives:
            families = _compatibility_route_families(
                sorted(research_families_by_arch.get(archetype_id, []))
            )
