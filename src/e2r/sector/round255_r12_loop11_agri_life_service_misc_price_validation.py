"""Round-255 R12 Loop-11 agri/life-service/misc validation pack.

Round 255 converts ``docs/round/round_255.md`` into structured,
...
ROUND255_SOURCE_ROUND_PATH = "docs/round/round_255.md"
ROUND255_ANALYST_ROUND_ID = "round_183"
ROUND255_RAW_LARGE_SECTOR_LABEL = "AGRI_LIFE_SERVICE_MISC"
ROUND255_LARGE_SECTOR = Round10LargeSector.EDUCATION_LIFE_AGRI_MISC
ROUND255_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round255_r12_loop11_agri_life_service_misc_price_validation"
ROUND255_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r12_loop11_round255.jsonl"
ROUND255_DEFAULT_AUDIT_PATH = "data/sector_taxonomy/round255_r12_loop11_agri_life_service_misc_price_validation_audit.json"
ROUND255_DEFAULT_STAGE3_BIAS = "conservative_except_verified_recurring_service"

ROUND255_REQUIRED_TARGET_ALIASES: Mapping[str, str] = {
...
ROUND255_GREEN_REQUIRED_FIELDS: tuple[str, ...] = (
    "recurring_revenue_or_repeat_purchase_confirmed",
    "churn_or_retention_stable",
    "arpu_or_pricing_power_confirmed",
    "paid_enrollment_conversion_confirmed",
    "utilization_rate_confirmed",
...
ROUND255_GREEN_FORBIDDEN_PATTERNS: tuple[str, ...] = (
    "policy_news_only",
...
ROUND255_STAGE4B_WATCH_TRIGGERS: tuple[str, ...] = (
...
ROUND255_HARD_4C_GATES: tuple[str, ...] = (
    "product_recall",
    "churn_spike",
    "arpu_decline",
...
ROUND255_PRICE_VALIDATION_FIELDS: tuple[str, ...] = (
    "price_data_source",
    "full_ohlc_available",
...
class Round255ShadowWeightRow:
    ...
    def as_row(self) -> dict[str, str]:
        return {
            ...
            "utilization_rate": _signed(self.utilization_rate),
...
@dataclass(frozen=True)
class Round255DeepSubArchetype:
    category: str
...
class Round255CaseCandidate:
    ...
    def large_sector(self) -> str:
        return ROUND255_LARGE_SECTOR.value
...
ROUND255_SCORE_ADJUSTMENTS: tuple[Round255ScoreAdjustment, ...] = (
    Round255ScoreAdjustment("recurring_revenue", 5, "raise", "렌탈·구독·반복구매는 R12의 가장 강한 구조 증거다."),
    Round255ScoreAdjustment("churn_stability", 5, "raise", "반복매출은 churn이 안정적일 때만 구조적이다."),
    Round255ScoreAdjustment("ARPU_or_repeat_purchase", 4, "raise", "ARPU 또는 반복구매가 확인되어야 수요 질을 인정한다."),
    Round255ScoreAdjustment("unit_economics", 5, "raise", "교육·돌봄·펫·외식은 unit economics가 없으면 정책/이벤트에 머문다."),
    Round255ScoreAdjustment("cash_conversion", 5, "raise", "성장보다 현금전환이 R12 Green 가드레일이다."),
    Round255ScoreAdjustment("paid_enrollment_conversion", 4, "raise", "의대정원 정책은 유료수강 전환이 확인되어야 한다."),
    Round255ScoreAdjustment("utilization_rate", 4, "raise", "돌봄·서비스는 실제 이용률이 Stage 2 이후 핵심이다."),
    Round255ScoreAdjustment("input_cost_pass_through", 5, "raise", "배추·사료곡물 shock 이후 가격전가가 확인되어야 한다."),
    Round255ScoreAdjustment("inventory_quality", 4, "raise", "재고·매출채권 안정은 원가 shock 통과 조건이다."),
    Round255ScoreAdjustment("subsidy_independence", 4, "raise", "보조금 의존 매출은 Green을 제한한다."),
    Round255ScoreAdjustment("policy_news_only", -5, "lower", "정책 뉴스만으로는 회사 EPS/FCF evidence가 아니다."),
    Round255ScoreAdjustment("education_quota_only", -5, "lower", "의대정원 headline은 수강생·ARPU 전까지 event premium이다."),
    Round255ScoreAdjustment("AI_textbook_theme_only", -5, "lower", "AI 교과서 테마는 채택·예산·매출 전까지 Green 금지다."),
    Round255ScoreAdjustment("birthrate_headline_only", -4, "lower", "출산율 반등만으로 유아·교육·돌봄주 매출을 만들지 않는다."),
    Round255ScoreAdjustment("pet_welfare_policy_only", -4, "lower", "펫/동물복지 정책은 상장사 매출전환 전까지 제한한다."),
    Round255ScoreAdjustment("celebrity_food_event_only", -5, "lower", "바이럴 외식 이벤트는 매출 evidence 전 price_moved_without_evidence다."),
    Round255ScoreAdjustment("input_price_spike_without_pass_through", -5, "lower", "원재료 가격 상승이 가격전가 없이 오면 margin 4C-watch다."),
    Round255ScoreAdjustment("subsidy_dependent_revenue", -4, "lower", "보조금 의존 수익은 지속성 점수를 제한한다."),
    Round255ScoreAdjustment("unconfirmed_demand_conversion", -5, "lower", "트래픽·정책 관심이 유료수요로 전환되지 않으면 Stage 3 금지다."),
)


ROUND255_SHADOW_WEIGHT_ROWS: tuple[Round255ShadowWeightRow, ...] = (
    Round255ShadowWeightRow(E2RArchetype.HOME_LIVING_RENTAL_RECURRING, 5, 5, 4, 3, 4, 5, 5, 2, 4, 3, -1, 3, 4, "Coway recurring rental can be Stage 3 candidate but needs accounts/churn/ARPU/FCF."),
    Round255ShadowWeightRow(E2RArchetype.EDUCATION_POLICY_MEDICAL_QUOTA, 3, 2, 5, 5, 3, 4, 4, 0, 1, 2, -5, 5, 4, "Medical quota is event until paid enrollment/ARPU/OPM confirm."),
    Round255ShadowWeightRow(E2RArchetype.EDTECH_AI_TEXTBOOK_POLICY_REVERSAL, 2, 2, 3, 4, 3, 4, 4, 0, 1, 2, -5, 4, 4, "AI textbook rollback and phone ban create edtech policy friction."),
    Round255ShadowWeightRow(E2RArchetype.CHILDCARE_DEMOGRAPHIC_POLICY_EVENT, 4, 3, 4, 5, 5, 4, 4, 1, 1, 3, -4, 5, 3, "Fertility rebound is Stage 2; company utilization and recurring revenue must confirm."),
    Round255ShadowWeightRow(E2RArchetype.AGRI_FOOD_INPUT_COST_SHOCK, 2, 0, 3, 2, 2, 4, 4, 5, 5, 2, -4, 4, 4, "Kimchi cabbage shock requires pass-through and inventory control."),
    Round255ShadowWeightRow(E2RArchetype.FEED_GRAIN_INPUT_COST_4C, 2, 0, 2, 1, 1, 4, 4, 5, 5, 2, -4, 4, 4, "Feed wheat cost spike is 4C-watch for livestock/feed names."),
    Round255ShadowWeightRow(E2RArchetype.PET_WELFARE_POLICY_TRANSITION, 2, 2, 3, 3, 3, 4, 4, 1, 2, 3, -5, 5, 3, "Dog-meat ban is policy event until pet/service monetization confirms."),
    Round255ShadowWeightRow(E2RArchetype.FOOD_SERVICE_CELEBRITY_EVENT_PREMIUM, 2, 0, 3, 3, 2, 4, 4, 3, 3, 1, -5, 5, 3, "Celebrity food event is price_moved_without_evidence until traffic and margins confirm."),
)


ROUND255_DEEP_SUB_ARCHETYPES: tuple[Round255DeepSubArchetype, ...] = (
    Round255DeepSubArchetype("생활 렌탈", E2RArchetype.HOME_LIVING_RENTAL_RECURRING, ("Coway", "water purifier", "air purifier", "bidet", "mattress rental", "Malaysia", "churn", "ARPU", "FCF")),
    Round255DeepSubArchetype("의대정원 교육", E2RArchetype.EDUCATION_POLICY_MEDICAL_QUOTA, ("MegaStudy", "YBM Net", "NE Neungyule", "Woongjin Thinkbig", "medical-school quota", "repeat course", "ARPU")),
    Round255DeepSubArchetype("AI 교과서 정책마찰", E2RArchetype.EDTECH_AI_TEXTBOOK_POLICY_REVERSAL, ("AI textbook rollback", "supplementary material", "classroom phone ban", "teacher parent backlash")),
    Round255DeepSubArchetype("출산·돌봄", E2RArchetype.CHILDCARE_DEMOGRAPHIC_POLICY_EVENT, ("fertility rebound", "childcare support", "foreign housekeeper pilot", "paid utilization")),
    Round255DeepSubArchetype("김치·농식품 input", E2RArchetype.AGRI_FOOD_INPUT_COST_SHOCK, ("Daesang", "CJ CheilJedang", "napa cabbage", "kimchi input cost", "government cabbage stock release")),
    Round255DeepSubArchetype("사료·축산 input", E2RArchetype.FEED_GRAIN_INPUT_COST_4C, ("Harim", "Maniker", "Farm Story", "feed wheat tender", "grain cost", "FX input cost")),
    Round255DeepSubArchetype("동물복지·펫 전환", E2RArchetype.PET_WELFARE_POLICY_TRANSITION, ("dog-meat ban", "pet food", "animal welfare", "shelter", "adoption", "transition subsidy")),
    Round255DeepSubArchetype("생활서비스 이벤트", E2RArchetype.FOOD_SERVICE_CELEBRITY_EVENT_PREMIUM, ("Kyochon F&B", "Cherrybro", "Neuromeka", "Jensen Huang chicken event", "celebrity food event")),
)


ROUND255_CASE_CANDIDATES: tuple[Round255CaseCandidate, ...] = (
    Round255CaseCandidate(
        case_id="r12_loop11_coway_recurring_rental_insufficient",
        symbol="021240",
        company_name="Coway",
        ...
        round_stage_failure_type="structural_candidate_not_green",
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_medical_quota_private_education_watch",
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_edtech_ai_textbook_rollback_phone_ban",
        ...
        reported_return_anchor="AI textbooks reclassified; teacher unprepared share 87.4%; phone/device ban effective 2026-03; 37% daily-life impact and 22% anxiety survey anchors",
        mfe_1d=None,
        mae_1d=None,
        stage1_price_anchor=None,
        stage2_price_anchor=None,
        ...
        stage4b_price_anchor=None,
        stage4c_price_anchor=None,
        extra_price_metrics={"ai_textbook_status": "official_textbook_to_supplementary_material", "teacher_unprepared_share_pct": 87.4, "phone_device_ban_effective": "2026-03", "students_social_media_affects_daily_life_pct": 37.0, "students_anxious_without_social_media_pct": 22.0, "digital_device_exception": "disability_or_educational_purpose"},
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_childcare_fertility_policy_watch",
        ...
        stage4b_price_anchor=None,
        stage4c_price_anchor=None,
        extra_price_metrics={"fertility_rate_2023": 0.72, "fertility_rate_2024": 0.75, "fertility_rate_2025": 0.80, "fertility_rebound_2023_to_2025_pct": 11.1, "births_2025_growth_pct": 6.8, "marriages_2025_growth_pct": 8.1, "seoul_fertility_growth_pct": 8.9, "foreign_housekeeper_pilot_workers": 100.0, "possible_expansion_workers": 1200.0, "pilot_expansion_multiple": 12.0},
        score_price_alignment="unknown",
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_kimchi_cabbage_input_cost_watch",
        ...
        reported_return_anchor="cabbage +217.9%, late October -41.2% vs mid-September; highland area -54.6%",
        mfe_1d=None,
        mae_1d=None,
        stage1_price_anchor=None,
        stage2_price_anchor=None,
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_feed_wheat_livestock_input_cost_watch",
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_dog_meat_ban_pet_welfare_transition",
    ...
    Round255CaseCandidate(
        case_id="r12_loop11_kyochon_cherrybro_neuromeka_jensen_event",
...
def round255_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND255_CASE_CANDIDATES:
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                "Round255 R12 Loop-11 agri/life-service/misc price-validation case. "
            ...
            must_have_fields=ROUND255_GREEN_REQUIRED_FIELDS,
            ...
            score_weight_hint={f"{item.axis}_delta": float(item.points) for item in ROUND255_SCORE_ADJUSTMENTS},
            green_guardrails=(
                ...
                *ROUND255_GREEN_REQUIRED_FIELDS,
                *ROUND255_GREEN_FORBIDDEN_PATTERNS,
            ),
...
def round255_case_rows() -> tuple[dict[str, str], ...]:
...
def round255_score_adjustment_rows() -> tuple[dict[str, str], ...]:
    return tuple(adjustment.as_row() for adjustment in ROUND255_SCORE_ADJUSTMENTS)


def round255_shadow_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(row.as_row() for row in ROUND255_SHADOW_WEIGHT_ROWS)


def round255_deep_sub_archetype_rows() -> tuple[dict[str, str], ...]:
    return tuple(row.as_row() for row in ROUND255_DEEP_SUB_ARCHETYPES)


def round255_price_validation_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round255_price_validation": "true"} for field in ROUND255_PRICE_VALIDATION_FIELDS)


def round255_target_alias_rows() -> tuple[dict[str, str], ...]:
    return tuple({"round255_label": label, "canonical_archetype": canonical} for label, canonical in ROUND255_REQUIRED_TARGET_ALIASES.items())


def round255_summary() -> dict[str, int | bool | str]:
    cases = ROUND255_CASE_CANDIDATES
    return {
        "source_round": ROUND255_SOURCE_ROUND_PATH,
        "analyst_round_id": ROUND255_ANALYST_ROUND_ID,
        "raw_large_sector_label": ROUND255_RAW_LARGE_SECTOR_LABEL,
        "large_sector": ROUND255_LARGE_SECTOR.value,
        ...
        "target_archetype_count": len(ROUND255_REQUIRED_TARGET_ALIASES),
        "deep_sub_archetype_count": len(ROUND255_DEEP_SUB_ARCHETYPES),
        "shadow_weight_row_count": len(ROUND255_SHADOW_WEIGHT_ROWS),
        "price_validation_completed": "partial_with_reported_price_anchors",
        "full_ohlc_complete": False,
        "r12_default_stage3_bias": ROUND255_DEFAULT_STAGE3_BIAS,
...
def round255_audit_payload() -> dict[str, object]:
    return {
        "source_round": ROUND255_SOURCE_ROUND_PATH,
        "analyst_round_id": ROUND255_ANALYST_ROUND_ID,
        "raw_large_sector_label": ROUND255_RAW_LARGE_SECTOR_LABEL,
        "large_sector": ROUND255_LARGE_SECTOR.value,
        "summary": round255_summary(),
        "target_aliases": dict(ROUND255_REQUIRED_TARGET_ALIASES),
        "round_case_types": {case.case_id: case.round_case_type for case in ROUND255_CASE_CANDIDATES},
        "green_required_fields": list(ROUND255_GREEN_REQUIRED_FIELDS),
        "green_forbidden_patterns": list(ROUND255_GREEN_FORBIDDEN_PATTERNS),
        "stage4b_watch_triggers": list(ROUND255_STAGE4B_WATCH_TRIGGERS),
        "hard_4c_gates": list(ROUND255_HARD_4C_GATES),
        "deep_sub_archetypes": round255_deep_sub_archetype_rows(),
        "shadow_weights": round255_shadow_weight_rows(),
        "what_not_to_change": [
            "do_not_use_round255_cases_as_candidate_generation_input",
            ...
            "keep_hard_4c_unconfirmed_for_round255",
        ],
    }


def render_round255_summary_markdown() -> str:
    summary = round255_summary()
    lines = [
    ...
    for case in ROUND255_CASE_CANDIDATES:
        lines.append(
            "| "
            + " | ".join(
                (
...
def render_round255_green_gate_review_markdown() -> str:
    lines = ["# Round 255 R12 Green Gate Review", "", "Do not apply these weights to production scoring yet.", "", "## Required Fields", ""]
    lines.extend(f"- {field}" for field in ROUND255_GREEN_REQUIRED_FIELDS)
    lines.extend(["", "## Forbidden Patterns", ""])
    lines.extend(f"- {field}" for field in ROUND255_GREEN_FORBIDDEN_PATTERNS)
    lines.extend(
        [
            "",
            "## Easy Example",
...
def render_round255_stage4b_4c_review_markdown() -> str:
    lines = ["# Round 255 R12 4B/4C Review", "", "## 4B Watch Triggers", ""]
    lines.extend(f"- {field}" for field in ROUND255_STAGE4B_WATCH_TRIGGERS)
    lines.extend(["", "## Hard 4C Gates", ""])
    lines.extend(f"- {field}" for field in ROUND255_HARD_4C_GATES)
    lines.extend(
        [
            "",
...
def render_round255_price_validation_plan_markdown() -> str:
    lines = [
        "# Round 255 R12 Price Validation Plan",
        "",
        "- price_validation_completed: partial_with_reported_price_anchors",
        "- full_ohlc_complete: false",
        "- hard_4c_confirmed: false",
    ...
    lines.extend(f"- {field}" for field in ROUND255_PRICE_VALIDATION_FIELDS)
    return "\n".join(lines) + "\n"


def write_round255_r12_loop11_reports(
    output_directory: str | Path = ROUND255_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND255_DEFAULT_CASES_PATH,
    audit_path: str | Path = ROUND255_DEFAULT_AUDIT_PATH,
) -> dict[str, Path]:
    output = Path(output_directory)
    output.mkdir(parents=True, exist_ok=True)
    paths = {
        "cases": write_case_library(round255_case_records(), cases_path),
        "audit": _write_json(round255_audit_payload(), audit_path),
        "summary": output / "round255_r12_loop11_price_validation_summary.md",
        "case_matrix": output / "round255_r12_loop11_case_matrix.csv",
        "target_aliases": output / "round255_r12_loop11_target_aliases.csv",
        "score_adjustments": output / "round255_r12_loop11_score_adjustments.csv",
        "shadow_weights": output / "round255_r12_loop11_shadow_weights.csv",
        "deep_sub_archetypes": output / "round255_r12_loop11_deep_sub_archetypes.csv",
        "price_validation_fields": output / "round255_r12_loop11_price_validation_fields.csv",
        "green_gate_review": output / "round255_r12_loop11_green_gate_review.md",
        "price_validation_plan": output / "round255_r12_loop11_price_validation_plan.md",
        "stage4b_4c_review": output / "round255_r12_loop11_stage4b_4c_review.md",
    }
    paths["summary"].write_text(render_round255_summary_markdown(), encoding="utf-8")
    _write_csv(round255_case_rows(), paths["case_matrix"])
    _write_csv(round255_target_alias_rows(), paths["target_aliases"])
    _write_csv(round255_score_adjustment_rows(), paths["score_adjustments"])
    _write_csv(round255_shadow_weight_rows(), paths["shadow_weights"])
    _write_csv(round255_deep_sub_archetype_rows(), paths["deep_sub_archetypes"])
    _write_csv(round255_price_validation_field_rows(), paths["price_validation_fields"])
    paths["green_gate_review"].write_text(render_round255_green_gate_review_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round255_price_validation_plan_markdown(), encoding="utf-8")
    paths["stage4b_4c_review"].write_text(render_round255_stage4b_4c_review_markdown(), encoding="utf-8")
    return paths
