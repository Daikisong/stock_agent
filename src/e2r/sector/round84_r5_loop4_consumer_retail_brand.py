"""Round-84 R5 Loop-4 consumer, retail, and brand calibration pack.

Round 84 tightens R5 by separating recurring export consumption, hero-product
concentration, offline sell-through, affiliate-commerce margin quality,
e-commerce trust breaks, fast-fashion regulation, rental recurrence, and
...
ROUND84_SOURCE_ROUND_PATH = "docs/round/round_84.md"
ROUND84_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round84_r5_loop4_consumer_retail_brand"
ROUND84_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r5_loop4_round84.jsonl"
ROUND84_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round84_r5_loop4_v4.csv"


@dataclass(frozen=True)
class Round84ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round84ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round84ScoreWeightDraft
...
) -> Round84ScoreWeightDraft:
    return Round84ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round84ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND84_SCORE_TARGETS: tuple[Round84ScoreTarget, ...] = (
    _target(
...
ROUND84_CASE_CANDIDATES: tuple[Round84CaseCandidate, ...] = (
    Round84CaseCandidate(
        ...
        ("round_84.md MarketWatch Samyang Buldak target revision",),
    ...
    Round84CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_84.md AP Samyang Denmark recall",),
    ...
    Round84CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_84.md AP Samyang Denmark partial reversal",),
    ...
    Round84CaseCandidate(
        ...
        ("round_84.md Reuters K-beauty US export",),
    ...
    Round84CaseCandidate(
        ...
        ("round_84.md AP K-beauty US tariff risk",),
    ...
    Round84CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_84.md FT APR beauty device",),
    ...
    Round84CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_84.md Vogue Medicube omnichannel",),
    ...
    Round84CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_84.md Reuters K-beauty fast beauty OEM/ODM",),
    ...
    Round84CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_84.md Barron's Coupang data breach",),
        "Data breach is a hard trust break; month-level date needs exact event-day backfill.",
        (E2RArchetype.RETAIL_ECOMMERCE_LOGISTICS,),
    ),
    Round84CaseCandidate(
        "coupang_post_breach_deleted_claim_case",
        "DATA_SECURITY_SUPPLIER_REGULATION_OVERLAY",
        "CPNG_BREACH_FOLLOWUP",
        "Coupang 유출정보 삭제 주장·조사 미확인",
        "US",
        "4c_thesis_break",
        None,
        None,
        ...
        None,
        date(2025, 12, 25),
        ("post_breach_deleted_claim", "ecommerce_scale", "customer_accounts"),
        ("regulator_not_confirmed", "post_incident_disclosure_risk", "customer_trust_damage", "security_remediation_cost"),
        "post_breach_trust_watch",
        "needs_price_backfill",
        ("round_84.md Reuters Coupang leaked information deletion claim",),
        "Post-incident claims do not clear the trust break until regulators confirm remediation.",
        (E2RArchetype.RETAIL_ECOMMERCE_LOGISTICS,),
    ),
    Round84CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_84.md Reuters Coupang supplier regulation",),
    ...
    Round84CaseCandidate(
        ...
        ("round_84.md Coway rental model reference",),
    ...
    Round84CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_84.md Barron's Whirlpool dividend suspension",),
    ...
    Round84CaseCandidate(
        ...
        ("round_84.md Reuters Shein Temu litigation",),
    ...
    Round84CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_84.md FT France EU Shein Temu crackdown",),
...
ROUND84_PRICE_FIELDS: tuple[str, ...] = (
...
def round84_target_for(target_id: str) -> Round84ScoreTarget | None:
    for target in ROUND84_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round84_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND84_CASE_CANDIDATES:
        target = round84_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round84 R5 Loop-4 case for {candidate.target_id}; "
...
def round84_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round84_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND84_CASE_CANDIDATES:
        target = round84_target_for(candidate.target_id)
...
def round84_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round84_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round84_backfill": "true"} for field in ROUND84_PRICE_FIELDS)


def round84_summary() -> dict[str, int | bool]:
    records = round84_case_records()
    return {
        "target_count": len(ROUND84_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND84_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND84_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND84_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND84_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round84_r5_loop4_reports(
    *,
    output_directory: str | Path = ROUND84_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND84_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND84_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round84_r5_loop4_consumer_retail_brand_summary.md",
        "case_matrix": output / "round84_r5_loop4_case_matrix.csv",
        "stage_date_plan": output / "round84_r5_loop4_stage_date_plan.csv",
        "green_guardrails": output / "round84_r5_loop4_green_guardrails.md",
        "risk_overlays": output / "round84_r5_loop4_risk_overlays.md",
        "price_validation_plan": output / "round84_r5_loop4_price_validation_plan.md",
        "price_fields": output / "round84_r5_loop4_price_fields.csv",
    }
    _write_case_jsonl(round84_case_records(), cases)
    _write_rows(round84_score_profile_rows(), score_profiles)
    _write_rows(round84_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round84_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round84_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round84_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round84_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round84_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round84_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round84_summary_markdown() -> str:
    summary = round84_summary()
    lines = [
        "# Round-84 R5 Loop-4 Consumer / Retail / Brand Summary",
        "",
        f"- source_round: `{ROUND84_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R5 Loop 4 says sales buzz and recurring FCF are different things.",
...
def render_round84_green_guardrail_markdown() -> str:
    lines = [
    ...
    for target in ROUND84_SCORE_TARGETS:
        lines.append(
            ...
            f"{', '.join(target.green_conditions) or 'not_applicable'} | {', '.join(target.loop4_penalty_axes)} |"
        )
    lines.extend(
        [
            "",
...
def render_round84_risk_overlay_markdown() -> str:
    lines = [
        "# Round-84 R5 Loop-4 Risk Overlays",
        "",
...
def render_round84_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-84 R5 Loop-4 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round84_case_candidate_rows():
...
def _score_price_alignment(candidate: Round84CaseCandidate) -> str:
...
def _rerating_result(candidate: Round84CaseCandidate) -> str:
...
def _score_weight_hint(target: Round84ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
...
__all__ = [
    "ROUND84_CASE_CANDIDATES",
    "ROUND84_DEFAULT_CASES_PATH",
    "ROUND84_DEFAULT_OUTPUT_DIRECTORY",
    "ROUND84_DEFAULT_SCORE_PROFILE_PATH",
    "ROUND84_PRICE_FIELDS",
    "ROUND84_SCORE_TARGETS",
    "Round84CaseCandidate",
    "Round84ScoreTarget",
    "Round84ScoreWeightDraft",
    "render_round84_green_guardrail_markdown",
    "render_round84_price_validation_plan_markdown",
    "render_round84_risk_overlay_markdown",
    "render_round84_summary_markdown",
    "round84_case_candidate_rows",
    "round84_case_records",
    "round84_price_field_rows",
    "round84_score_profile_rows",
    "round84_stage_date_rows",
    "round84_summary",
    "round84_target_for",
    "write_round84_r5_loop4_reports",
]
