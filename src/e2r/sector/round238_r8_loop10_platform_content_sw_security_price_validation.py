"""Round-238 R8 Loop-10 platform/content/SW/security price validation pack.

Round 238 is calibration/evaluation material only. It turns
``docs/round/round_238.md`` into case-library records and guardrail reports.
...
ROUND238_SOURCE_ROUND_PATH = "docs/round/round_238.md"
ROUND238_ANALYST_ROUND_ID = "round_166"
ROUND238_LARGE_SECTOR = Round10LargeSector.PLATFORM_CONTENT_SW_SECURITY
ROUND238_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round238_r8_loop10_platform_content_sw_security_price_validation"
ROUND238_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r8_loop10_round238.jsonl"
ROUND238_DEFAULT_AUDIT_PATH = "data/sector_taxonomy/round238_r8_loop10_platform_content_sw_security_price_validation_audit.json"

ROUND238_REQUIRED_TARGET_ALIASES: Mapping[str, str] = {
...
ROUND238_GREEN_REQUIRED_FIELDS: tuple[str, ...] = (
...
ROUND238_GREEN_FORBIDDEN_PATTERNS: tuple[str, ...] = (
...
ROUND238_STAGE4B_WATCH_TRIGGERS: tuple[str, ...] = (
...
ROUND238_HARD_4C_GATES: tuple[str, ...] = (
...
ROUND238_PRICE_VALIDATION_FIELDS: tuple[str, ...] = (
    "price_data_source",
    "full_ohlc_available",
...
class Round238CaseCandidate:
    ...
    def large_sector(self) -> str:
        return ROUND238_LARGE_SECTOR.value
...
ROUND238_SCORE_ADJUSTMENTS: tuple[Round238ScoreAdjustment, ...] = (
    Round238ScoreAdjustment("recurring_revenue", 5, "raise", "SaaS/SW는 일회성 구축보다 반복 매출이 Stage 3 visibility를 만든다."),
    Round238ScoreAdjustment("arr_proxy", 5, "raise", "ARR proxy나 구독·유지보수 반복 매출이 있어야 AI/SW 매출 지속성을 본다."),
    Round238ScoreAdjustment("paid_usage_conversion", 5, "raise", "무료 사용량이나 MAU보다 유료 전환이 중요하다."),
    Round238ScoreAdjustment("bookings_repeatability", 4, "raise", "엔터프라이즈 수주가 반복되면 Stage 2에서 Stage 3 후보로 올라간다."),
    Round238ScoreAdjustment("enterprise_contract_quality", 4, "raise", "B2B 계약은 고객·기간·갱신성이 확인돼야 점수를 준다."),
    Round238ScoreAdjustment("opm_improvement", 5, "raise", "플랫폼/SW는 매출 증가가 OPM 개선으로 이어져야 체급 변화다."),
    Round238ScoreAdjustment("fcf_conversion", 5, "raise", "콘텐츠·게임·클라우드도 FCF가 따라오지 않으면 Green을 제한한다."),
    Round238ScoreAdjustment("customer_retention_or_churn", 4, "raise", "재계약률/이탈률은 반복 매출의 품질 증거다."),
    Round238ScoreAdjustment("ip_monetization_beyond_launch", 4, "raise", "게임·웹툰·K-pop은 출시/컴백 이후 반복 monetization이 필요하다."),
    Round238ScoreAdjustment("operational_trust", 5, "raise", "보안·개인정보·지배구조 신뢰가 통과돼야 플랫폼 Green을 허용한다."),
    Round238ScoreAdjustment("security_privacy_trust", 5, "raise", "플랫폼은 데이터·보안 신뢰가 유지돼야 반복 매출의 질을 인정한다."),
    Round238ScoreAdjustment("ai_feature_only", -5, "lower", "AI 기능 탑재만으로는 유료 매출 증거가 아니다."),
    Round238ScoreAdjustment("ai_partnership_headline_only", -5, "lower", "AI 파트너십 헤드라인은 paid usage 전까지 event premium이다."),
    Round238ScoreAdjustment("mau_without_arpu", -4, "lower", "MAU만 있고 ARPU/paid usage가 없으면 Stage 3를 막는다."),
    Round238ScoreAdjustment("ipo_debut_premium", -5, "lower", "IPO/debut premium은 반복 monetization 전까지 4B-watch다."),
    Round238ScoreAdjustment("mna_without_integration", -4, "lower", "M&A 발표만 있고 통합·가동률·매출 전환이 없으면 제한한다."),
    Round238ScoreAdjustment("ai_capex_without_revenue", -5, "lower", "AI capex는 매출/비용절감으로 연결돼야 한다."),
    Round238ScoreAdjustment("game_launch_first_week_only", -4, "lower", "첫 주 판매량만으로는 retention과 DLC/후속 매출을 알 수 없다."),
    Round238ScoreAdjustment("single_ip_dependence", -4, "lower", "단일 IP 의존은 Stage 3 확신을 낮춘다."),
    Round238ScoreAdjustment("founder_legal_risk", -5, "lower", "창업자·핵심 경영진 법적 리스크는 RedTeam 입력이다."),
    Round238ScoreAdjustment("privacy_security_trust_break", -5, "lower", "데이터 유출·보안 사고는 플랫폼 논리 훼손 감시다."),
    Round238ScoreAdjustment("data_breach_revenue_cut", -5, "lower", "보상·매출 감소가 동반된 유출은 hard 4C 후보가 된다."),
)


ROUND238_SHADOW_WEIGHT_ROWS: tuple[Round238ShadowWeightRow, ...] = (
    Round238ShadowWeightRow(E2RArchetype.B2B_SAAS_ERP_WORKFLOW, 5, 5, 3, 3, 5, 0, 2, 3, 2, -2, 1, 3, 3, "Douzone/EQT is Stage 2 until ARR/churn/OPM/FCF confirm."),
    Round238ShadowWeightRow(E2RArchetype.AI_CLOUD_CAPITAL_ALLOCATION, 3, 3, 3, 2, 5, 0, 5, 3, 2, -5, 2, 5, 3, "Samsung SDS KKR event is 4B-watch before AI revenue conversion."),
    Round238ShadowWeightRow(E2RArchetype.CLOUD_AI_SOFTWARE_INFRA, 4, 4, 3, 2, 5, 0, 4, 3, 2, -4, 2, 4, 3, "LG CNS IPO weak price action blocks Green despite cloud/AI mix."),
    Round238ShadowWeightRow(E2RArchetype.WEBTOON_PLATFORM_IP_MONETIZATION, 3, 3, 5, 4, 5, 5, 2, 3, 2, -3, 2, 5, 4, "Webtoon needs paid usage, ARPU, IP monetization and FCF beyond MAU/IPO."),
    Round238ShadowWeightRow(E2RArchetype.AI_SOFTWARE_APPLICATION, 2, 2, 5, 2, 4, 0, 5, 3, 2, -5, 2, 5, 3, "Kakao/OpenAI partnership is price_moved_without_evidence until monetization."),
    Round238ShadowWeightRow(E2RArchetype.GAME_CONTENT_IP_REPEAT_MONETIZATION, 3, 0, 3, 5, 5, 5, 0, 3, 2, -3, 2, 4, 4, "Shift Up needs repeat bookings/pipeline conversion beyond launch and IPO."),
    Round238ShadowWeightRow(E2RArchetype.SECURITY_OPERATIONAL_TRUST_OVERLAY, 0, 0, 0, 0, 3, 0, 0, 5, 5, -2, 5, 4, 5, "SK Telecom breach is strong 4C-watch with revenue/fine/compensation impact."),
    Round238ShadowWeightRow(E2RArchetype.PLATFORM_GOVERNANCE_LEGAL_RISK, 0, 0, 0, 0, 0, 3, 0, 5, 2, -3, 5, 4, 5, "HYBE legal risk is 4C-watch; hard 4C only after legal outcome materially impairs business."),
)


ROUND238_CASE_CANDIDATES: tuple[Round238CaseCandidate, ...] = (
    Round238CaseCandidate(
        case_id="r8_loop10_douzone_eqt_b2b_saas_stage2_watch",
        symbol="012510",
        company_name="더존비즈온",
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_samsung_sds_ai_cloud_kkr_event_premium",
        symbol="018260",
        company_name="삼성SDS",
        primary_archetype=E2RArchetype.AI_CLOUD_CAPITAL_ALLOCATION,
        secondary_archetypes=(E2RArchetype.CLOUD_AI_SOFTWARE_INFRA, E2RArchetype.EVENT_PREMIUM, E2RArchetype.PRICE_ONLY_RALLY),
        ...
        stage4b_status="watch",
        hard_4c_confirmed=False,
        evidence_fields=("event_mfe_20_8pct", "morning_gain_19_4pct", "kospi_gain_3_0pct", "relative_outperformance_17_8pp", "convertible_bond_820m_usd", "existing_cash_6_4tn_krw", "combined_cash_plus_cb_7_607tn_krw"),
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_lg_cns_ipo_cloud_ai_price_failed",
        symbol="LG_CNS",
        company_name="LG CNS",
        ...
        stage4b_status="watch",
        hard_4c_confirmed=False,
        evidence_fields=("ipo_price_61900_krw", "opening_price_60500_krw", "debut_trade_price_59700_krw", "ipo_proceeds_1_2tn_krw", "morning_market_value_5_79tn_krw", "cloud_ai_sales_more_than_half"),
        ...
        reported_return_anchor="debut MAE -3.55%; opening return -2.26%; cloud/AI was more than half of sales; morning market value 5.79T KRW",
        mfe_1d=None,
        mae_1d=-3.55,
        stage1_price_anchor=None,
        ...
        stage4b_price_anchor=None,
        stage4c_price_anchor=None,
        extra_price_metrics={"ipo_price_krw": 61900.0, "opening_price_krw": 60500.0, "opening_return_pct": -2.26, "debut_trade_price_krw": 59700.0, "debut_mae_pct": -3.55, "ipo_proceeds_krw_tn": 1.2, "morning_market_value_krw_tn": 5.79, "cloud_ai_sales_mix_more_than_half": True},
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_naver_webtoon_ipo_disney_ip_watch",
        symbol="035420/WBTN",
        ...
        stage4b_status="watch",
        hard_4c_confirmed=False,
        evidence_fields=("ipo_price_21_usd", "first_day_close_23_usd", "next_day_high_25_66_usd", "disney_event_mfe_62pct", "disney_event_revenue_348_3m_usd", "adjusted_q2_profit_0_07_usd", "mau_155m"),
        red_flag_fields=("ipo_premium_before_profit", "ip_event_before_monetization", "post_ipo_drawdown_minus_55pct_watch", "mau_without_arpu"),
        ...
        reported_return_anchor="first-day +9.5%; next-day MFE +22.2%; Disney event MFE +62%; pre-Disney post-IPO drawdown -55%; MAU about 155M",
        mfe_1d=22.2,
        mae_1d=None,
        stage1_price_anchor=None,
        ...
        stage4c_price_anchor=None,
        extra_price_metrics={"ipo_price_usd": 21.0, "debut_high_usd": 24.0, "debut_mfe_pct": 14.3, "first_day_close_usd": 23.0, "first_day_return_pct": 9.5, "next_day_high_usd": 25.66, "next_day_mfe_pct": 22.2, "disney_event_price_usd": 15.16, "disney_event_mfe_pct": 62.0, "pre_disney_post_ipo_drawdown_pct": -55.0, "disney_event_revenue_usd_mn": 348.3, "disney_event_revenue_growth_pct": 8.5, "adjusted_q2_profit_per_share_usd": 0.07, "expected_q2_loss_per_share_usd": -0.14, "mau_context_mn": 155.0},
        score_price_alignment="unknown",
        rerating_result="unknown",
        stage_failure_type="stage2_watch_success",
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_kakao_openai_partnership_price_only_rally",
        symbol="035720",
        company_name="카카오",
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_shiftup_single_ip_repeat_monetization_watch",
        symbol="462870",
        company_name="시프트업",
        ...
        case_type="success_candidate",
        round_case_type="success_candidate_ipo_watch",
        ...
        stage4b_status="watch",
        hard_4c_confirmed=False,
        evidence_fields=("ipo_raise_435bn_krw", "ipo_valuation_3_5tn_krw", "2023_opm_65_7pct", "nikke_revenue_255bn_krw", "stellar_blade_pc_3d_sales_above_1m", "stellar_blade_total_sales_above_3m", "sequel_project_spirits_pipeline"),
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_skt_data_breach_operational_trust_4c_watch",
        symbol="017670",
        company_name="SK텔레콤",
        ...
        round_case_type="strong_4c_watch_operational_trust_break",
        stage1_date=None,
        stage2_date=None,
        stage3_date=None,
        stage4b_date=None,
        ...
        stage4b_status="watch",
        hard_4c_confirmed=False,
        evidence_fields=("event_mae_8_5pct", "close_return_minus_6_7pct", "relative_underperformance_minus_6_8pp", "affected_users_23m", "stores_2600_plus", "protection_signups_5_54m", "leaked_data_26_96m_pieces", "security_investment_700bn_krw", "revenue_forecast_cut_800bn_krw", "customer_package_500bn_krw", "pipc_fine_134_8bn_krw"),
        ...
        stage4b_price_anchor=None,
        stage4c_price_anchor=None,
        extra_price_metrics={"event_intraday_mae_pct": -8.5, "event_close_return_pct": -6.7, "kospi_same_day_return_pct": 0.1, "relative_underperformance_pp": -6.8, "affected_users_mn": 23.0, "stores_count": 2600.0, "protection_signups_mn": 5.54, "signup_share_pct": 24.1, "july_event_return_pct": -5.6, "leaked_data_pieces_mn": 26.96, "security_investment_krw_bn": 700.0, "security_investment_annual_krw_bn": 140.0, "revenue_forecast_cut_krw_bn": 800.0, "customer_package_krw_bn": 500.0, "pipc_fine_krw_bn": 134.8, "pipc_fine_usd_mn": 96.5},
    ...
    Round238CaseCandidate(
        case_id="r8_loop10_hybe_founder_legal_governance_4c_watch",
        symbol="352820",
        company_name="HYBE",
...
def round238_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND238_CASE_CANDIDATES:
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                "Round238 R8 Loop-10 platform/content/SW/security price-path "
            ...
            must_have_fields=ROUND238_GREEN_REQUIRED_FIELDS,
            ...
            score_weight_hint={f"{item.axis}_delta": float(item.points) for item in ROUND238_SCORE_ADJUSTMENTS},
            green_guardrails=(
...
def round238_case_rows() -> tuple[dict[str, str], ...]:
...
def round238_score_adjustment_rows() -> tuple[dict[str, str], ...]:
    return tuple(adjustment.as_row() for adjustment in ROUND238_SCORE_ADJUSTMENTS)


def round238_shadow_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(row.as_row() for row in ROUND238_SHADOW_WEIGHT_ROWS)


def round238_price_validation_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round238_price_validation": "true"} for field in ROUND238_PRICE_VALIDATION_FIELDS)


def round238_target_alias_rows() -> tuple[dict[str, str], ...]:
    return tuple({"round238_label": label, "canonical_archetype": canonical} for label, canonical in ROUND238_REQUIRED_TARGET_ALIASES.items())


def round238_summary() -> dict[str, int | bool | str]:
    cases = ROUND238_CASE_CANDIDATES
    return {
        "source_round": ROUND238_SOURCE_ROUND_PATH,
        "analyst_round_id": ROUND238_ANALYST_ROUND_ID,
        "large_sector": ROUND238_LARGE_SECTOR.value,
        ...
        "strong_4c_watch_count": sum(1 for case in cases if "4c_watch" in case.round_case_type),
        "hard_4c_case_count": sum(1 for case in cases if case.hard_4c_confirmed),
        "target_archetype_count": len(ROUND238_REQUIRED_TARGET_ALIASES),
...
def round238_audit_payload() -> dict[str, object]:
    return {
        "source_round": ROUND238_SOURCE_ROUND_PATH,
        "analyst_round_id": ROUND238_ANALYST_ROUND_ID,
        "large_sector": ROUND238_LARGE_SECTOR.value,
        "summary": round238_summary(),
        "target_aliases": dict(ROUND238_REQUIRED_TARGET_ALIASES),
        "round_case_types": {case.case_id: case.round_case_type for case in ROUND238_CASE_CANDIDATES},
        "green_required_fields": list(ROUND238_GREEN_REQUIRED_FIELDS),
        "green_forbidden_patterns": list(ROUND238_GREEN_FORBIDDEN_PATTERNS),
        "stage4b_watch_triggers": list(ROUND238_STAGE4B_WATCH_TRIGGERS),
        "hard_4c_gates": list(ROUND238_HARD_4C_GATES),
        "what_not_to_change": [
            "do_not_use_round238_cases_as_candidate_generation_input",
            ...
            "do_not_upgrade_4c_watch_to_hard_4c_without_confirming_business_impairment",
        ],
    }


def render_round238_summary_markdown() -> str:
    summary = round238_summary()
    lines = [
        ...
        f"- strong 4C-watch cases: {summary['strong_4c_watch_count']}",
    ...
    for case in ROUND238_CASE_CANDIDATES:
        lines.append(
            "| "
            + " | ".join(
                (
...
def render_round238_green_gate_review_markdown() -> str:
    lines = [
        "# Round 238 R8 Green Gate Review",
        "",
    ...
    lines.extend(f"- {field}" for field in ROUND238_GREEN_REQUIRED_FIELDS)
    lines.extend(["", "## Forbidden Patterns", ""])
    lines.extend(f"- {field}" for field in ROUND238_GREEN_FORBIDDEN_PATTERNS)
    lines.extend(
        [
            "",
            "## Easy Example",
...
def render_round238_stage4b_4c_review_markdown() -> str:
    lines = ["# Round 238 R8 4B/4C Review", "", "## 4B Watch Triggers", ""]
    lines.extend(f"- {field}" for field in ROUND238_STAGE4B_WATCH_TRIGGERS)
    lines.extend(["", "## Hard 4C Gates", ""])
    lines.extend(f"- {field}" for field in ROUND238_HARD_4C_GATES)
...
def render_round238_price_validation_plan_markdown() -> str:
    lines = [
    ...
    lines.extend(f"- {field}" for field in ROUND238_PRICE_VALIDATION_FIELDS)
    return "\n".join(lines) + "\n"


def write_round238_r8_loop10_reports(
    output_directory: str | Path = ROUND238_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND238_DEFAULT_CASES_PATH,
    audit_path: str | Path = ROUND238_DEFAULT_AUDIT_PATH,
) -> dict[str, Path]:
    output = Path(output_directory)
    output.mkdir(parents=True, exist_ok=True)

    paths = {
        "cases": write_case_library(round238_case_records(), cases_path),
        "audit": _write_json(round238_audit_payload(), audit_path),
        "summary": output / "round238_r8_loop10_price_validation_summary.md",
        "case_matrix": output / "round238_r8_loop10_case_matrix.csv",
        "target_aliases": output / "round238_r8_loop10_target_aliases.csv",
        "score_adjustments": output / "round238_r8_loop10_score_adjustments.csv",
        "shadow_weights": output / "round238_r8_loop10_shadow_weights.csv",
        "price_validation_fields": output / "round238_r8_loop10_price_validation_fields.csv",
        "green_gate_review": output / "round238_r8_loop10_green_gate_review.md",
        "price_validation_plan": output / "round238_r8_loop10_price_validation_plan.md",
        "stage4b_4c_review": output / "round238_r8_loop10_stage4b_4c_review.md",
    }
    paths["summary"].write_text(render_round238_summary_markdown(), encoding="utf-8")
    _write_csv(round238_case_rows(), paths["case_matrix"])
    _write_csv(round238_target_alias_rows(), paths["target_aliases"])
    _write_csv(round238_score_adjustment_rows(), paths["score_adjustments"])
    _write_csv(round238_shadow_weight_rows(), paths["shadow_weights"])
    _write_csv(round238_price_validation_field_rows(), paths["price_validation_fields"])
    paths["green_gate_review"].write_text(render_round238_green_gate_review_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round238_price_validation_plan_markdown(), encoding="utf-8")
    paths["stage4b_4c_review"].write_text(render_round238_stage4b_4c_review_markdown(), encoding="utf-8")
    return paths
