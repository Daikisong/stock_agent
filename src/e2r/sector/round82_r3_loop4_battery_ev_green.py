"""Round-82 R3 Loop-4 battery, EV, and green-energy pack.

Round 82 tightens the R3 split again. It separates ESS LFP grid storage,
AI data-center storage, EV-to-ESS redeployment, disclosure-confidence caps,
...
evidence unless contracts, customers, margins, utilization, and FCF are
...
ROUND82_SOURCE_ROUND_PATH = "docs/round/round_82.md"
ROUND82_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round82_r3_loop4_battery_ev_green"
ROUND82_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r3_loop4_round82.jsonl"
ROUND82_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round82_r3_loop4_v4.csv"


@dataclass(frozen=True)
class Round82ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round82ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round82ScoreWeightDraft
...
) -> Round82ScoreWeightDraft:
    return Round82ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round82ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND82_SCORE_TARGETS: tuple[Round82ScoreTarget, ...] = (
    _target(
        ...
        note="Loop 4 lowers battery-material scores because EV CAPA and contracts can quickly become 4C.",
    ),
    _target(
...
ROUND82_CASE_CANDIDATES: tuple[Round82CaseCandidate, ...] = (
    Round82CaseCandidate(
        "lg_energy_lfp_4_3b_contract_case",
        "BATTERY_CONTRACT_DISCLOSURE_CONFIDENCE",
        "373220",
        "LG Energy Solution LFP $4.3B contract disclosure cap",
        "KR",
        "success_candidate",
        None,
        date(2025, 7, 30),
        ...
        ("round_82.md Reuters LGES $4.3B LFP contract",),
        "Contract value and duration support Stage 2, but customer and use-case are undisclosed, so Stage 3-Green must be capped.",
        (E2RArchetype.ESS_LFP_GRID_STORAGE,),
    ),
    Round82CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_82.md Reuters SK On Flatiron ESS deal",),
    ...
    Round82CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_82.md Reuters GM-LG Ohio battery plant",),
    ...
    Round82CaseCandidate(
        ...
        ("round_82.md Financial Times Ford Energy storage pivot",),
    ...
    Round82CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_82.md Reuters Ford LGES contract cancellation",),
    ...
    Round82CaseCandidate(
        ...
        ("round_82.md Reuters Redwood recycling funding",),
    ...
    Round82CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_82.md Reuters EQT KJ Environment platform",),
    ...
    Round82CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_82.md Reuters Hyundai hydrogen fuel-cell plant",),
    ...
    Round82CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_82.md Reuters Qcells customs detention",),
    ...
    Round82CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_82.md Reuters Orsted Sunrise Wind impairment",),
    ...
    Round82CaseCandidate(
        ...
        "missing_price_data",
        ("round_82.md Reuters lithium prices after crash",),
    ...
    Round82CaseCandidate(
        ...
        "missing_price_data",
        ("round_82.md Reuters energy storage lithium demand",),
    ...
    Round82CaseCandidate(
        ...
        "missing_price_data",
        ("round_82.md Reuters Korea EV battery certification",),
    ...
    Round82CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_82.md AP News Moss Landing battery storage fire",),
    ...
    Round82CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_82.md arXiv battery health validation",),
    ...
    Round82CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_82.md speculative battery technology guardrail",),
...
ROUND82_PRICE_FIELDS: tuple[str, ...] = (
    ...
    "facility_permitting_delay_flag", "fire_safety_capex_flag",
    "score_price_alignment", "price_validation_status", "review_notes",
)


def round82_target_for(target_id: str) -> Round82ScoreTarget | None:
    for target in ROUND82_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round82_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND82_CASE_CANDIDATES:
        target = round82_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round82 R3 Loop-4 case for {candidate.target_id}; "
...
def round82_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round82_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND82_CASE_CANDIDATES:
        target = round82_target_for(candidate.target_id)
...
def round82_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round82_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round82_backfill": "true"} for field in ROUND82_PRICE_FIELDS)


def round82_summary() -> dict[str, int | bool]:
    records = round82_case_records()
    return {
        "target_count": len(ROUND82_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND82_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND82_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND82_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND82_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round82_r3_loop4_reports(
    *,
    output_directory: str | Path = ROUND82_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND82_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND82_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round82_r3_loop4_battery_ev_green_summary.md",
        "case_matrix": output / "round82_r3_loop4_case_matrix.csv",
        "stage_date_plan": output / "round82_r3_loop4_stage_date_plan.csv",
        "green_guardrails": output / "round82_r3_loop4_green_guardrails.md",
        "risk_overlays": output / "round82_r3_loop4_risk_overlays.md",
        "price_validation_plan": output / "round82_r3_loop4_price_validation_plan.md",
        "price_fields": output / "round82_r3_loop4_price_fields.csv",
    }
    _write_case_jsonl(round82_case_records(), cases)
    _write_rows(round82_score_profile_rows(), score_profiles)
    _write_rows(round82_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round82_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round82_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round82_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round82_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round82_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round82_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round82_summary_markdown() -> str:
    summary = round82_summary()
    lines = [
        "# Round-82 R3 Loop-4 Battery / EV / Green-Energy Summary",
        "",
        f"- source_round: `{ROUND82_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R3 Loop 4 says EV growth and durable FCF are different things.",
...
def render_round82_green_guardrail_markdown() -> str:
    lines = [
...
def render_round82_risk_overlay_markdown() -> str:
    lines = [
        "# Round-82 R3 Loop-4 Risk Overlays",
        "",
...
def render_round82_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-82 R3 Loop-4 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round82_case_candidate_rows():
...
def _score_price_alignment(candidate: Round82CaseCandidate) -> str:
...
def _rerating_result(candidate: Round82CaseCandidate) -> str:
...
def _score_weight_hint(target: Round82ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
