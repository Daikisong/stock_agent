"""Round-161 R3 Loop-10 battery, EV, and green-energy pack.

Round 161 turns the R3 battery/EV/green-energy pack into a score-stage-price
alignment layer. It keeps the Round-108 archetype split, but makes the stage
caps explicit: ESS contracts, EV-to-ESS redeployment, and solid-state licenses
can be strong Stage 2 evidence, while Stage 3 still needs revenue recognition,
ESS OPM, utilization, FCF, EPS revision, and price-path confirmation. It also
raises contract cancellation, idle plant, UFLPA/customs, wind impairment,
BESS/EV fire, and SOH opacity into hard RedTeam gates.

Simple example: Tesla/LGES Megapack evidence can lift contract visibility to
Stage 2. It is not Stage 3-Green until Lansing ramp-up, ESS margin, utilization,
FCF, and EPS revision are visible as-of the case date.
...
The source round lists 23 primary Loop-10 targets. This pack keeps those
targets plus three adjacent/helper targets from the existing R3 taxonomy
(`HYDROGEN_FUEL_CELL_INFRA`, `CARBON_CREDIT_CBAM_COMPLIANCE`, and
`SPECULATIVE_BATTERY_TECH`) because the source text still discusses hydrogen,
carbon, and speculative battery technology as nearby guardrail surfaces.
"""
...
ROUND161_SOURCE_ROUND_PATH = "docs/round/round_161.md"
ROUND161_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round161_r3_loop10_battery_ev_green"
ROUND161_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r3_loop10_round161.jsonl"
ROUND161_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round161_r3_loop10_v10.csv"


@dataclass(frozen=True)
class Round161ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round161ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round161ScoreWeightDraft
...
@dataclass(frozen=True)
class Round161ScoreStagePriceAlignment:
    case_id: str
    score_stage: str
...
) -> Round161ScoreWeightDraft:
    return Round161ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round161ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND161_BASE_SCORE_WEIGHTS: tuple[Round161BaseScoreWeight, ...] = (
    Round161BaseScoreWeight(
        "eps_fcf_opm_transition",
        24,
        "Stage 3 승격의 중심축. ESS OPM, revenue recognition, FCF, EPS revision이 없으면 Green을 만들지 않는다.",
    ),
    Round161BaseScoreWeight(
        "contract_visibility",
        22,
        "고객, 계약금액, 계약기간, GWh, 생산시점이 같이 확인될수록 Stage 2 가시성이 강해진다.",
    ),
    Round161BaseScoreWeight(
        "demand_durability_structural_change",
        16,
        "EV 성장 서사가 아니라 grid ESS, AI data center storage, China dependency reduction처럼 수요 지속성이 보여야 한다.",
    ),
    Round161BaseScoreWeight(
        "safety_regulatory_disclosure_confidence",
        14,
        "계약취소, 고객/용도 비공개, UFLPA, BESS fire, SOH opacity 같은 안전·규제·공시 리스크를 더 강한 별도 축으로 본다.",
    ),
    Round161BaseScoreWeight(
        "capa_utilization_capital_efficiency_fcf_stability",
        10,
        "EV 라인 전환, Lansing ramp-up, idle plant 해소, utilization, fixed-cost absorption을 본다.",
    ),
    Round161BaseScoreWeight(
        "market_mispricing_rerating_gap",
        8,
        "시장이 아직 EV CAPA/정책 테마로만 보는지, ESS/FCF 구조 변화를 반영했는지 확인한다.",
    ),
    Round161BaseScoreWeight(
        "valuation_room_4b_runway",
        6,
        "이미 ESS·전고체·친환경 테마가 가격에 과도하게 반영되면 4B-watch로 제한한다.",
    ),
)


ROUND161_STAGE_CAPS: tuple[Round161StageCap, ...] = (
    Round161StageCap(
        "narrative_only_cap",
        "Stage 1",
        "EV/ESS/recycling/solid-state/solar/wind/hydrogen narrative only",
        "EV growth, ESS, or solar factory headline without customer, value, utilization, margin, or FCF.",
    ),
    Round161StageCap(
        "stage2_contract_visibility_cap",
        "Stage 2",
        "contract/customer/GWh/license/production date partly confirmed, but revenue, OPM, utilization, and FCF unconfirmed",
        "LGES-Tesla Megapack 3 or SK On-Flatiron ESS supply is visible, but ESS margin and ramp-up remain open.",
    ),
    Round161StageCap(
        "stage3_operating_confirmation_required",
        "Stage 3 candidate",
        "revenue recognition, ESS OPM, utilization, FCF, EPS revision, and price path are visible",
        "Contract value alone is not enough; the operating line must absorb fixed cost and produce measurable FCF.",
    ),
    Round161StageCap(
        "stage4b_narrative_priced_ahead",
        "4B-watch",
        "ESS, solid-state, recycling, or AI storage narrative is priced before contracts, revenue, or margins",
        "QuantumScape/VW license can be event-premium Stage 2 plus 4B-watch, not commercialization Green.",
    ),
    Round161StageCap(
        "stage4c_hard_redteam",
        "4C",
        "contract cancellation, expected revenue loss, idle plant, UFLPA/customs, impairment, BESS/EV fire, SOH opacity",
        "Ford-LGES cancellation, Ultium Ohio idle, Qcells UFLPA furlough, Orsted impairment, or Moss Landing fire.",
    ),
)


ROUND161_SCORE_STAGE_PRICE_ALIGNMENT: tuple[Round161ScoreStagePriceAlignment, ...] = (
    Round161ScoreStagePriceAlignment(
        "tesla_lges_megapack3_lansing_case",
        "Stage 2",
        "LGES +0.6% around contract confirmation",
        "visibility confirmed but not Stage 3",
        "give contract detail credit; keep EPS/FCF, utilization, and ESS OPM cap",
    ),
    Round161ScoreStagePriceAlignment(
        "sk_on_flatiron_ess_7_2gwh_case",
        "Stage 2",
        "contract value undisclosed; price backfill needed",
        "Stage 2 cap is appropriate",
        "strengthen contract_value_missing and margin_unverified penalty",
    ),
    Round161ScoreStagePriceAlignment(
        "ford_energy_storage_pivot_case",
        "EV-to-ESS Stage 2",
        "Ford +20% over two days and +6.7% on event day",
        "EV-to-ESS pivot price path is confirmed",
        "raise EV_TO_ESS watch weight, but do not generalize to battery suppliers without direct contracts",
    ),
    Round161ScoreStagePriceAlignment(
        "redwood_recycling_energy_storage_case",
        "structural reference",
        "private company; public price validation unavailable",
        "archetype reference only",
        "require direct listed-company exposure before scoring public equities",
    ),
    Round161ScoreStagePriceAlignment(
        "quantumscape_vw_solid_state_license_case",
        "Stage 2 + 4B-watch",
        "premarket +40%",
        "event premium confirmed; Green not confirmed",
        "keep solid-state license cap until mass production, royalty revenue, yield, and cost are visible",
    ),
    Round161ScoreStagePriceAlignment(
        "ford_lges_ev_contract_cancel_case",
        "Stage 4C",
        "LGES intraday drawdown up to -7.6%",
        "RedTeam gate aligned with price path",
        "strengthen contract_cancellation and expected_revenue_loss gate",
    ),
    Round161ScoreStagePriceAlignment(
        "lges_freudenberg_contract_cancel_case",
        "Stage 4C",
        "expected revenue loss after Ford and Freudenberg cancellations around KRW 13.5T",
        "4C reinforced",
        "make expected_revenue_loss an explicit field",
    ),
    Round161ScoreStagePriceAlignment(
        "gm_lg_ultium_ohio_idle_case",
        "Stage 4C",
        "operating status downgraded; restart uncertain",
        "EV CAPA false Green",
        "strengthen plant_idle and utilization penalty; treat Tennessee ESS conversion separately as Watch",
    ),
    Round161ScoreStagePriceAlignment(
        "qcells_customs_detention_furlough_case",
        "Stage 4C",
        "1,000-worker furlough from stalled shipments",
        "solar supply-chain risk confirmed",
        "strengthen UFLPA/customs/FEOC gate",
    ),
    Round161ScoreStagePriceAlignment(
        "orsted_sunrise_wind_impairment_case",
        "Stage 4C",
        "maximum price-path loss around -18%",
        "project economics RedTeam aligned",
        "strengthen financing_cost, foundation_cost, delay, and impairment fields",
    ),
    Round161ScoreStagePriceAlignment(
        "moss_landing_bess_fire_case",
        "4C-watch",
        "sector safety/permitting gate",
        "BESS demand needs safety cap",
        "raise BESS safety, permitting, insurance, and local-opposition gate",
    ),
    Round161ScoreStagePriceAlignment(
        "battery_soh_transparency_case",
        "4C-watch",
        "not a direct price event",
        "second-life valuation cap",
        "require independent SOH and residual-capacity validation",
    ),
)


ROUND161_SCORE_TARGETS: tuple[Round161ScoreTarget, ...] = (
    _target(
        ...
        note="Loop 10 lowers battery-material scores because EV CAPA, idle plants, and contract cancellations can quickly become 4C.",
    ),
    _target(
        "BATTERY_EQUIPMENT_PARTS",
        E2RArchetype.BATTERY_EQUIPMENT_PARTS,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        _w(18, 16, 10, 10, 8, 8, 10),
...
ROUND161_CASE_CANDIDATES: tuple[Round161CaseCandidate, ...] = (
    Round161CaseCandidate(
        ...
        ("round_161.md Reuters LGES $4.3B LFP contract",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters Tesla-LGES Megapack 3 Lansing confirmation",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters SK On Flatiron ESS deal",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters GM-LG Ohio battery plant",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters SK On Ford JV dissolution",),
    ...
    Round161CaseCandidate(
        ...
        ("round_161.md Financial Times Ford Energy storage pivot",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters Ford LGES contract cancellation",),
    ...
    Round161CaseCandidate(
        ...
        "ev_battery_contract_cancellation_hard_4c",
        "needs_price_backfill",
        ("round_161.md LGES-Freudenberg contract cancellation",),
    ...
    Round161CaseCandidate(
        ...
        ("round_161.md Reuters Redwood recycling funding",),
    ...
    Round161CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_161.md second-life battery grid storage SOH guardrail",),
    ...
    Round161CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_161.md arXiv graphite battery supply security",),
    ...
    Round161CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_161.md Reuters EQT KJ Environment platform",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters Hyundai hydrogen fuel-cell plant",),
    ...
    Round161CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_161.md Reuters Qcells customs detention",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters Orsted Sunrise Wind impairment",),
    ...
    Round161CaseCandidate(
        ...
        "missing_price_data",
        ("round_161.md Reuters lithium prices after crash",),
    ...
    Round161CaseCandidate(
        ...
        "missing_price_data",
        ("round_161.md Reuters energy storage lithium demand",),
    ...
    Round161CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_161.md Reuters lithium ESS demand sodium-ion risk",),
    ...
    Round161CaseCandidate(
        ...
        "missing_price_data",
        ("round_161.md Reuters Korea EV battery certification",),
    ...
    Round161CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_161.md Guardian Moss Landing battery storage fire",),
    ...
    Round161CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_161.md arXiv battery health validation",),
    ...
    Round161CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_161.md Reuters Volkswagen QuantumScape solid-state license",),
    ...
    Round161CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_161.md speculative battery technology guardrail",),
...
ROUND161_PRICE_FIELDS: tuple[str, ...] = (
...
def round161_target_for(target_id: str) -> Round161ScoreTarget | None:
    for target in ROUND161_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round161_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND161_CASE_CANDIDATES:
        target = round161_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round161 R3 Loop-10 case for {candidate.target_id}; "
            ...
            green_guardrails=(
                ...
                "stage2_visibility_is_not_stage3_operating_confirmation",
                ...
                "stage3_requires_revenue_recognition_ess_opm_utilization_fcf_eps_revision",
...
def round161_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round161_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND161_CASE_CANDIDATES:
        target = round161_target_for(candidate.target_id)
...
def round161_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round161_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round161_backfill": "true"} for field in ROUND161_PRICE_FIELDS)


def round161_base_score_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
        ...
        for row in ROUND161_BASE_SCORE_WEIGHTS
    )


def round161_stage_cap_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "cap_id": row.cap_id,
        ...
        for row in ROUND161_STAGE_CAPS
    )


def round161_score_stage_price_alignment_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "case_id": row.case_id,
        ...
        for row in ROUND161_SCORE_STAGE_PRICE_ALIGNMENT
    )


def round161_summary() -> dict[str, int | bool]:
    records = round161_case_records()
    source_target_count = 23
    return {
        "target_count": len(ROUND161_SCORE_TARGETS),
        "source_target_count": source_target_count,
        "helper_overlay_target_count": len(ROUND161_SCORE_TARGETS) - source_target_count,
        "case_candidate_count": len(records),
        "base_score_component_count": len(ROUND161_BASE_SCORE_WEIGHTS),
        "stage_cap_count": len(ROUND161_STAGE_CAPS),
        "score_stage_price_alignment_count": len(ROUND161_SCORE_STAGE_PRICE_ALIGNMENT),
        ...
        "green_possible_count": sum(1 for target in ROUND161_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND161_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND161_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND161_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round161_r3_loop10_reports(
    *,
    output_directory: str | Path = ROUND161_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND161_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND161_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round161_r3_loop10_battery_ev_green_summary.md",
        "case_matrix": output / "round161_r3_loop10_case_matrix.csv",
        "stage_date_plan": output / "round161_r3_loop10_stage_date_plan.csv",
        "green_guardrails": output / "round161_r3_loop10_green_guardrails.md",
        "risk_overlays": output / "round161_r3_loop10_risk_overlays.md",
        "price_validation_plan": output / "round161_r3_loop10_price_validation_plan.md",
        "price_fields": output / "round161_r3_loop10_price_fields.csv",
        "base_score_weights": output / "round161_r3_loop10_base_score_weights.csv",
        "stage_caps": output / "round161_r3_loop10_stage_caps.csv",
        "score_stage_price_alignment": output / "round161_r3_loop10_score_stage_price_alignment.csv",
        "score_stage_price_alignment_md": output / "round161_r3_loop10_score_stage_price_alignment.md",
    }
    _write_case_jsonl(round161_case_records(), cases)
    _write_rows(round161_score_profile_rows(), score_profiles)
    _write_rows(round161_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round161_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round161_price_field_rows(), paths["price_fields"])
    _write_rows(round161_base_score_weight_rows(), paths["base_score_weights"])
    _write_rows(round161_stage_cap_rows(), paths["stage_caps"])
    _write_rows(round161_score_stage_price_alignment_rows(), paths["score_stage_price_alignment"])
    paths["summary"].write_text(render_round161_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round161_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round161_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round161_price_validation_plan_markdown(), encoding="utf-8")
    paths["score_stage_price_alignment_md"].write_text(
        render_round161_score_stage_price_alignment_markdown(),
        encoding="utf-8",
    )
    return paths


def render_round161_summary_markdown() -> str:
    summary = round161_summary()
    lines = [
        "# Round-161 R3 Loop-10 Battery / EV / Green-Energy Summary",
        "",
        f"- source_round: `{ROUND161_SOURCE_ROUND_PATH}`",
        "- large_sector: `BATTERY_EV_GREEN`",
        "- loop: `R3 Loop 10 / v10.0`",
        ...
        "## Interpretation",
        "",
        "- R3 Loop 10 says Stage 2 visibility and Stage 3 operating confirmation are different things.",
        "- The source round names 23 primary targets; this pack keeps 3 adjacent R3 guardrail targets so hydrogen, carbon-compliance, and speculative battery technology remain separated.",
        "- Example: LGES-Tesla Megapack 3 has customer, use case, value, duration, Lansing production, and 2027 start evidence, so Stage 2 is reasonable.",
        "- Example: the same case is not Stage 3-Green until production ramp, utilization, ESS OPM, FCF, and EPS revision are visible.",
        "- Example: Ford Energy can validate EV-to-ESS pivot price reaction, but KR suppliers cannot inherit that evidence without direct contracts.",
        "- Example: QuantumScape-VW licensing can trigger event premium and 4B-watch, but license is not mass production.",
        "- Example: LGES-Ford cancellation, Ultium Ohio idle, Qcells UFLPA, and Orsted impairment are hard 4C evidence.",
        "- Example: BESS fire and SOH opacity are safety/transparency caps, not positive ESS or recycling evidence.",
    ]
    return "\n".join(lines) + "\n"


def render_round161_green_guardrail_markdown() -> str:
    lines = [
        "# Round-161 R3 Loop-10 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply R3 Loop-10 v10.0 weights to production scoring yet.",
...
def render_round161_risk_overlay_markdown() -> str:
    lines = [
        "# Round-161 R3 Loop-10 Risk Overlays",
        "",
...
def render_round161_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-161 R3 Loop-10 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round161_case_candidate_rows():
...
def render_round161_score_stage_price_alignment_markdown() -> str:
    lines = [
        "# Round-161 R3 Loop-10 Score / Stage / Price Alignment",
        "",
        "Round 161 checks whether the score table's stage interpretation matches the observed operating and price path.",
...
def _score_price_alignment(candidate: Round161CaseCandidate) -> str:
...
def _rerating_result(candidate: Round161CaseCandidate) -> str:
...
def _score_weight_hint(target: Round161ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
