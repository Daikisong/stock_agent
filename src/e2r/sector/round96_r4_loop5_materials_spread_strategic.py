"""Round-96 R4 Loop-5 materials, spread, and strategic-resources pack.

Round 96 tightens R4 by separating commodity price cycles, spread rebounds,
event premiums, export-control macro events, rare-earth magnet supply-chain
evidence, copper processing cost overlays, gold-miner jurisdiction rerating,
and true price-floor/offtake rerating. It is calibration/report material only:
...
ROUND96_SOURCE_ROUND_PATH = "docs/round/round_96.md"
ROUND96_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round96_r4_loop5_materials_spread_strategic"
ROUND96_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r4_loop5_round96.jsonl"
ROUND96_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round96_r4_loop5_v5.csv"


@dataclass(frozen=True)
class Round96ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round96ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round96ScoreWeightDraft
...
) -> Round96ScoreWeightDraft:
    return Round96ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round96ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND96_SCORE_TARGETS: tuple[Round96ScoreTarget, ...] = (
    _target(
        "REFINING_OIL_SPREAD",
...
ROUND96_CASE_CANDIDATES: tuple[Round96CaseCandidate, ...] = (
    Round96CaseCandidate(
        ...
        ("round_96.md AP MP Materials Pentagon agreement",),
    ...
    Round96CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_96.md AP MP Materials Apple magnet agreement",),
    ...
    Round96CaseCandidate(
        ...
        ("round_96.md Reuters rare-earth export controls",),
    ...
    Round96CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_96.md Reuters rare-earth approval delay",),
    ...
    Round96CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_96.md WSJ copper record high",),
    ...
    Round96CaseCandidate(
        ...
        ("round_96.md Reuters Barrick Q1 profit and buyback",),
    ...
    Round96CaseCandidate(
        ...
        "gold_miner_jurisdiction_rerating",
        "missing_direct_symbol_mapping",
        ("round_96.md Reuters Equinox Orla gold producer merger",),
    ...
    Round96CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_96.md Reuters POSCO Alaska LNG deal",),
    ...
    Round96CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_96.md Reuters SK Innovation Q1 profit",),
    ...
    Round96CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_96.md Reuters Korean petrochemical oversupply",),
    ...
    Round96CaseCandidate(
        ...
        "missing_price_data",
        ("round_96.md Reuters lithium prices after crash",),
    ...
    Round96CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_96.md FT BHP dividend cut",),
    ...
    Round96CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_96.md Reuters Korea Zinc tender offer",),
    ...
    Round96CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_96.md Reuters Korea Zinc share issuance probe",),
    ...
    Round96CaseCandidate(
        ...
        ("round_96.md Reuters International Paper DS Smith divestment",),
    ...
    Round96CaseCandidate(
        ...
        "missing_price_data",
        ("round_96.md advanced material speculative theme note",),
...
ROUND96_PRICE_FIELDS: tuple[str, ...] = (
...
def round96_target_for(target_id: str) -> Round96ScoreTarget | None:
...
def round96_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND96_CASE_CANDIDATES:
        target = round96_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round96 R4 Loop-5 case for {candidate.target_id}; "
...
def round96_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round96_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND96_CASE_CANDIDATES:
        target = round96_target_for(candidate.target_id)
...
def round96_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round96_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round96_backfill": "true"} for field in ROUND96_PRICE_FIELDS)


def round96_summary() -> dict[str, int | bool]:
    records = round96_case_records()
    return {
        "target_count": len(ROUND96_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND96_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND96_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND96_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND96_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round96_r4_loop5_reports(
    *,
    output_directory: str | Path = ROUND96_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND96_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND96_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round96_r4_loop5_materials_spread_strategic_summary.md",
        "case_matrix": output / "round96_r4_loop5_case_matrix.csv",
        "stage_date_plan": output / "round96_r4_loop5_stage_date_plan.csv",
        "green_guardrails": output / "round96_r4_loop5_green_guardrails.md",
        "risk_overlays": output / "round96_r4_loop5_risk_overlays.md",
        "price_validation_plan": output / "round96_r4_loop5_price_validation_plan.md",
        "price_fields": output / "round96_r4_loop5_price_fields.csv",
    }
    _write_case_jsonl(round96_case_records(), cases)
    _write_rows(round96_score_profile_rows(), score_profiles)
    _write_rows(round96_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round96_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round96_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round96_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round96_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round96_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round96_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round96_summary_markdown() -> str:
    summary = round96_summary()
    lines = [
        "# Round-96 R4 Loop-5 Materials / Spread / Strategic Resources Summary",
        "",
        f"- source_round: `{ROUND96_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R4 Loop 5 says price increase and structural rerating are different things.",
        ...
        "- Example: rare-earth magnets need customer qualification, production ramp, feedstock, and FCF; export-control headlines alone are not enough.",
        "- Example: copper AI-grid demand is useful, but stockpile distortion and mine restart can make it only Watch.",
        "- Example: gold miners can rerate on safer jurisdictions and buybacks, but AISC, production, M&A dilution, and gold-price reversal still matter.",
...
def render_round96_green_guardrail_markdown() -> str:
    lines = [
        "# Round-96 R4 Loop-5 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply R4 Loop-5 v5.0 weights to production scoring yet.",
...
def render_round96_risk_overlay_markdown() -> str:
    lines = [
        "# Round-96 R4 Loop-5 Risk Overlays",
        "",
...
def render_round96_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-96 R4 Loop-5 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round96_case_candidate_rows():
...
def _score_price_alignment(candidate: Round96CaseCandidate) -> str:
...
def _rerating_result(candidate: Round96CaseCandidate) -> str:
...
def _score_weight_hint(target: Round96ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
...
__all__ = [
    ...
    "ROUND96_DEFAULT_CASES_PATH",
    "ROUND96_DEFAULT_OUTPUT_DIRECTORY",
    "ROUND96_DEFAULT_SCORE_PROFILE_PATH",
