"""Round-95 R3 Loop-5 battery, EV, and green-energy pack.

Round 95 tightens the R3 split again after R2 Loop 5. It separates Tesla
Megapack supply-chain confirmation, EV CAPA contract cancellation, ESS LFP
grid storage, AI data-center storage, EV-to-ESS redeployment, disclosure-
confidence caps, BESS/EV safety gates, SOH transparency, and speculative
battery technology. It keeps the core rule explicit: EV growth narrative is
not durable EPS/FCF evidence unless contracts, customers, margins,
utilization, and FCF are visible as-of the case date.
...
ROUND95_SOURCE_ROUND_PATH = "docs/round/round_95.md"
ROUND95_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round95_r3_loop5_battery_ev_green"
ROUND95_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r3_loop5_round95.jsonl"
ROUND95_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round95_r3_loop5_v5.csv"


@dataclass(frozen=True)
class Round95ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round95ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round95ScoreWeightDraft
...
) -> Round95ScoreWeightDraft:
    return Round95ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round95ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND95_SCORE_TARGETS: tuple[Round95ScoreTarget, ...] = (
    _target(
        ...
        note="Loop 5 lowers battery-material scores because EV CAPA, idle plants, and contract cancellations can quickly become 4C.",
    ),
    _target(
...
ROUND95_CASE_CANDIDATES: tuple[Round95CaseCandidate, ...] = (
    Round95CaseCandidate(
        ...
        ("round_95.md Reuters LGES $4.3B LFP contract",),
    ...
    Round95CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_95.md Reuters Tesla-LGES Megapack 3 Lansing confirmation",),
    ...
    Round95CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_95.md Reuters SK On Flatiron ESS deal",),
    ...
    Round95CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_95.md Reuters GM-LG Ohio battery plant",),
    ...
    Round95CaseCandidate(
        ...
        ("round_95.md Financial Times Ford Energy storage pivot",),
    ...
    Round95CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_95.md Reuters Ford LGES contract cancellation",),
    ...
    Round95CaseCandidate(
        ...
        "ev_battery_contract_cancellation_hard_4c",
        "needs_price_backfill",
        ("round_95.md LGES-Freudenberg contract cancellation",),
    ...
    Round95CaseCandidate(
        ...
        ("round_95.md Reuters Redwood recycling funding",),
    ...
    Round95CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_95.md Reuters EQT KJ Environment platform",),
    ...
    Round95CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_95.md Reuters Hyundai hydrogen fuel-cell plant",),
    ...
    Round95CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_95.md Reuters Qcells customs detention",),
    ...
    Round95CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_95.md Reuters Orsted Sunrise Wind impairment",),
    ...
    Round95CaseCandidate(
        ...
        "missing_price_data",
        ("round_95.md Reuters lithium prices after crash",),
    ...
    Round95CaseCandidate(
        ...
        "missing_price_data",
        ("round_95.md Reuters energy storage lithium demand",),
    ...
    Round95CaseCandidate(
        ...
        "missing_price_data",
        ("round_95.md Reuters Korea EV battery certification",),
    ...
    Round95CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_95.md AP News Moss Landing battery storage fire",),
    ...
    Round95CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_95.md arXiv battery health validation",),
    ...
    Round95CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_95.md speculative battery technology guardrail",),
...
ROUND95_PRICE_FIELDS: tuple[str, ...] = (
...
def round95_target_for(target_id: str) -> Round95ScoreTarget | None:
...
def round95_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND95_CASE_CANDIDATES:
        target = round95_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round95 R3 Loop-5 case for {candidate.target_id}; "
...
def round95_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round95_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND95_CASE_CANDIDATES:
        target = round95_target_for(candidate.target_id)
...
def round95_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round95_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round95_backfill": "true"} for field in ROUND95_PRICE_FIELDS)


def round95_summary() -> dict[str, int | bool]:
    records = round95_case_records()
    return {
        "target_count": len(ROUND95_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND95_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND95_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND95_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND95_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round95_r3_loop5_reports(
    *,
    output_directory: str | Path = ROUND95_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND95_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND95_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round95_r3_loop5_battery_ev_green_summary.md",
        "case_matrix": output / "round95_r3_loop5_case_matrix.csv",
        "stage_date_plan": output / "round95_r3_loop5_stage_date_plan.csv",
        "green_guardrails": output / "round95_r3_loop5_green_guardrails.md",
        "risk_overlays": output / "round95_r3_loop5_risk_overlays.md",
        "price_validation_plan": output / "round95_r3_loop5_price_validation_plan.md",
        "price_fields": output / "round95_r3_loop5_price_fields.csv",
    }
    _write_case_jsonl(round95_case_records(), cases)
    _write_rows(round95_score_profile_rows(), score_profiles)
    _write_rows(round95_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round95_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round95_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round95_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round95_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round95_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round95_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round95_summary_markdown() -> str:
    summary = round95_summary()
    lines = [
        "# Round-95 R3 Loop-5 Battery / EV / Green-Energy Summary",
        "",
        f"- source_round: `{ROUND95_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R3 Loop 5 says EV growth and durable FCF are different things.",
...
def render_round95_green_guardrail_markdown() -> str:
    lines = [
        "# Round-95 R3 Loop-5 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply R3 Loop-5 v5.0 weights to production scoring yet.",
...
def render_round95_risk_overlay_markdown() -> str:
    lines = [
        "# Round-95 R3 Loop-5 Risk Overlays",
        "",
...
def render_round95_price_validation_plan_markdown() -> str:
    lines = [
    ...
    for row in round95_case_candidate_rows():
...
def _score_price_alignment(candidate: Round95CaseCandidate) -> str:
...
def _rerating_result(candidate: Round95CaseCandidate) -> str:
...
def _score_weight_hint(target: Round95ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
