"""Round-85 R6 Loop-4 financial, capital-allocation, and digital-finance pack.

Round 85 tightens R6 by separating policy/value-up/fintech/stablecoin names
from verified execution: ROE, CET1, K-ICS, CSM, credit cost, treasury-share
cancellation, dividends, take rate, FCF, reserve, redemption, bank exchange
stakes, and exchange security.
...
ROUND85_SOURCE_ROUND_PATH = "docs/round/round_85.md"
ROUND85_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round85_r6_loop4_financial_capital_digital"
ROUND85_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r6_loop4_round85.jsonl"
ROUND85_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round85_r6_loop4_v4.csv"


@dataclass(frozen=True)
class Round85ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round85ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round85ScoreWeightDraft
...
def _w(
    ...
    capital: int | str,
    confidence: int | str,
) -> Round85ScoreWeightDraft:
    return Round85ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round85ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND85_SCORE_TARGETS: tuple[Round85ScoreTarget, ...] = (
    _target(
...
ROUND85_CASE_CANDIDATES: tuple[Round85CaseCandidate, ...] = (
    Round85CaseCandidate(
        ...
        ("round_85.md Reuters commercial act treasury cancellation",),
    ...
    Round85CaseCandidate(
        ...
        ("round_85.md Reuters SK Square buybacks",),
    ...
    Round85CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_85.md Reuters Samsung treasury cancellation",),
    ...
    Round85CaseCandidate(
        ...
        ("round_85.md section 4-4 bank and financial holding candidate",),
        "Round 85 keeps banks and financial holdings Green-capable only after ROE, CET1, credit cost, PF exposure, and return execution are verified.",
    ),
    Round85CaseCandidate(
        ...
        "needs_named_case_and_price_backfill",
        ("round_85.md section 4-5 insurance underwriting candidate",),
        "Round 85 keeps insurance Green-capable, but separates CSM quality, K-ICS, loss ratio, and investment-loss risks from bank value-up.",
    ),
    Round85CaseCandidate(
        ...
        ("round_85.md Financial Times Samsung C&T activist rejection",),
    ...
    Round85CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_85.md Reuters Korea Zinc tender offer",),
    ...
    Round85CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_85.md Reuters Korea Zinc share issuance probe",),
    ...
    Round85CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_85.md Reuters Korea tax policy shock",),
    ...
    Round85CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_85.md Barron's AI windfall tax policy shock",),
    ...
    Round85CaseCandidate(
        ...
        ("round_85.md Reuters Stripe tender offer",),
    ...
    Round85CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_85.md Reuters Mynt GCash IPO",),
    ...
    Round85CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_85.md Reuters Toss global push won stablecoin",),
        "Toss fintech is a success candidate, while won stablecoin remains Watch before regulation, volume, reserve, and fees.",
        (E2RArchetype.PAYMENT_FINTECH_INFRA, E2RArchetype.DIGITAL_ASSET_TOKENIZATION, E2RArchetype.REGULATED_STABLECOIN_INFRA),
    ),
    Round85CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_85.md Reuters Circle IPO valuation",),
    ...
    Round85CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_85.md Reuters Bank of England stablecoin regulation",),
    ...
    Round85CaseCandidate(
        ...
        "needs_source_date_and_price_backfill",
        ("round_85.md Reuters TerraUSD Do Kwon",),
    ...
    Round85CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_85.md Reuters Hana Bank Dunamu stake",),
    ...
    Round85CaseCandidate(
        ...
        "needs_named_case_and_price_backfill",
        ("round_85.md digital asset exchange security cycle checklist",),
...
ROUND85_PRICE_FIELDS: tuple[str, ...] = (
    ...
    "security_incident_flag", "fcf_margin", "ipo_valuation", "ipo_timeline_status",
...
def round85_target_for(target_id: str) -> Round85ScoreTarget | None:
...
def round85_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND85_CASE_CANDIDATES:
        target = round85_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round85 R6 Loop-4 case for {candidate.target_id}; "
...
def round85_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round85_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND85_CASE_CANDIDATES:
        target = round85_target_for(candidate.target_id)
...
def round85_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round85_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round85_backfill": "true"} for field in ROUND85_PRICE_FIELDS)


def round85_summary() -> dict[str, int | bool]:
    records = round85_case_records()
    return {
        "target_count": len(ROUND85_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND85_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND85_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND85_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND85_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round85_r6_loop4_reports(
    *,
    output_directory: str | Path = ROUND85_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND85_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND85_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round85_r6_loop4_financial_capital_digital_summary.md",
        "case_matrix": output / "round85_r6_loop4_case_matrix.csv",
        "stage_date_plan": output / "round85_r6_loop4_stage_date_plan.csv",
        "green_guardrails": output / "round85_r6_loop4_green_guardrails.md",
        "risk_overlays": output / "round85_r6_loop4_risk_overlays.md",
        "price_validation_plan": output / "round85_r6_loop4_price_validation_plan.md",
        "price_fields": output / "round85_r6_loop4_price_fields.csv",
    }
    _write_case_jsonl(round85_case_records(), cases)
    _write_rows(round85_score_profile_rows(), score_profiles)
    _write_rows(round85_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round85_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round85_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round85_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round85_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round85_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round85_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round85_summary_markdown() -> str:
    summary = round85_summary()
    lines = [
        "# Round-85 R6 Loop-4 Financial / Capital / Digital Summary",
        "",
        f"- source_round: `{ROUND85_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R6 Loop 4 says low PBR, value-up policy, and digital-asset labels are not enough; execution decides.",
...
def render_round85_green_guardrail_markdown() -> str:
    lines = [
        "# Round-85 R6 Loop-4 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply R6 Loop-4 v4.0 weights to production scoring yet.",
...
def render_round85_risk_overlay_markdown() -> str:
    lines = [
        "# Round-85 R6 Loop-4 Risk Overlays",
        "",
...
def render_round85_price_validation_plan_markdown() -> str:
    lines = [
    ...
    for row in round85_case_candidate_rows():
...
def _score_price_alignment(candidate: Round85CaseCandidate) -> str:
...
def _rerating_result(candidate: Round85CaseCandidate) -> str:
...
def _score_weight_hint(target: Round85ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
