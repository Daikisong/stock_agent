"""Round-98 R6 Loop-5 financial, capital-allocation, and digital-finance pack.

Round 98 tightens R6 by separating policy/value-up/fintech/stablecoin names
...
ROUND98_SOURCE_ROUND_PATH = "docs/round/round_98.md"
ROUND98_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round98_r6_loop5_financial_capital_digital"
ROUND98_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r6_loop5_round98.jsonl"
ROUND98_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round98_r6_loop5_v5.csv"


@dataclass(frozen=True)
class Round98ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round98ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round98ScoreWeightDraft
...
def _w(
    ...
    capital: int | str,
    confidence: int | str,
) -> Round98ScoreWeightDraft:
    return Round98ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round98ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND98_SCORE_TARGETS: tuple[Round98ScoreTarget, ...] = (
    _target(
...
ROUND98_CASE_CANDIDATES: tuple[Round98CaseCandidate, ...] = (
    Round98CaseCandidate(
        ...
        ("round_98.md Reuters commercial act treasury cancellation",),
    ...
    Round98CaseCandidate(
        ...
        ("round_98.md Reuters SK Square buybacks",),
    ...
    Round98CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_98.md Reuters Samsung treasury cancellation",),
    ...
    Round98CaseCandidate(
        ...
        ("round_98.md section 4-4 bank and financial holding candidate",),
        "Round 98 keeps banks and financial holdings Green-capable only after ROE, CET1, credit cost, PF exposure, and return execution are verified.",
        (E2RArchetype.FINANCIAL_SPREAD_BALANCE_SHEET,),
    ),
    Round98CaseCandidate(
        ...
        "needs_named_case_and_price_backfill",
        ("round_98.md section 4-5 insurance underwriting candidate",),
        "Round 98 keeps insurance Green-capable, but separates CSM quality, K-ICS, loss ratio, and investment-loss risks from bank value-up.",
    ),
    Round98CaseCandidate(
        ...
        ("round_98.md Financial Times Samsung C&T activist rejection",),
    ...
    Round98CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_98.md Reuters Korea Zinc tender offer",),
    ...
    Round98CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_98.md Reuters Korea Zinc share issuance probe",),
    ...
    Round98CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_98.md Reuters Korea tax policy shock",),
    ...
    Round98CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_98.md Barron's AI windfall tax policy shock",),
    ...
    Round98CaseCandidate(
        ...
        ("round_98.md Reuters Mynt GCash IPO",),
    ...
    Round98CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_98.md Reuters Clear Street IPO valuation cut",),
    ...
    Round98CaseCandidate(
        ...
        "missing_public_price_data",
        ("round_98.md Reuters Toss global push won stablecoin",),
    ...
    Round98CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_98.md Reuters Circle IPO valuation",),
    ...
    Round98CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_98.md Reuters Bank of England stablecoin regulation",),
    ...
    Round98CaseCandidate(
        ...
        "needs_source_date_and_price_backfill",
        ("round_98.md Reuters TerraUSD Do Kwon",),
    ...
    Round98CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_98.md Reuters Hana Bank Dunamu stake",),
    ...
    Round98CaseCandidate(
        ...
        ("round_98.md WSJ Dunamu Naver Financial merger option",),
    ...
    Round98CaseCandidate(
        ...
        "needs_named_case_and_price_backfill",
        ("round_98.md digital asset exchange security cycle checklist",),
...
ROUND98_PRICE_FIELDS: tuple[str, ...] = (
...
def round98_target_for(target_id: str) -> Round98ScoreTarget | None:
...
def round98_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND98_CASE_CANDIDATES:
        target = round98_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round98 R6 Loop-5 case for {candidate.target_id}; "
...
def round98_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round98_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND98_CASE_CANDIDATES:
        target = round98_target_for(candidate.target_id)
...
def round98_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round98_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round98_backfill": "true"} for field in ROUND98_PRICE_FIELDS)


def round98_summary() -> dict[str, int | bool]:
    records = round98_case_records()
    return {
        "target_count": len(ROUND98_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND98_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND98_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND98_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND98_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round98_r6_loop5_reports(
    *,
    output_directory: str | Path = ROUND98_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND98_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND98_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round98_r6_loop5_financial_capital_digital_summary.md",
        "case_matrix": output / "round98_r6_loop5_case_matrix.csv",
        "stage_date_plan": output / "round98_r6_loop5_stage_date_plan.csv",
        "green_guardrails": output / "round98_r6_loop5_green_guardrails.md",
        "risk_overlays": output / "round98_r6_loop5_risk_overlays.md",
        "price_validation_plan": output / "round98_r6_loop5_price_validation_plan.md",
        "price_fields": output / "round98_r6_loop5_price_fields.csv",
    }
    _write_case_jsonl(round98_case_records(), cases)
    _write_rows(round98_score_profile_rows(), score_profiles)
    _write_rows(round98_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round98_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round98_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round98_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round98_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round98_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round98_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round98_summary_markdown() -> str:
    summary = round98_summary()
    lines = [
        "# Round-98 R6 Loop-5 Financial / Capital / Digital Summary",
        "",
        f"- source_round: `{ROUND98_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R6 Loop 5 says low PBR, value-up policy, and digital-asset labels are not enough; execution decides.",
...
def render_round98_green_guardrail_markdown() -> str:
    lines = [
        "# Round-98 R6 Loop-5 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply R6 Loop-5 v5.0 weights to production scoring yet.",
...
def render_round98_risk_overlay_markdown() -> str:
    lines = [
        "# Round-98 R6 Loop-5 Risk Overlays",
        "",
...
def render_round98_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-98 R6 Loop-5 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round98_case_candidate_rows():
...
def _score_price_alignment(candidate: Round98CaseCandidate) -> str:
...
def _rerating_result(candidate: Round98CaseCandidate) -> str:
...
def _score_weight_hint(target: Round98ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
...
__all__ = [
    ...
    "ROUND98_DEFAULT_CASES_PATH",
    "ROUND98_DEFAULT_OUTPUT_DIRECTORY",
    "ROUND98_DEFAULT_SCORE_PROFILE_PATH",
