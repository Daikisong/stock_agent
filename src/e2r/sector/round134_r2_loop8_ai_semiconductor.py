"""Round-134 R2 Loop-8 AI, semiconductor, and electronics pack.

Round 134 returns to R2 after the R1 Loop-8 pass. It separates HBM leaders,
...
ROUND134_SOURCE_ROUND_PATH = "docs/round/round_134.md"
ROUND134_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round134_r2_loop8_ai_semiconductor"
ROUND134_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r2_loop8_round134.jsonl"
ROUND134_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round134_r2_loop8_v8.csv"


@dataclass(frozen=True)
class Round134ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round134ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round134ScoreWeightDraft
...
@dataclass(frozen=True)
class Round134ScoreStagePriceAlignment:
    case_id: str
    detected_stage: str
...
) -> Round134ScoreWeightDraft:
    return Round134ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)


ROUND134_BASE_SCORE_WEIGHTS: tuple[Round134BaseScoreWeight, ...] = (
    Round134BaseScoreWeight("eps_fcf_revision", 25, "up", "SK Hynix, Applied Materials, and Broadcom show price paths follow OP/EPS/FCF evidence."),
    Round134BaseScoreWeight("customer_shipment_revenue_visibility", 22, "up", "Customer names, shipments, AI orders, and guidance create Stage 2 signal."),
    Round134BaseScoreWeight("bottleneck_pricing_power", 19, "hold_up", "HBM, NAND, TSMC capacity, optical PCB, and photonics bottlenecks still matter."),
    Round134BaseScoreWeight("market_mispricing_gap", 10, "cap_after_recognition", "The old memory/cyclical or generic server frame must still be wrong, but SK Hynix/Kioxia-like rerating quickly reduces the gap."),
    Round134BaseScoreWeight("valuation_room_4b_runway", 8, "cap_after_rerating", "SK Hynix, Kioxia, and IPO cases show good structures can already be 4B."),
    Round134BaseScoreWeight("capital_discipline_fcf_stability", 8, "up", "CoreWeave, Cerebras, and Ecolab-CoolIT make FCF, debt, circular financing, and M&A price central."),
    Round134BaseScoreWeight("information_confidence_disclosure_detail", 8, "up", "Supermicro and list-only AI disclosures require accounting, customer, amount, period, and parser confidence checks."),
)


ROUND134_STAGE_CAPS: tuple[Round134StageCap, ...] = (
    Round134StageCap(
        "Stage 1",
        "45",
...
ROUND134_SCORE_STAGE_PRICE_ALIGNMENT: tuple[Round134ScoreStagePriceAlignment, ...] = (
    Round134ScoreStagePriceAlignment(
...
ROUND134_SCORE_TARGETS: tuple[Round134ScoreTarget, ...] = (
    Round134ScoreTarget(
        "MEMORY_HBM_CAPACITY",
        ...
        "Loop 8 keeps HBM Green-capable, but valuation credit is reduced because successful HBM cases can already be 4B-watch.",
    ),
    Round134ScoreTarget(
        ...
        "Loop 8 splits HBM LTA/prepayment from general HBM because price bands, prepayment, and reserved capacity are stronger visibility than headlines.",
    ),
    Round134ScoreTarget(
    ...
    Round134ScoreTarget(
        ...
        "Loop 8 separates supply reallocation from simple memory price rebound: consumer exit must become AI/HBM/enterprise mix and EPS evidence.",
    ),
    Round134ScoreTarget(
    ...
    Round134ScoreTarget(
        ...
        "Loop 8 splits hyperscaler custom ASICs from generic AI chips because named customer, capacity, margin, and repeat revenue matter.",
    ),
    Round134ScoreTarget(
...
ROUND134_CASE_CANDIDATES: tuple[Round134CaseCandidate, ...] = (
    Round134CaseCandidate(
        "sk_hynix_hbm_trillion_case",
        "MEMORY_HBM_CAPACITY",
        "000660",
        ...
        ("round_134.md Reuters SK Hynix AI boom market value",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters SK Hynix record profit and buyback",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Samsung HBM4 shipment",),
    ...
    Round134CaseCandidate(
        ...
        "hbm_catchup_execution_candidate",
        "needs_price_backfill",
        ("round_134.md Reuters Samsung AMD AI memory MOU",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Samsung labor strike plan",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Micron consumer memory exit",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Kioxia AI profit", "round_134.md FT Kioxia profit surge"),
    ...
    Round134CaseCandidate(
        ...
        "semi_capex_aligned",
        "needs_price_backfill",
        ("round_134.md Reuters Applied Materials guidance",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Broadcom custom AI chip demand",),
    ...
    Round134CaseCandidate(
        ...
        "missing_price_data",
        ("round_134.md custom ASIC margin concentration guardrail",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Nvidia CoWoS-L reference",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Broadcom supply constraints",),
        "Optical networking/PCB bottleneck becomes stronger in Loop 8 because lead time expanded from weeks to months.",
        (E2RArchetype.ADVANCED_PACKAGING_PCB,),
    ),
    Round134CaseCandidate(
        "cisco_ai_networking_orders_case",
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Cisco AI orders",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Tower Semiconductor AI chip deals",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Foxconn Q1 profit",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Ecolab CoolIT acquisition",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md FT OpenAI CoreWeave contract",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters CoreWeave OpenAI expanded pact",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters CoreWeave circular financing watch",),
    ...
    Round134CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_134.md Reuters Supermicro EY resignation",),
    ...
    Round134CaseCandidate(
        ...
        "missing_price_data",
        ("round_134.md CXL/glass substrate guardrail",),
    ...
    Round134CaseCandidate(
        ...
        "theme_without_revenue",
        "missing_price_data",
        ("round_134.md FuriosaAI related stock watch",),
    ...
    Round134CaseCandidate(
        ...
        "missing_price_data",
        ("round_134.md AI CAPEX crowding overlay",),
...
ROUND134_PRICE_FIELDS: tuple[str, ...] = (
    "case_id",
    "symbol",
    "company_name",
    "primary_archetype",
...
def round134_target_for(target_id: str) -> Round134ScoreTarget | None:
    for target in ROUND134_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round134_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND134_CASE_CANDIDATES:
        target = round134_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round134 R2 Loop-8 case for {candidate.target_id}; "
...
def round134_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round134_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND134_CASE_CANDIDATES:
        target = round134_target_for(candidate.target_id)
...
def round134_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round134_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round134_backfill": "true"} for field in ROUND134_PRICE_FIELDS)


def round134_base_score_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(weight.as_row() for weight in ROUND134_BASE_SCORE_WEIGHTS)


def round134_stage_cap_rows() -> tuple[dict[str, str], ...]:
    return tuple(cap.as_row() for cap in ROUND134_STAGE_CAPS)


def round134_score_stage_price_alignment_rows() -> tuple[dict[str, str], ...]:
    return tuple(alignment.as_row() for alignment in ROUND134_SCORE_STAGE_PRICE_ALIGNMENT)


def round134_summary() -> dict[str, int | bool]:
    records = round134_case_records()
    return {
        "target_count": len(ROUND134_SCORE_TARGETS),
        "case_candidate_count": len(records),
        "base_score_component_count": len(ROUND134_BASE_SCORE_WEIGHTS),
        "stage_cap_count": len(ROUND134_STAGE_CAPS),
        "score_stage_price_alignment_count": len(ROUND134_SCORE_STAGE_PRICE_ALIGNMENT),
        ...
        "green_possible_count": sum(1 for target in ROUND134_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND134_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND134_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "hard_gate_target_count": sum(1 for target in ROUND134_SCORE_TARGETS if target.hard_gate),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round134_r2_loop8_reports(
    *,
    output_directory: str | Path = ROUND134_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND134_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND134_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round134_r2_loop8_ai_semiconductor_summary.md",
        "case_matrix": output / "round134_r2_loop8_case_matrix.csv",
        "stage_date_plan": output / "round134_r2_loop8_stage_date_plan.csv",
        "green_guardrails": output / "round134_r2_loop8_green_guardrails.md",
        "loop8_risk_overlays": output / "round134_r2_loop8_risk_overlays.md",
        "price_validation_plan": output / "round134_r2_loop8_price_validation_plan.md",
        "price_fields": output / "round134_r2_loop8_price_fields.csv",
        "base_score_weights": output / "round134_r2_loop8_base_score_weights.csv",
        "stage_caps": output / "round134_r2_loop8_stage_caps.csv",
        "score_stage_price_alignment": output / "round134_r2_loop8_score_stage_price_alignment.csv",
        "score_stage_price_alignment_md": output / "round134_r2_loop8_score_stage_price_alignment.md",
    }
    _write_case_jsonl(round134_case_records(), cases)
    _write_rows(round134_score_profile_rows(), score_profiles)
    _write_rows(round134_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round134_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round134_price_field_rows(), paths["price_fields"])
    _write_rows(round134_base_score_weight_rows(), paths["base_score_weights"])
    _write_rows(round134_stage_cap_rows(), paths["stage_caps"])
    _write_rows(round134_score_stage_price_alignment_rows(), paths["score_stage_price_alignment"])
    paths["summary"].write_text(render_round134_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round134_green_guardrail_markdown(), encoding="utf-8")
    paths["loop8_risk_overlays"].write_text(render_round134_loop8_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round134_price_validation_plan_markdown(), encoding="utf-8")
    paths["score_stage_price_alignment_md"].write_text(render_round134_score_stage_price_alignment_markdown(), encoding="utf-8")
    return paths


def render_round134_summary_markdown() -> str:
    summary = round134_summary()
    lines = [
        "# Round-134 R2 Loop-8 AI / Semiconductor / Electronics Summary",
        "",
        f"- source_round: `{ROUND134_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R2 Loop 8 says `AI 수혜` is not a score bucket.",
...
def render_round134_green_guardrail_markdown() -> str:
    lines = [
        "# Round-134 R2 Loop-8 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply R2 Loop-8 v8.0 weights to production scoring yet.",
...
def render_round134_loop8_risk_overlay_markdown() -> str:
    lines = [
        "# Round-134 R2 Loop-8 Risk Overlays",
        "",
...
def render_round134_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-134 R2 Loop-8 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round134_case_candidate_rows():
...
def render_round134_score_stage_price_alignment_markdown() -> str:
    lines = [
        "# Round-134 R2 Loop-8 Score -> Stage -> Price Alignment",
        "",
        "Round 134 uses the R2 v8 score table to test whether AI/semiconductor evidence produced the right stage and whether that stage matched the observed price path in the source note.",
        "",
        "## R2 v8 Base Score Weights",
        "",
        "| component | points | direction | reason |",
        "| --- | ---: | --- | --- |",
    ]
    for row in round134_base_score_weight_rows():
    ...
    for row in round134_stage_cap_rows():
        lines.append(
            "| "
    ...
    for row in round134_score_stage_price_alignment_rows():
        lines.append(
            "| "
...
def _score_price_alignment(candidate: Round134CaseCandidate) -> str:
...
def _rerating_result(candidate: Round134CaseCandidate) -> str:
...
def _score_weight_hint(target: Round134ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
