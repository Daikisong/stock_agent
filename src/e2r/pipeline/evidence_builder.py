...
def evidence_from_financial_actual(item: FinancialActual, market: Market) -> Evidence:
    ...
    return Evidence(
        ...
        url_or_identifier=f"feature://{item.symbol}/financial_actual/{item.period_end.isoformat()}",
...
def evidence_from_consensus(item: ConsensusSnapshot, market: Market) -> Evidence:
    ...
    return Evidence(
        ...
        url_or_identifier=f"feature://{item.symbol}/consensus/{item.date.isoformat()}/{item.fiscal_year}",
...
def evidence_from_consensus_revision(item: ConsensusRevision, market: Market) -> Evidence:
    ...
    return Evidence(
        ...
        source_name=item.source,
        source_tier=source_tier,
        url_or_identifier=f"feature://{item.symbol}/consensus_revision/{item.date.isoformat()}/{item.fiscal_year}",
        published_at=timestamp,
        observed_at=timestamp,
        available_at=timestamp,
        as_of_date=item.as_of_date,
