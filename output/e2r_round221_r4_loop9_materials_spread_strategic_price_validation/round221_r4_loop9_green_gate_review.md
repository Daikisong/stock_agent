# Round 221 R4 Green Gate Review
...
| capacity_shutdown_confirmed | raise | 4 | Daesan NCC 가동중단처럼 확정된 shutdown은 Stage 2 relief다. |
...
| tariff_resilience | raise | 4 | 관세 환경에서도 수출 마진이 방어되는지 확인한다. |
| resource_security_with_downstream_margin | raise | 4 | resource security는 downstream margin과 연결될 때만 강해진다. |
| commodity_price_up_only | lower | -5 | 원자재 가격 상승만으로는 EPS/FCF 체급 변화를 증명하지 못한다. |
| tariff_event_only | lower | -4 | 관세 headline은 수출업체에는 4C-watch가 될 수 있다. |
...
| unconfirmed_media_report | lower | -5 | 미확정 고객·M&A 보도는 event premium이다. |
| mna_rumor_without_transaction | lower | -5 | 거래가 확정되지 않은 M&A rumor는 Green 근거가 아니다. |
| lithium_price_event | lower | -5 | 리튬 가격 반등은 cycle/event로 먼저 본다. |
