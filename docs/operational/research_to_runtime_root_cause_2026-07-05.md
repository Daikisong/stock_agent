...
쉬운 예: 예전에는 한 과목(C05) 시험지만 10장 채점된 상태였고, 현재는 0개 과목의 시험지가 채점대에 올라왔다. 하지만 필수 증빙칸이 비어 있으면 전체 과목 합격은 아니다.

## Current Facts

- full_thesis_row_count: `0`
- full_thesis_by_archetype: `{}`
- distinct_full_thesis_archetype_count: `0`
- c05_full_thesis_share: `0.0`
- required_positive_missing_full_thesis_row_count: `0`
- green_gap_full_thesis_row_count: `0`
...
1. 왜 예전 production FULL_THESIS 10개가 전부 C05였고, 현재는 어떻게 바뀌었나?
   - 예전 seed target은 UNKNOWN이었고 source_primary/planner top1 경로가 C05로 쏠렸다. 현재 promoted row는 none 경로로 분산됐다.
...
4. C05가 아닌 후보는 왜 0개인가?
   - 이 질문은 예전 C05-only 산출물에 대한 질문이다. 현재 non-C05 score-path row는 none이고, mandatory missing prefix는 C06/C08/C15/C17/C24/C28다.
