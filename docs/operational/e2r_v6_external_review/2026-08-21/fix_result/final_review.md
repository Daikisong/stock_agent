...
- branch: `fix/e2r-stable-gap-fixpoint-20260821`
...
게시 HEAD와 origin 일치 여부는 최종 push 뒤 `git rev-parse HEAD`와 `git rev-parse origin/fix/e2r-stable-gap-fixpoint-20260821`로 검증한다. 문서 자체가 들어가는 commit hash를 문서 안에 하드코딩하지 않는다.
...
R1~R6의 stable gap identity, materiality, disposition, semantic fixpoint, score/Stage materiality 정책은 앞선 한글 commit들에 분리돼 있다.
