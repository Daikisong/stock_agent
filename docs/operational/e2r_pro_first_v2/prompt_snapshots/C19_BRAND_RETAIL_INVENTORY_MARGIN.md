...
- job_id: `PROMPT-SNAPSHOT-C19_BRAND_RETAIL_INVENTORY_MARGIN`
- run_id: `PROMPT-SNAPSHOT-RUN-C19_BRAND_RETAIL_INVENTORY_MARGIN`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C19_BRAND_RETAIL_INVENTORY_MARGIN]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C19_BRAND_RETAIL_INVENTORY_MARGIN]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 브랜드 수요·sell-through·재고·가격결정력이 할인/원가를 거쳐 margin과 cash로 전환되는 메커니즘
- required_bridge_axes: consumer_sell_through, margin, customer
- source roles: issuer retail/채널 KPI·유통사/플랫폼·재고/markdown·실적/margin/FCF·revision
- false-positive guard: 매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q01` — 매장/온라인 sell-through·same-store·제품/지역 mix가 무엇인가
   ...
   - primitives: sell_through_confirmed
   ...
   - guard: 매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다.
