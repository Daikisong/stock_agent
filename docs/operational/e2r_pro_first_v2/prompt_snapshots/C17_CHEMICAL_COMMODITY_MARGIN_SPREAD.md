...
- job_id: `PROMPT-SNAPSHOT-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD`
- run_id: `PROMPT-SNAPSHOT-RUN-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 화학 제품 ASP와 feedstock 원가의 실현 spread가 inventory timing·가동률을 거쳐 margin/FCF로 전환되는 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source roles: issuer 실적/제품별 ASP·원재료 공식가격·가동률/정비·고객수요·재고/FCF·revision
- false-positive guard: 원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q01` — 제품별 ASP·feedstock cost·realized spread가 무엇인가
   ...
   - guard: 원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다.
...
5. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q05` — 원재료 급등·재고손실·공급증설·수요둔화 counter는 무엇인가
   ...
   - primitives: raw_material_cost_risk
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: earnings_visibility, information_confidence
