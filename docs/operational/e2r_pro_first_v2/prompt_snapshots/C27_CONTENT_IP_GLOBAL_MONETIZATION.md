...
- job_id: `PROMPT-SNAPSHOT-C27_CONTENT_IP_GLOBAL_MONETIZATION`
- run_id: `PROMPT-SNAPSHOT-RUN-C27_CONTENT_IP_GLOBAL_MONETIZATION`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C27_CONTENT_IP_GLOBAL_MONETIZATION]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C27_CONTENT_IP_GLOBAL_MONETIZATION]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 콘텐츠/IP ownership·global launch·retention·라이선스/반복매출이 hit risk와 비용을 넘어 cash로 이어지는 메커니즘
- required_bridge_axes: software_retention, customer, margin
- source roles: issuer/플랫폼/배급사·권리/계약·사용자/매출 KPI·실적/비용/FCF·IP 법적자료·revision
- false-positive guard: 공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q01` — IP ownership·권리지역·수익배분·partner/platform 조건은 무엇인가
   ...
   - primitives: ip_monetization_visible
   ...
   - guard: 공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다.
2. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q02` — global launch·사용자/시청/판매·retention·repeat engagement가 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: global_launch_conversion, repeat_revenue, user_retention
...
5. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q05` — hit concentration·platform dependency·release delay·IP dispute·token/theme hype counter는 무엇인가
   ...
   - primitives: token_or_theme_hype_risk
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: earnings_visibility, information_confidence
