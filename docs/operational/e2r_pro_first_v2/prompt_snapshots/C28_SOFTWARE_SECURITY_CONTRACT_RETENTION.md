...
- job_id: `PROMPT-SNAPSHOT-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION`
- run_id: `PROMPT-SNAPSHOT-RUN-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 소프트웨어·보안 계약의 ARR/RPO·renewal·NRR/churn·margin leverage가 cash로 이어지는 메커니즘
- required_bridge_axes: software_retention, customer, margin
- source roles: issuer recurring KPI/계약/실적·고객 공식자료·RPO/renewal disclosures·breach/regulatory·FCF/SBC·revision
- false-positive guard: 소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q01` — ARR·recurring revenue·RPO/backlog 정의와 기간·conversion은 무엇인가
   ...
   - primitives: arr_growth_visible, rpo_to_sales
   ...
   - guard: 소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다.
2. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q02` — GRR/NRR·renewal·churn·logo/customer retention이 확인되는가
   - roles: ECONOMIC_BRIDGE
   - primitives: nrr, retention_or_renewal
...
4. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q04` — gross margin·sales efficiency·CAC/payback·operating leverage·FCF가 무엇인가
   ...
   - primitives: recurring_margin_leverage
...
7. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q07` — 경쟁·가격·breach·customer loss·SBC/dilution·revision·valuation counter는 무엇인가
   - roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, EXPECTATION_VALUATION, LIFECYCLE_SUPERSESSION
   - primitives: QUESTION_SPECIFIC_DIRECT_PREDICATE
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL, CURRENT_MARKET_DATA, LAWFUL_REVISION_DATA
   - affected components: earnings_visibility, information_confidence
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: 소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다.
