...
- job_id: `PROMPT-SNAPSHOT-C14_EV_DEMAND_SLOWDOWN_4B_4C`
- run_id: `PROMPT-SNAPSHOT-RUN-C14_EV_DEMAND_SLOWDOWN_4B_4C`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C14_EV_DEMAND_SLOWDOWN_4B_4C]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C14_EV_DEMAND_SLOWDOWN_4B_4C]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: EV 수요 둔화·재고·가격인하·고객 CAPEX 축소가 계약·가동률·현금의 thesis break로 이어지는지 판정하는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source roles: OEM/배터리 issuer·계약 정정/취소·재고/가동률·실적/impairment·정부/산업 수요자료
- false-positive guard: 산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q01` — EV 판매·배터리 출하·재고·가격·가동률 둔화가 issuer에 직접 귀속되는가
   ...
   - primitives: ev_demand_slowdown, inventory_spike, price_cut_risk, thesis_break_confirmed
