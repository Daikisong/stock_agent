...
- job_id: `PROMPT-SNAPSHOT-C15_MATERIAL_SPREAD_SUPERCYCLE`
- run_id: `PROMPT-SNAPSHOT-RUN-C15_MATERIAL_SPREAD_SUPERCYCLE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C15_MATERIAL_SPREAD_SUPERCYCLE]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C15_MATERIAL_SPREAD_SUPERCYCLE]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 원재료/제품 가격 변화가 pass-through·재고 lag·utilization을 거쳐 실현 margin·cash로 전환되는 supercycle 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source roles: issuer 제품가격/실적·공식 commodity/industry data·고객/수요·재고/가동률·FCF·revision
- false-positive guard: 원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q01` — commodity headline과 issuer 제품 ASP/pass-through를 분리했는가
   ...
   - guard: 원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다.
2. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q02` — feedstock·제품 가격·spread·재고평가·계약 가격 lag가 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: spread_expansion, pricing_power_confirmed
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
