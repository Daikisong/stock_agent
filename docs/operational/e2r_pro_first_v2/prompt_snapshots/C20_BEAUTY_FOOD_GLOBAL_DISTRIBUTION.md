...
- job_id: `PROMPT-SNAPSHOT-C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION`
- run_id: `PROMPT-SNAPSHOT-RUN-C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 뷰티/식품의 해외 유통 scale·고객 다변화·repeat order·고마진 mix가 cash로 전환되는 메커니즘
- required_bridge_axes: consumer_sell_through, margin, customer
- source roles: issuer 지역/SKU/채널 실적·유통 플랫폼/파트너·통관/규제·재고/FCF·revision
- false-positive guard: 해외 입점·viral headline을 반복 매출과 고마진 mix로 자동 확장하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION_Q01` — 국가·플랫폼·distributor·SKU별 수출/매출 growth가 무엇인가
   ...
   - primitives: export_growth_pct, platform_distribution_scale
   ...
   - guard: 해외 입점·viral headline을 반복 매출과 고마진 mix로 자동 확장하지 않는다.
2. `C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION_Q02` — 초도 입점과 반복 주문·retention·sell-through를 분리했는가
   - roles: ECONOMIC_BRIDGE
   - primitives: repeat_order_confirmed
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 해외 입점·viral headline을 반복 매출과 고마진 mix로 자동 확장하지 않는다.
3. `C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION_Q03` — 고객/지역/브랜드 concentration과 diversification이 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: brand_customer_diversification
...
4. `C20_BEAUTY_FOOD_GLOBAL_DISTRIBUTION_Q04` — 제품 mix·ASP·OEM/ODM·marketing·물류가 margin에 미치는 영향은 무엇인가
   ...
   - primitives: high_margin_mix_improvement
