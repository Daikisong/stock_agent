...
- job_id: `PROMPT-SNAPSHOT-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG`
- run_id: `PROMPT-SNAPSHOT-RUN-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 정부 간 방산 프레임워크가 구속력 있는 계약·납품·수금·마진으로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source roles: 대상 회사 공시/IR·발주국 정부/조달기관·수출승인 기관·계약 금융기관·실적/현금흐름·revision
- false-positive guard: 정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q01` — MOU·framework·LOI와 구속력 있는 본계약을 분리했는가
   ...
   - guard: 정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다.
2. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q02` — 정부 고객·예산·수출승인·금융지원·계약 당사자가 확인되는가
   - roles: ECONOMIC_BRIDGE
   - primitives: export_contract, government_customer
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
...
3. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q03` — 수량·단가·납품 일정·현지화·offset·유지보수 범위가 무엇인가
   ...
   - primitives: delivery_schedule, cost_overrun
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
