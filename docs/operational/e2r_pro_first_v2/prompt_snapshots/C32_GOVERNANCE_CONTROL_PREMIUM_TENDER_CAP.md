...
- job_id: `PROMPT-SNAPSHOT-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP`
- run_id: `PROMPT-SNAPSHOT-RUN-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 지배권 거래·tender offer의 확정 조건·minority cash path·승인·event spread를 판정하는 메커니즘
- required_bridge_axes: capital_return, guard_risk
- source roles: tender/공개매수/주요사항 공시·거래 당사자·규제기관·법원·financing·시장 spread
- false-positive guard: 경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q01` — binding tender/매매계약·가격·수량·대상 주주·조건이 무엇인가
   ...
   - primitives: tender_offer_confirmed
   ...
   - guard: 경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다.
...
4. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q04` — control premium floor·잔여지분·상장유지/폐지·희석 영향을 무엇인가
   - roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION, FINANCIAL_CASH_CONVERSION
   - primitives: control_premium_floor
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: 경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다.
5. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q05` — withdrawal·조건 미충족·가격조정·소송·event spread risk는 무엇인가
   - roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, EXPECTATION_VALUATION
   - primitives: event_spread_risk
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
