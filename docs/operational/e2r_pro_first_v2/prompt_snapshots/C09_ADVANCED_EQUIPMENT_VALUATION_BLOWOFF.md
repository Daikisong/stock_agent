...
- job_id: `PROMPT-SNAPSHOT-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF`
- run_id: `PROMPT-SNAPSHOT-RUN-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 첨단장비의 주문·매출 bridge와 현재 valuation/price blowoff를 대조하는 과열·4B 메커니즘
- required_bridge_axes: guard_risk, valuation_repricing, customer
- source roles: issuer order/실적·고객 CAPEX·revision/consensus·시장 valuation·공시 자금조달·경쟁사 자료
- false-positive guard: 주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q01` — 실제 고객 주문·backlog·납품·매출 bridge가 있는가
   ...
   - guard: 주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다.
2. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q02` — 고객 quality·capacity cycle·제품 경쟁력이 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: customer_quality_visible
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
...
5. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q05` — 고객 CAPEX·경쟁 증설·order pushout·inventory·share issuance counter는 무엇인가
   ...
   - primitives: order_to_revenue_bridge, capex_cycle_risk
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
