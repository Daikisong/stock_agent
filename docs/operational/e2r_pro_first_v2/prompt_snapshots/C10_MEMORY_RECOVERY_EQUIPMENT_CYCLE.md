...
- job_id: `PROMPT-SNAPSHOT-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE`
- run_id: `PROMPT-SNAPSHOT-RUN-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 메모리 가격·재고·공급규율 회복이 장비 주문과 issuer 매출·FCF로 전환되는 cycle 메커니즘
- required_bridge_axes: customer, backlog, margin, valuation_repricing
- source roles: 메모리 issuer/고객·장비 issuer 주문/IR·산업 가격/재고 공식자료·실적/FCF·revision
- false-positive guard: 메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q01` — DRAM/NAND 가격·재고·utilization·공급 discipline의 현재 상태는 무엇인가
   ...
   - primitives: memory_price_increase_mentioned, supply_discipline_mentioned
...
3. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q03` — advanced packaging/공정 전환 병목이 대상 장비 주문으로 직접 연결되는가
   - roles: ECONOMIC_BRIDGE
   - primitives: advanced_packaging_bottleneck
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다.
4. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q04` — order-to-revenue·매출·margin·FCF conversion이 확인되는가
   ...
   - primitives: cycle_to_revenue_bridge, order_to_revenue_bridge, fcf_quality_score
...
5. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q05` — 재고 조정·공급 증설·가격 반락·고객 CAPEX 지연 counter는 무엇인가
   ...
   - primitives: supply_demand_tightness
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: 메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다.
6. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q06` — 회복 초기/중기/후기 phase와 revision breadth는 무엇인가
   - roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION, EXPECTATION_VALUATION
...
7. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q07` — normalized valuation과 cycle midpoint cash flow는 무엇인가
   - roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION, EXPECTATION_VALUATION
   - primitives: cycle_demand_visibility, inventory_cycle_repair
