...
- job_id: `PROMPT-SNAPSHOT-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT`
- run_id: `PROMPT-SNAPSHOT-RUN-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 의료기기 승인·reimbursement·installed base·procedure·consumable repeat가 수출·마진·FCF로 이어지는 메커니즘
- required_bridge_axes: bio_commercialization, guard_risk
- source roles: 규제/보험기관·병원/고객·issuer installed-base/procedure KPI·distributor·실적/margin/FCF
- false-positive guard: 허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q01` — 국가별 규제 승인·reimbursement code·coverage·가격이 무엇인가
   ...
   - primitives: reimbursement_confirmed
   ...
   - guard: 허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다.
2. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q02` — installed base·procedure volume·utilization·consumable attach가 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: procedure_volume_growth
...
4. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q04` — device ASP·consumable mix·gross margin·service revenue가 무엇인가
   ...
   - primitives: consumable_repeat_revenue, gross_margin_bridge
...
5. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q05` — 제조 capacity·quality/safety recall·규제 유지·교육/채택 risk는 무엇인가
   ...
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, TRIAL_REGISTRY, ISSUER_FILING
