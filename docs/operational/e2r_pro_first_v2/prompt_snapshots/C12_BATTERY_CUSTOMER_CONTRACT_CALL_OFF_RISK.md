...
- job_id: `PROMPT-SNAPSHOT-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK`
- run_id: `PROMPT-SNAPSHOT-RUN-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 배터리 고객 계약의 실제 call-off·volume·고객 CAPEX 변화와 취소/지연 위험을 판정하는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source roles: issuer/고객 계약·OEM CAPEX/판매·JV 자료·실적/가동률·공시 정정/취소·현금흐름
- false-positive guard: 고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q01` — 현재 유효한 고객 계약과 call-off 권리·최소물량·가격·기간이 무엇인가
   ...
   - guard: 고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다.
2. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q02` — 고객 EV 판매·공장 CAPEX·platform 일정과 issuer volume visibility가 일치하는가
   - roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - primitives: customer_capex_decline, contract_cancelled_or_delayed, volume_visibility
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: 고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다.
