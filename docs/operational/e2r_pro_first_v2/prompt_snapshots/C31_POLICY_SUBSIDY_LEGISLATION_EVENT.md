...
- job_id: `PROMPT-SNAPSHOT-C31_POLICY_SUBSIDY_LEGISLATION_EVENT`
- run_id: `PROMPT-SNAPSHOT-RUN-C31_POLICY_SUBSIDY_LEGISLATION_EVENT`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C31_POLICY_SUBSIDY_LEGISLATION_EVENT]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C31_POLICY_SUBSIDY_LEGISLATION_EVENT]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 정책·법률·보조금이 시행규칙·eligibility·예산을 통과해 대상 회사 현금/수요로 이어지는 메커니즘
- required_bridge_axes: guard_risk, valuation_repricing, contract
- source roles: 법률/정부/규제기관·예산/가이드라인·issuer eligibility/수취·고객/산업·실적/FCF
- false-positive guard: 정책 발표·테마를 대상 회사의 확정 보조금/매출로 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C31_POLICY_SUBSIDY_LEGISLATION_EVENT_Q01` — 법안 제안·통과·공포·시행규칙·예산 배정을 분리했는가
   ...
   - primitives: policy_or_regulatory_confirmed, subsidy_capture_visible
   ...
   - guard: 정책 발표·테마를 대상 회사의 확정 보조금/매출로 만들지 않는다.
...
4. `C31_POLICY_SUBSIDY_LEGISLATION_EVENT_Q04` — implementation timeline·기관·신청/승인 상태는 무엇인가
   ...
   - primitives: implementation_timeline, policy_headline_only
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
...
7. `C31_POLICY_SUBSIDY_LEGISLATION_EVENT_Q07` — revision·valuation이 정책 기대를 선반영했는가
   - roles: ECONOMIC_BRIDGE, EXPECTATION_VALUATION
   - primitives: QUESTION_SPECIFIC_DIRECT_PREDICATE
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, CURRENT_MARKET_DATA, LAWFUL_REVISION_DATA
   - affected components: market_mispricing, valuation_rerating, information_confidence
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 정책 발표·테마를 대상 회사의 확정 보조금/매출로 만들지 않는다.
