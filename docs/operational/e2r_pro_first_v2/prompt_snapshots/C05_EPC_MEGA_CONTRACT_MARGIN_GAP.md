...
- job_id: `PROMPT-SNAPSHOT-C05_EPC_MEGA_CONTRACT_MARGIN_GAP`
- run_id: `PROMPT-SNAPSHOT-RUN-C05_EPC_MEGA_CONTRACT_MARGIN_GAP`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C05_EPC_MEGA_CONTRACT_MARGIN_GAP]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C05_EPC_MEGA_CONTRACT_MARGIN_GAP]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 대형 EPC 계약의 규모와 기간이 공정률·원가·지체상금·운전자본을 거쳐 이익과 현금으로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source roles: 단일판매공급계약/사업보고서·발주처·프로젝트 update·실적 note·현금흐름·분쟁/법원·revision
- false-positive guard: 계약금액만으로 이익·현금·Stage를 올리지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q01` — 계약금액·기간·대상·고객·기존 매출 대비 규모와 회사 귀속분은 무엇인가
   ...
   - guard: 계약금액만으로 이익·현금·Stage를 올리지 않는다.
2. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q02` — 설계·조달·시공 범위와 공정률·매출 인식·대금 지급 milestone은 무엇인가
   ...
   - primitives: contract_amount_to_prior_sales
...
4. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q04` — 프로젝트 mix가 gross margin·OPM·FCF에 미치는 영향이 실제로 확인되는가
   ...
   - primitives: margin_bridge_visible
   - source roles: ISSUER_EARNINGS, OFFICIAL_FILING
   - affected components: bottleneck_pricing, eps_fcf_explosion, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 계약금액만으로 이익·현금·Stage를 올리지 않는다.
...
7. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q07` — revision과 valuation은 headline contract와 margin gap을 구분하는가
   ...
   - primitives: contract_duration_months
