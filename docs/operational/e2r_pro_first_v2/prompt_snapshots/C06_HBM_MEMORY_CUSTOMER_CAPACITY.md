...
- job_id: `PROMPT-SNAPSHOT-C06_HBM_MEMORY_CUSTOMER_CAPACITY`
- run_id: `PROMPT-SNAPSHOT-RUN-C06_HBM_MEMORY_CUSTOMER_CAPACITY`
...
- role: `PRIMARY`
- mechanism: HBM/메모리의 고객 qualification·allocation·qualified capacity·ASP/mix가 이익과 FCF로 전환되는 메커니즘
- required_bridge_axes: customer, backlog, contract, margin
- source roles: issuer earnings/filing/IR·고객/플랫폼 공식자료·경쟁사 공식자료·장비/패키징 파트너·revision·시장데이터
- false-positive guard: HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q01` — 고객 allocation·booking·commitment·LTA가 구속력 있는 물량/기간/가격으로 확인되는가
   ...
   - primitives: customer_preorder_or_allocation, memory_price_increase_mentioned
   ...
   - guard: HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다.
2. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q02` — capacity sold-out·pre-sold·수요 초과가 issuer 제품과 기간에 직접 귀속되는가
   - roles: ECONOMIC_BRIDGE
   - primitives: hbm_capacity_constraint, hbm_capacity_pre_sold
...
4. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q04` — HBM 세대별 양산·상업 출하·shipment mix·revenue mix가 확인되는가
   - roles: ECONOMIC_BRIDGE
   - primitives: revenue_visibility_contract
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
...
7. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q07` — 중기 revision·consensus·normalized valuation·expectation gap은 무엇인가
   ...
   - primitives: medium_term_revision_visibility
