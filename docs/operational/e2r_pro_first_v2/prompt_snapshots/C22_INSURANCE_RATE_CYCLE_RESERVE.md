...
- job_id: `PROMPT-SNAPSHOT-C22_INSURANCE_RATE_CYCLE_RESERVE`
- run_id: `PROMPT-SNAPSHOT-RUN-C22_INSURANCE_RATE_CYCLE_RESERVE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C22_INSURANCE_RATE_CYCLE_RESERVE]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C22_INSURANCE_RATE_CYCLE_RESERVE]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: CSM·신계약가치·loss ratio·reserve quality·K-ICS가 자본환원과 valuation으로 이어지는 보험 메커니즘
- required_bridge_axes: insurance_quality, capital_return, guard_risk
- source roles: 보험사 실적/공시·K-ICS/감독자료·계리/CSM note·자본환원·시장/peer valuation·revision
- false-positive guard: CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q01` — CSM growth·신계약 CSM·상각·NBM의 질과 일회성 조정은 무엇인가
   ...
   - primitives: csm_growth_visible, loss_ratio_quality
   ...
   - guard: CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다.
2. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q02` — 손해율·사업비율·reserve adequacy·가정변경·재보험 상태는 무엇인가
   ...
   - primitives: reserve_quality_visible
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
...
4. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q04` — 배당·자사주·capital return 실행과 규제 여력은 무엇인가
   ...
   - primitives: capital_return_execution
