...
- job_id: `PROMPT-SNAPSHOT-C11_BATTERY_ORDERBOOK_RERATING`
- run_id: `PROMPT-SNAPSHOT-RUN-C11_BATTERY_ORDERBOOK_RERATING`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C11_BATTERY_ORDERBOOK_RERATING]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C11_BATTERY_ORDERBOOK_RERATING]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 배터리 orderbook이 고객 call-off·ramp·ex-credit margin·FCF로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source roles: issuer 계약/IR·OEM/JV 고객 공식자료·공장 ramp·실적/AMPC note·FCF·revision
- false-positive guard: 수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C11_BATTERY_ORDERBOOK_RERATING_Q01` — orderbook/수주잔고의 고객·제품·기간·구속력·취소/콜오프 조건은 무엇인가
   ...
   - guard: 수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다.
...
3. `C11_BATTERY_ORDERBOOK_RERATING_Q03` — ASP·원재료 pass-through·AMPC 제외 margin·mix가 무엇인가
   ...
   - primitives: ex_credit_margin
   - source roles: ISSUER_EARNINGS, OFFICIAL_FILING
   - affected components: earnings_visibility, eps_fcf_explosion, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다.
...
5. `C11_BATTERY_ORDERBOOK_RERATING_Q05` — 고객 capex 축소·call-off·계약 지연/취소·재고·price cut counter는 무엇인가
   - roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - primitives: customer_contract, call_off_risk
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: 수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다.
