...
- job_id: `PROMPT-SNAPSHOT-C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK`
- run_id: `PROMPT-SNAPSHOT-RUN-C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: PF 보증·미분양·자금조달 비용이 분양/회수·자산매각·부채축소를 거쳐 balance-sheet repair 또는 break로 이어지는 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source roles: 사업보고서/PF note·신용평가·대주단/법원·분양/사업장 자료·유동성/현금흐름·정정공시
- false-positive guard: PF 언급이나 건설업 일반 위험을 대상 회사의 현재 default로 자동 확대하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK_Q01` — PF exposure·우발채무·보증·maturity·상환순위·사업장별 위험은 무엇인가
   ...
   - primitives: pf_exposure_reduced, occupancy_or_presale_visible
   ...
   - guard: PF 언급이나 건설업 일반 위험을 대상 회사의 현재 default로 자동 확대하지 않는다.
2. `C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK_Q02` — 분양률·입주율·미분양·공정률·cash collection이 무엇인가
   ...
   - primitives: cash_collection_visible
...
4. `C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK_Q04` — 자산매각·유상증자·채무조정·보증 해소가 실제 repair인가
   ...
   - primitives: balance_sheet_repair
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
