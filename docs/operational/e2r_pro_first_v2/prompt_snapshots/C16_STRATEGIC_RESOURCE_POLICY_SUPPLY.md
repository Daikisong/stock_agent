...
- job_id: `PROMPT-SNAPSHOT-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY`
- run_id: `PROMPT-SNAPSHOT-RUN-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 전략자원 shortage·정책지원·offtake·permit가 실제 생산·판매·현금으로 이어지는 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source roles: issuer filing/technical report·정부 permit/정책·offtake 고객·프로젝트 금융·생산/원가·시장 supply data
- false-positive guard: 자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q01` — offtake 계약의 고객·최소물량·가격·기간·조건은 무엇인가
   ...
   - guard: 자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다.
...
3. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q03` — 정책/보조금·정부 승인·인프라·환경/지역사회 조건은 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: policy_supply_support
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
