...
- job_id: `PROMPT-SNAPSHOT-C21_FINANCIAL_ROE_PBR_CAPITAL_RETURN`
- run_id: `PROMPT-SNAPSHOT-RUN-C21_FINANCIAL_ROE_PBR_CAPITAL_RETURN`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C21_FINANCIAL_ROE_PBR_CAPITAL_RETURN]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C21_FINANCIAL_ROE_PBR_CAPITAL_RETURN]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 금융사의 normalized ROE·자본적정성·신용비용과 실제 자본환원이 PBR rerating으로 이어지는 메커니즘
- required_bridge_axes: capital_return, valuation_repricing, guard_risk
- source roles: 금융감독/issuer 실적·자본/자산건전성 공시·이사회 자본환원·시장/peer valuation·revision
- false-positive guard: 낮은 PBR이나 선언형 밸류업만으로 자본환원 실행·ROE quality를 가정하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
7. `C21_FINANCIAL_ROE_PBR_CAPITAL_RETURN_Q07` — revision과 shareholder return이 rerating을 지지하는가
   ...
   - primitives: treasury_share_cancellation, capital_return_execution
   - source roles: CURRENT_MARKET_DATA, LAWFUL_REVISION_DATA
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 낮은 PBR이나 선언형 밸류업만으로 자본환원 실행·ROE quality를 가정하지 않는다.
