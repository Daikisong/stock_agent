...
- job_id: `PROMPT-SNAPSHOT-C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY`
- run_id: `PROMPT-SNAPSHOT-RUN-C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 정책·규제·프로젝트 award가 허가·소송·공정·원가를 통과해 대상 회사 현금으로 이어지는 메커니즘
- required_bridge_axes: guard_risk, valuation_repricing, contract
- source roles: 법령/규제기관·법원/허가기관·발주처·issuer 공시/IR·consortium partner·프로젝트 금융·실적/FCF
- false-positive guard: 정책 수혜·원전 테마·협력 발표를 대상 회사의 award/현금으로 자동 연결하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY_Q01` — 정책 발표·법안·규제 승인·실제 프로젝트 award를 분리했는가
   ...
   - primitives: policy_or_regulatory_confirmed, project_award_confirmed
   ...
   - guard: 정책 수혜·원전 테마·협력 발표를 대상 회사의 award/현금으로 자동 연결하지 않는다.
2. `C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY_Q02` — 대상 회사의 consortium 지분·공급 범위·계약 당사자·직접 cash route는 무엇인가
   ...
   - primitives: direct_company_cash_route
...
3. `C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY_Q03` — permit·환경심사·법원·행정소송·지역 승인 상태와 다음 milestone은 무엇인가
   ...
   - primitives: permit_or_legal_delay
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
...
4. `C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY_Q04` — 공정·납품·수금 일정과 cost overrun·지연·보증 부담은 무엇인가
   - roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, LIFECYCLE_SUPERSESSION, FINANCIAL_CASH_CONVERSION
   - primitives: cost_overrun
...
7. `C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY_Q07` — revision·valuation이 정책 headline과 실제 cash route를 구분하는가
   ...
   - primitives: QUESTION_SPECIFIC_DIRECT_PREDICATE
   - source roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, ISSUER_EARNINGS, OFFICIAL_FILING, CURRENT_MARKET_DATA, LAWFUL_REVISION_DATA
   - affected components: eps_fcf_explosion, capital_allocation, information_confidence
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 정책 수혜·원전 테마·협력 발표를 대상 회사의 award/현금으로 자동 연결하지 않는다.
