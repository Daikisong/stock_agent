...
- job_id: `PROMPT-SNAPSHOT-C08_SEMI_TEST_SOCKET_CUSTOMER_QUALITY`
- run_id: `PROMPT-SNAPSHOT-RUN-C08_SEMI_TEST_SOCKET_CUSTOMER_QUALITY`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C08_SEMI_TEST_SOCKET_CUSTOMER_QUALITY]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C08_SEMI_TEST_SOCKET_CUSTOMER_QUALITY]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 테스트소켓/프로브 제품 노출이 고객 qualification·직접 주문·반복 출하·매출/마진으로 전환되는 메커니즘
- required_bridge_axes: customer, backlog, contract, margin
- source roles: issuer IR/공시·고객/파트너 공식자료·공급계약·실적/제품별 매출·margin·revision
- false-positive guard: socket/HBM 제품 소개나 품질상만으로 Actionable/Green을 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C08_SEMI_TEST_SOCKET_CUSTOMER_QUALITY_Q01` — 제품 roadmap/profile과 실제 named customer qualification을 분리했는가
   ...
   - primitives: named_customer_quality, qualification_confirmed
   ...
   - guard: socket/HBM 제품 소개나 품질상만으로 Actionable/Green을 만들지 않는다.
...
4. `C08_SEMI_TEST_SOCKET_CUSTOMER_QUALITY_Q04` — socket/probe 수요가 issuer 매출·ASP·gross margin·OPM으로 전환됐는가
   ...
   - primitives: socket_or_test_demand_visible, margin_bridge_visible
