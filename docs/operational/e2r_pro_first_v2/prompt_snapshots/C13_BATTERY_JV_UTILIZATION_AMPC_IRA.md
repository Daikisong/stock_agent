...
- job_id: `PROMPT-SNAPSHOT-C13_BATTERY_JV_UTILIZATION_AMPC_IRA`
- run_id: `PROMPT-SNAPSHOT-RUN-C13_BATTERY_JV_UTILIZATION_AMPC_IRA`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C13_BATTERY_JV_UTILIZATION_AMPC_IRA]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C13_BATTERY_JV_UTILIZATION_AMPC_IRA]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: JV 가동률·AMPC/IRA 보조금이 ex-credit economics와 현금으로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source roles: JV/issuer/customer 공식자료·IRA/정부 규정·실적 note·현금흐름·CAPEX·revision
- false-positive guard: gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q01` — JV 소유구조·생산능력·실제 utilization·yield·고객 계약은 무엇인가
   ...
   - primitives: jv_utilization, customer_contract
   ...
   - guard: gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다.
2. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q02` — AMPC/보조금 인식액·현금 수취·회계 분류와 ex-credit margin은 무엇인가
   ...
   - primitives: ampc_or_subsidy_capture, ex_credit_margin
   - source roles: AUDITOR_FILING, REGULATOR_OFFICIAL, ISSUER_EARNINGS, OFFICIAL_FILING
...
5. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q05` — 정책 eligibility·domestic content·법률/정권 reversal risk는 무엇인가
   ...
   - primitives: policy_reversal_risk
