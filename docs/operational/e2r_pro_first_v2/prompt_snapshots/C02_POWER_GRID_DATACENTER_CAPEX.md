...
- job_id: `PROMPT-SNAPSHOT-C02_POWER_GRID_DATACENTER_CAPEX`
- run_id: `PROMPT-SNAPSHOT-RUN-C02_POWER_GRID_DATACENTER_CAPEX`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C02_POWER_GRID_DATACENTER_CAPEX]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C02_POWER_GRID_DATACENTER_CAPEX]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 전력망·데이터센터 CAPEX가 고객 주문, 긴 lead time, 공급제약, 가격결정력과 매출로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source roles: 공식 수주/IR·유틸리티/클라우드 고객 공식자료·공장/lead-time 공시·실적·산업 설비 공급자료·revision
- false-positive guard: 데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C02_POWER_GRID_DATACENTER_CAPEX_Q01` — 데이터센터·전력망·유틸리티 고객이 대상 회사의 직접 고객으로 확인되는가
   ...
   - primitives: datacenter_customer
   ...
   - guard: 데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다.
2. `C02_POWER_GRID_DATACENTER_CAPEX_Q02` — backlog·book-to-bill·lead time·납품 일정과 현재 증설 전 saleable capacity는 무엇인가
   ...
   - primitives: order_backlog_to_sales, lead_time_extended, capacity_constraint
...
4. `C02_POWER_GRID_DATACENTER_CAPEX_Q04` — 실현 ASP·가격 인상·제품 mix·마진 전환이 확인되는가
   ...
   - primitives: pricing_power_confirmed
