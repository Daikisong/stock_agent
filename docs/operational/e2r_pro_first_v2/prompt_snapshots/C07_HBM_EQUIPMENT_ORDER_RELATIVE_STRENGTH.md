...
- job_id: `PROMPT-SNAPSHOT-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH`
- run_id: `PROMPT-SNAPSHOT-RUN-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: HBM/첨단패키징 장비의 직접 고객 주문과 backlog가 납품·매출·마진으로 전환되는 메커니즘
- required_bridge_axes: customer, backlog, contract, margin
- source roles: 공시 수주·issuer IR/실적·고객 CAPEX/공장 자료·장비 qualification·backlog·revision
- false-positive guard: 고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q01` — named HBM/packaging customer·qualification·직접 장비 주문이 확인되는가
   ...
   - primitives: hbm_customer_order, customer_contract_visible
...
3. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q03` — 고객 CAPEX와 대상 회사 order-to-revenue timing이 일치하는가
   - roles: ECONOMIC_BRIDGE
   - primitives: order_to_revenue_bridge
   - source roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
...
7. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q07` — relative strength나 가격은 비점수 보조 신호로만 분리됐는가
   - roles: ECONOMIC_BRIDGE
   - primitives: relative_strength_score
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다.
