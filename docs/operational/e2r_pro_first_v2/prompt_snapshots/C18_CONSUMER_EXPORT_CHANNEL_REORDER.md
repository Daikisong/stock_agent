...
- job_id: `PROMPT-SNAPSHOT-C18_CONSUMER_EXPORT_CHANNEL_REORDER`
- run_id: `PROMPT-SNAPSHOT-RUN-C18_CONSUMER_EXPORT_CHANNEL_REORDER`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C18_CONSUMER_EXPORT_CHANNEL_REORDER]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C18_CONSUMER_EXPORT_CHANNEL_REORDER]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 수출·채널 sell-through가 distributor inventory와 reorder를 통과해 반복 매출·마진으로 이어지는 메커니즘
- required_bridge_axes: consumer_sell_through, margin, customer
- source roles: issuer 지역/채널 실적·세관/수출자료·유통사/플랫폼 공식자료·재고/반품·FCF·revision
- false-positive guard: 수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q01` — 지역·채널·제품별 export growth와 sell-in/sell-through를 분리했는가
   ...
   - primitives: export_growth_pct, sell_through_confirmed
   ...
   - guard: 수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다.
2. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q02` — distributor/retailer inventory·반품·재고일수·reorder가 확인되는가
   - roles: ECONOMIC_BRIDGE
   - primitives: channel_reorder_confirmed
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected components: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - adequate search: official-first, routes>=1, no-new-route confirmations=2
   - guard: 수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다.
3. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q03` — repeat order·channel expansion·customer concentration이 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: repeat_order_confirmed, opm_expansion_pctp
