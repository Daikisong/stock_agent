...
- job_id: `PROMPT-SNAPSHOT-C24_BIO_TRIAL_DATA_EVENT_RISK`
- run_id: `PROMPT-SNAPSHOT-RUN-C24_BIO_TRIAL_DATA_EVENT_RISK`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C24_BIO_TRIAL_DATA_EVENT_RISK]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C24_BIO_TRIAL_DATA_EVENT_RISK]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 임상 데이터의 endpoint quality·안전성·규제경로·partner·runway가 binary event 위험을 넘어 상업화 옵션으로 이어지는 메커니즘
- required_bridge_axes: bio_commercialization, guard_risk
- source roles: clinical trial registry·학회/논문·규제기관·issuer filing/IR·partner 공식자료·funding/cash·safety update
- false-positive guard: headline response·poster·price spike를 endpoint quality·approval·commercialization으로 확대하지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C24_BIO_TRIAL_DATA_EVENT_RISK_Q01` — trial design·population·endpoint·control·통계계획·sample size가 무엇인가
   ...
   - primitives: trial_quality_visible
   - source roles: TRIAL_REGISTRY, REGULATOR_OFFICIAL, ISSUER_FILING
   ...
   - guard: headline response·poster·price spike를 endpoint quality·approval·commercialization으로 확대하지 않는다.
...
4. `C24_BIO_TRIAL_DATA_EVENT_RISK_Q04` — 규제기관 피드백·다음 단계·approval path·일정이 무엇인가
   ...
   - primitives: approval_not_confirmed
...
6. `C24_BIO_TRIAL_DATA_EVENT_RISK_Q06` — cash runway·추가 임상비·희석·funding risk는 무엇인가
   ...
   - primitives: cash_runway_risk
   - source roles: TRIAL_REGISTRY, REGULATOR_OFFICIAL, ISSUER_FILING, ISSUER_EARNINGS, OFFICIAL_FILING
   - affected components: earnings_visibility, information_confidence
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: headline response·poster·price spike를 endpoint quality·approval·commercialization으로 확대하지 않는다.
7. `C24_BIO_TRIAL_DATA_EVENT_RISK_Q07` — binary event unresolved·실패·CRL·endpoint miss가 현재 OPEN/RESOLVED인지 무엇인가
   ...
   - primitives: binary_event_unresolved
   - source roles: TRIAL_REGISTRY, REGULATOR_OFFICIAL, ISSUER_FILING
   - affected components: earnings_visibility, information_confidence
   - adequate search: official-first, routes>=2, no-new-route confirmations=2
   - guard: headline response·poster·price spike를 endpoint quality·approval·commercialization으로 확대하지 않는다.
