...
- job_id: `PROMPT-SNAPSHOT-C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE`
- run_id: `PROMPT-SNAPSHOT-RUN-C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-SNAPSHOT-RUN-C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE]]`와 `[[E2R_PRO_JOB_ID:PROMPT-SNAPSHOT-C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE]]` marker를 각각 정확히 한 번 출력한다.
...
- role: `PRIMARY`
- mechanism: 사용자 engagement·ARPU·광고수요·take rate가 비용 구조를 넘어 operating leverage와 cash로 이어지는 플랫폼 메커니즘
- required_bridge_axes: software_retention, customer, margin
- source roles: issuer KPI/실적·광고주/플랫폼 공식자료·규제기관·트래픽 보조자료·FCF/SBC·revision
- false-positive guard: 다운로드·MAU·AI/광고 테마만으로 ARPU·take rate·margin을 만들지 않는다.
- score_authority: `false`
- stage_authority: `false`
...
1. `C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE_Q01` — MAU/DAU·time spent·retention·traffic source의 질은 무엇인가
   ...
   - guard: 다운로드·MAU·AI/광고 테마만으로 ARPU·take rate·margin을 만들지 않는다.
2. `C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE_Q02` — ARPU·ad load·CPM/CPC·광고주 수요·take rate가 무엇인가
   - roles: ECONOMIC_BRIDGE
   - primitives: arpu_growth_pct, ad_revenue_growth_pct, take_rate_improvement
   - source roles: ISSUER_OFFICIAL, OFFICIAL_FILING
...
3. `C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE_Q03` — 광고/거래 매출 성장과 segment margin·operating leverage가 확인되는가
   ...
   - primitives: operating_leverage_visible
...
6. `C26_PLATFORM_AD_REVENUE_OPERATING_LEVERAGE_Q06` — FCF·SBC·CAPEX·revision·valuation은 무엇인가
   - roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION, EXPECTATION_VALUATION, LIFECYCLE_SUPERSESSION
   - primitives: QUESTION_SPECIFIC_DIRECT_PREDICATE
