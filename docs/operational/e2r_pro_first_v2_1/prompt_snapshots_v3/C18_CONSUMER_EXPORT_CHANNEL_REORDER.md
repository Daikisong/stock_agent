...
- job_id: `PROMPT-V3-SNAPSHOT-C18_CONSUMER_EXPORT_CHANNEL_REORDER`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C18_CONSUMER_EXPORT_CHANNEL_REORDER`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C18_CONSUMER_EXPORT_CHANNEL_REORDER]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C18_CONSUMER_EXPORT_CHANNEL_REORDER]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 수출·채널 sell-through가 distributor inventory와 reorder를 통과해 반복 매출·마진으로 이어지는 메커니즘
- required_bridge_axes: consumer_sell_through, margin, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer 지역/채널 실적·세관/수출자료·유통사/플랫폼 공식자료·재고/반품·FCF·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q01` — 지역·채널·제품별 export growth와 sell-in/sell-through를 분리했는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: export_growth_pct, sell_through_confirmed
   ...
   - false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
2. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q02` — distributor/retailer inventory·반품·재고일수·reorder가 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: channel_reorder_confirmed
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
3. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q03` — repeat order·channel expansion·customer concentration이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: repeat_order_confirmed, opm_expansion_pctp
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
4. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q04` — ASP·mix·promotion·물류·FX가 gross margin/OPM에 미치는 영향은 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
5. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q05` — 매출채권·재고·marketing spend·FCF 전환은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
6. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q06` — 일회성 stocking·채널 밀어내기·반품·규제/관세 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["수출액·입점·초도물량만으로 소비자 sell-through와 repeat order를 확정하지 않는다."]
7. `C18_CONSUMER_EXPORT_CHANNEL_REORDER_Q07` — revision·valuation이 실제 reorder를 반영했는가
   - mandatory_for_full_thesis: `true`
