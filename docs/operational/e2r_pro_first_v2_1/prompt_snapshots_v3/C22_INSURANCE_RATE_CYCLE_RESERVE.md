...
- job_id: `PROMPT-V3-SNAPSHOT-C22_INSURANCE_RATE_CYCLE_RESERVE`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C22_INSURANCE_RATE_CYCLE_RESERVE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C22_INSURANCE_RATE_CYCLE_RESERVE]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C22_INSURANCE_RATE_CYCLE_RESERVE]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: CSM·신계약가치·loss ratio·reserve quality·K-ICS가 자본환원과 valuation으로 이어지는 보험 메커니즘
- required_bridge_axes: insurance_quality, capital_return, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"보험사 실적/공시·K-ICS/감독자료·계리/CSM note·자본환원·시장/peer valuation·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q01` — CSM growth·신계약 CSM·상각·NBM의 질과 일회성 조정은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: csm_growth_visible, loss_ratio_quality
   ...
   - false_positive_guards: ["CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다."]
2. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q02` — 손해율·사업비율·reserve adequacy·가정변경·재보험 상태는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: reserve_quality_visible
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   ...
   - false_positive_guards: ["CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다."]
3. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q03` — K-ICS·가용자본·금리/투자손익 sensitivity는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: k_ics_ratio
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다."]
4. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q04` — 배당·자사주·capital return 실행과 규제 여력은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: capital_return_execution
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다."]
5. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q05` — 회계/계리 변경·reserve 강화·대형 손해·금리 reversal counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["CSM 총액이나 금리 방향만으로 earnings quality와 배당여력을 확정하지 않는다."]
6. `C22_INSURANCE_RATE_CYCLE_RESERVE_Q06` — normalized earnings·PBR/EV·revision은 무엇인가
   - mandatory_for_full_thesis: `true`
