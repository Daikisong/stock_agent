...
- job_id: `PROMPT-V3-SNAPSHOT-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: HBM/첨단패키징 장비의 직접 고객 주문과 backlog가 납품·매출·마진으로 전환되는 메커니즘
- required_bridge_axes: customer, backlog, contract, margin
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"공시 수주·issuer IR/실적·고객 CAPEX/공장 자료·장비 qualification·backlog·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q01` — named HBM/packaging customer·qualification·직접 장비 주문이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: hbm_customer_order, customer_contract_visible
   ...
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
2. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q02` — equipment backlog·수주잔고·lead time·납기와 반복 주문이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: equipment_order_backlog
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
3. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q03` — 고객 CAPEX와 대상 회사 order-to-revenue timing이 일치하는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: order_to_revenue_bridge
   - required_source_roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - preferred_source_families: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
4. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q04` — advanced packaging bottleneck이 대상 장비의 실제 utilization·ASP·마진으로 이어지는가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
5. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q05` — 고객 집중·단일 장비 cycle·중고장비/경쟁 장비·수율/qualification counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
6. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q06` — CAPEX cycle·재고·매출채권·선수금·FCF 전환은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
7. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q07` — relative strength나 가격은 비점수 보조 신호로만 분리됐는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: relative_strength_score
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객의 HBM 투자나 업계 장비 수요를 대상 회사 주문으로 자동 귀속하지 않는다."]
8. `C07_HBM_EQUIPMENT_ORDER_RELATIVE_STRENGTH_Q08` — revision·valuation이 주문 bridge를 선반영했는가
   - mandatory_for_full_thesis: `true`
