...
- job_id: `PROMPT-V3-SNAPSHOT-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 의료기기 승인·reimbursement·installed base·procedure·consumable repeat가 수출·마진·FCF로 이어지는 메커니즘
- required_bridge_axes: bio_commercialization, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"규제/보험기관·병원/고객·issuer installed-base/procedure KPI·distributor·실적/margin/FCF","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q01` — 국가별 규제 승인·reimbursement code·coverage·가격이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: reimbursement_confirmed
   ...
   - false_positive_guards: ["허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다."]
2. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q02` — installed base·procedure volume·utilization·consumable attach가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: procedure_volume_growth
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다."]
3. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q03` — 수출·distributor·hospital/customer concentration과 reorder가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: export_growth_pct
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다."]
4. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q04` — device ASP·consumable mix·gross margin·service revenue가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: consumable_repeat_revenue, gross_margin_bridge
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다."]
5. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q05` — 제조 capacity·quality/safety recall·규제 유지·교육/채택 risk는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, LIFECYCLE_SUPERSESSION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, TRIAL_REGISTRY, ISSUER_FILING
   - preferred_source_families: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, TRIAL_REGISTRY, ISSUER_FILING
   ...
   - false_positive_guards: ["허가·수출 인증·설치 한 건을 reimbursement·procedure growth·repeat consumable 매출로 자동 연결하지 않는다."]
6. `C25_MEDICAL_DEVICE_EXPORT_REIMBURSEMENT_Q06` — working capital·CAPEX·FCF·revision·valuation은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION, EXPECTATION_VALUATION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING, CURRENT_MARKET_DATA, LAWFUL_REVISION_DATA
   - preferred_source_families: ISSUER_EARNINGS, OFFICIAL_FILING, CURRENT_MARKET_DATA, LAWFUL_REVISION_DATA
