...
- job_id: `PROMPT-V3-SNAPSHOT-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 첨단장비의 주문·매출 bridge와 현재 valuation/price blowoff를 대조하는 과열·4B 메커니즘
- required_bridge_axes: guard_risk, valuation_repricing, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer order/실적·고객 CAPEX·revision/consensus·시장 valuation·공시 자금조달·경쟁사 자료","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q01` — 실제 고객 주문·backlog·납품·매출 bridge가 있는가
   ...
   - false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
2. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q02` — 고객 quality·capacity cycle·제품 경쟁력이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: customer_quality_visible
   - required_source_roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - preferred_source_families: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
3. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q03` — 현재/forward normalized EPS·FCF와 valuation multiple은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION, FINANCIAL_CASH_CONVERSION, EXPECTATION_VALUATION
   - required_primitives: valuation_overheat
   ...
   - false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
4. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q04` — 가격 상승이 비가격 evidence보다 앞섰는가, price-only blowoff인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: DIRECT_PREDICATE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
5. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q05` — 고객 CAPEX·경쟁 증설·order pushout·inventory·share issuance counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: order_to_revenue_bridge, capex_cycle_risk
   - required_source_roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - preferred_source_families: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
6. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q06` — revision breadth와 cash conversion이 valuation을 지지하는가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["주가·테마·목표가만으로 mispricing 또는 thesis break를 만들지 않는다."]
7. `C09_ADVANCED_EQUIPMENT_VALUATION_BLOWOFF_Q07` — 4B/4C 후보가 현재 OPEN non-price claim으로 확인되는가
   - mandatory_for_full_thesis: `true`
