...
- job_id: `PROMPT-V3-SNAPSHOT-C13_BATTERY_JV_UTILIZATION_AMPC_IRA`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C13_BATTERY_JV_UTILIZATION_AMPC_IRA`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C13_BATTERY_JV_UTILIZATION_AMPC_IRA]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C13_BATTERY_JV_UTILIZATION_AMPC_IRA]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: JV 가동률·AMPC/IRA 보조금이 ex-credit economics와 현금으로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"JV/issuer/customer 공식자료·IRA/정부 규정·실적 note·현금흐름·CAPEX·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q01` — JV 소유구조·생산능력·실제 utilization·yield·고객 계약은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: jv_utilization, customer_contract
   ...
   - false_positive_guards: ["gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다."]
2. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q02` — AMPC/보조금 인식액·현금 수취·회계 분류와 ex-credit margin은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: ampc_or_subsidy_capture, ex_credit_margin
   - required_source_roles: AUDITOR_FILING, REGULATOR_OFFICIAL, ISSUER_EARNINGS, OFFICIAL_FILING
   - preferred_source_families: AUDITOR_FILING, REGULATOR_OFFICIAL, ISSUER_EARNINGS, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다."]
3. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q03` — 고객 ramp·volume·ASP·제품 mix가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다."]
4. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q04` — CAPEX·파트너 출자·부채·현금소진·FCF는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: DIRECT_PREDICATE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다."]
5. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q05` — 정책 eligibility·domestic content·법률/정권 reversal risk는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: policy_reversal_risk
   ...
   - false_positive_guards: ["gross AMPC 금액을 영구적인 영업마진·현금으로 그대로 세지 않는다."]
6. `C13_BATTERY_JV_UTILIZATION_AMPC_IRA_Q06` — 보조금 제외 사업성이 개선되는지 revision과 valuation으로 확인되는가
   - mandatory_for_full_thesis: `true`
