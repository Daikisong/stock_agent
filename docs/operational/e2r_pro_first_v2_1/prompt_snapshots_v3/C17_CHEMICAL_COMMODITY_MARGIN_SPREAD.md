...
- job_id: `PROMPT-V3-SNAPSHOT-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C17_CHEMICAL_COMMODITY_MARGIN_SPREAD]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 화학 제품 ASP와 feedstock 원가의 실현 spread가 inventory timing·가동률을 거쳐 margin/FCF로 전환되는 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer 실적/제품별 ASP·원재료 공식가격·가동률/정비·고객수요·재고/FCF·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q01` — 제품별 ASP·feedstock cost·realized spread가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: spread_expansion
   ...
   - false_positive_guards: ["원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다."]
2. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q02` — spot spread와 issuer 계약가격·재고 원가 lag를 분리했는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다."]
3. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q03` — 가동률·정기보수·수요·고객 mix·shipment는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다."]
4. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q04` — gross margin·OPM·EPS·FCF 전환이 실제로 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: opm_expansion_pctp
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다."]
5. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q05` — 원재료 급등·재고손실·공급증설·수요둔화 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["원재료/제품 spot 가격 차이만으로 issuer realized margin을 만들지 않는다."]
6. `C17_CHEMICAL_COMMODITY_MARGIN_SPREAD_Q06` — cycle phase·revision·normalized valuation은 무엇인가
   - mandatory_for_full_thesis: `true`
