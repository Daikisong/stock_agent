...
- job_id: `PROMPT-V3-SNAPSHOT-C01_ORDER_BACKLOG_MARGIN_BRIDGE`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C01_ORDER_BACKLOG_MARGIN_BRIDGE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C01_ORDER_BACKLOG_MARGIN_BRIDGE]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C01_ORDER_BACKLOG_MARGIN_BRIDGE]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 수주잔고가 실제 납품·매출·마진·현금으로 전환되는 산업재/인프라형 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"공시 계약서·사업보고서·수주잔고 표·issuer IR·고객/정부 발주처·실적/현금흐름·합법적 revision 자료","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["계약 제목이나 총잔고만으로 마진·현금·납품을 자동 추론하지 않는다."]
- score_authority: `false`
- stage_authority: `false`
...
1. `C01_ORDER_BACKLOG_MARGIN_BRIDGE_Q01` — 현재 유효한 수주잔고의 금액·기간·제품·고객·취소 가능성과 기존 매출 대비 규모는 무엇인가
   ...
   - required_primitives: named_customer_quality
...
4. `C01_ORDER_BACKLOG_MARGIN_BRIDGE_Q04` — 계약 mix와 원가·가격조정 조항이 OPM 개선 또는 훼손으로 어떻게 이어지는가
   ...
   - required_primitives: contract_quality, opm_expansion_pctp
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
