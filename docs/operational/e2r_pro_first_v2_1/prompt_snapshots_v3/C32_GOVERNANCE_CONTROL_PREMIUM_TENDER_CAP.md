...
- job_id: `PROMPT-V3-SNAPSHOT-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 지배권 거래·tender offer의 확정 조건·minority cash path·승인·event spread를 판정하는 메커니즘
- required_bridge_axes: capital_return, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"tender/공개매수/주요사항 공시·거래 당사자·규제기관·법원·financing·시장 spread","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q01` — binding tender/매매계약·가격·수량·대상 주주·조건이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: tender_offer_confirmed
   ...
   - false_positive_guards: ["경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다."]
2. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q02` — minority shareholder가 실제 현금화할 수 있는 경로와 강제/선택 조건은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: minority_cash_path
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다."]
3. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q03` — financing·regulatory/antitrust·주총·법원 승인과 timeline은 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다."]
4. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q04` — control premium floor·잔여지분·상장유지/폐지·희석 영향을 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION, FINANCIAL_CASH_CONVERSION
   - required_primitives: control_premium_floor
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다."]
5. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q05` — withdrawal·조건 미충족·가격조정·소송·event spread risk는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, EXPECTATION_VALUATION
   - required_primitives: event_spread_risk
   - required_source_roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   - preferred_source_families: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["경영권 분쟁·MOU·대주주 변경 가능성을 확정 현금 floor로 보지 않는다."]
6. `C32_GOVERNANCE_CONTROL_PREMIUM_TENDER_CAP_Q06` — rumor·기사·경영권 분쟁과 확정 공시를 분리했는가
   - mandatory_for_full_thesis: `true`
