...
- job_id: `PROMPT-V3-SNAPSHOT-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 메모리 가격·재고·공급규율 회복이 장비 주문과 issuer 매출·FCF로 전환되는 cycle 메커니즘
- required_bridge_axes: customer, backlog, margin, valuation_repricing
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"메모리 issuer/고객·장비 issuer 주문/IR·산업 가격/재고 공식자료·실적/FCF·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q01` — DRAM/NAND 가격·재고·utilization·공급 discipline의 현재 상태는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: memory_price_increase_mentioned, supply_discipline_mentioned
   ...
   - false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
2. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q02` — end-market 수요와 고객 CAPEX·장비 order recovery가 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: end_market_demand_visibility, equipment_order_recovery
   ...
   - false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
3. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q03` — advanced packaging/공정 전환 병목이 대상 장비 주문으로 직접 연결되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: advanced_packaging_bottleneck
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
4. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q04` — order-to-revenue·매출·margin·FCF conversion이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: cycle_to_revenue_bridge, order_to_revenue_bridge, fcf_quality_score
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
5. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q05` — 재고 조정·공급 증설·가격 반락·고객 CAPEX 지연 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: supply_demand_tightness
   - required_source_roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - preferred_source_families: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
6. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q06` — 회복 초기/중기/후기 phase와 revision breadth는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["메모리 가격 반등 기사만으로 장비 회사 매출 회복을 확정하지 않는다."]
7. `C10_MEMORY_RECOVERY_EQUIPMENT_CYCLE_Q07` — normalized valuation과 cycle midpoint cash flow는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION, EXPECTATION_VALUATION
   - required_primitives: cycle_demand_visibility, inventory_cycle_repair
