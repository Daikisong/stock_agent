...
- job_id: `PROMPT-V3-SNAPSHOT-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C28_SOFTWARE_SECURITY_CONTRACT_RETENTION]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 소프트웨어·보안 계약의 ARR/RPO·renewal·NRR/churn·margin leverage가 cash로 이어지는 메커니즘
- required_bridge_axes: software_retention, customer, margin
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer recurring KPI/계약/실적·고객 공식자료·RPO/renewal disclosures·breach/regulatory·FCF/SBC·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q01` — ARR·recurring revenue·RPO/backlog 정의와 기간·conversion은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: arr_growth_visible, rpo_to_sales
   ...
   - false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
2. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q02` — GRR/NRR·renewal·churn·logo/customer retention이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: nrr, retention_or_renewal
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
3. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q03` — 신규/upsell·contract duration·customer concentration·public/enterprise mix가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
4. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q04` — gross margin·sales efficiency·CAC/payback·operating leverage·FCF가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: recurring_margin_leverage
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
5. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q05` — 서비스/라이선스/일회성 구축 매출과 반복매출을 분리했는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
6. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q06` — 보안/AI/공공계약 headline만 있고 retention bridge가 없는 false positive는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["소프트웨어/보안 단어·수주 한 건·프로젝트 매출을 ARR/NRR/retention으로 자동 해석하지 않는다."]
7. `C28_SOFTWARE_SECURITY_CONTRACT_RETENTION_Q07` — 경쟁·가격·breach·customer loss·SBC/dilution·revision·valuation counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, EXPECTATION_VALUATION, LIFECYCLE_SUPERSESSION
   - required_primitives: DIRECT_PREDICATE
