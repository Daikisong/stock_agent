...
- job_id: `PROMPT-V3-SNAPSHOT-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 정부 간 방산 프레임워크가 구속력 있는 계약·납품·수금·마진으로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"대상 회사 공시/IR·발주국 정부/조달기관·수출승인 기관·계약 금융기관·실적/현금흐름·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q01` — MOU·framework·LOI와 구속력 있는 본계약을 분리했는가
   ...
   - false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
2. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q02` — 정부 고객·예산·수출승인·금융지원·계약 당사자가 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: export_contract, government_customer
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
3. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q03` — 수량·단가·납품 일정·현지화·offset·유지보수 범위가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: delivery_schedule, cost_overrun
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   ...
   - false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
4. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q04` — 잔고에서 매출·마진·선수금·수금·FCF로 이어지는 bridge가 있는가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
5. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q05` — 환율·원가·지체상금·현지생산·정치/승인 지연 counter가 있는가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
6. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q06` — 계약 변경·취소·납품 완료·후속 tranche로 lifecycle이 어떻게 변했는가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["정상회담·협상·우선협상·프레임워크를 확정 수주로 승격하지 않는다."]
7. `C03_DEFENSE_EXPORT_FRAMEWORK_BACKLOG_Q07` — revision과 valuation이 이미 수출 기대를 어느 정도 반영했는가
   - mandatory_for_full_thesis: `true`
