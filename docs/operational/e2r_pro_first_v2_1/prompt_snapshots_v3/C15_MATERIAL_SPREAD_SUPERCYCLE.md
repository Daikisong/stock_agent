...
- job_id: `PROMPT-V3-SNAPSHOT-C15_MATERIAL_SPREAD_SUPERCYCLE`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C15_MATERIAL_SPREAD_SUPERCYCLE`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C15_MATERIAL_SPREAD_SUPERCYCLE]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C15_MATERIAL_SPREAD_SUPERCYCLE]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 원재료/제품 가격 변화가 pass-through·재고 lag·utilization을 거쳐 실현 margin·cash로 전환되는 supercycle 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer 제품가격/실적·공식 commodity/industry data·고객/수요·재고/가동률·FCF·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q01` — commodity headline과 issuer 제품 ASP/pass-through를 분리했는가
   ...
   - false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
2. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q02` — feedstock·제품 가격·spread·재고평가·계약 가격 lag가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: spread_expansion, pricing_power_confirmed
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
3. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q03` — 수요·고객·shipment·utilization과 공급증설/감산이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: utilization_rate
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
4. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q04` — 실현 gross margin·OPM·EPS·FCF 전환이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: fcf_quality_score
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
5. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q05` — inventory/working capital·CAPEX·cash conversion은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: inventory_cycle
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING
   - preferred_source_families: ISSUER_EARNINGS, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
6. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q06` — cycle phase·신규 공급·가격 반락·late result counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["원자재 가격 상승·테마 기사·기록적 과거 실적만으로 현재 spread를 확정하지 않는다."]
7. `C15_MATERIAL_SPREAD_SUPERCYCLE_Q07` — revision·normalized valuation이 completed cycle을 선반영했는가
   - mandatory_for_full_thesis: `true`
