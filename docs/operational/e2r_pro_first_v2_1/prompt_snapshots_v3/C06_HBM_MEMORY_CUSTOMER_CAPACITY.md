...
- job_id: `PROMPT-V3-SNAPSHOT-C06_HBM_MEMORY_CUSTOMER_CAPACITY`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C06_HBM_MEMORY_CUSTOMER_CAPACITY`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C06_HBM_MEMORY_CUSTOMER_CAPACITY]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C06_HBM_MEMORY_CUSTOMER_CAPACITY]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: HBM/메모리의 고객 qualification·allocation·qualified capacity·ASP/mix가 이익과 FCF로 전환되는 메커니즘
- required_bridge_axes: customer, backlog, contract, margin
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer earnings/filing/IR·고객/플랫폼 공식자료·경쟁사 공식자료·장비/패키징 파트너·revision·시장데이터","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q01` — 고객 allocation·booking·commitment·LTA가 구속력 있는 물량/기간/가격으로 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: customer_preorder_or_allocation, memory_price_increase_mentioned
   ...
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
2. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q02` — capacity sold-out·pre-sold·수요 초과가 issuer 제품과 기간에 직접 귀속되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: hbm_capacity_constraint, hbm_capacity_pre_sold
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
3. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q03` — 고객별 qualification pass·진행·지연·실패·reopen 상태는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
4. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q04` — HBM 세대별 양산·상업 출하·shipment mix·revenue mix가 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: revenue_visibility_contract
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
5. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q05` — DRAM/HBM 실현 ASP·가격·제품 mix와 매출·영업이익 전환이 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
6. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q06` — margin·FCF·cash conversion과 CAPEX·yield·advanced packaging throughput은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: DIRECT_PREDICATE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
7. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q07` — 중기 revision·consensus·normalized valuation·expectation gap은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, EXPECTATION_VALUATION
   - required_primitives: medium_term_revision_visibility
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["HBM 단어·제품 스펙·업계 수요·고객사 CAPA를 issuer allocation/매출로 확대하지 않는다."]
8. `C06_HBM_MEMORY_CUSTOMER_CAPACITY_Q08` — 범용 DRAM/NAND drag·경쟁사 capacity·oversupply·고객집중 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
