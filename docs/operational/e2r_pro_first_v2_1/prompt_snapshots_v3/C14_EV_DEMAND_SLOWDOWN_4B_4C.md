...
- job_id: `PROMPT-V3-SNAPSHOT-C14_EV_DEMAND_SLOWDOWN_4B_4C`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C14_EV_DEMAND_SLOWDOWN_4B_4C`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C14_EV_DEMAND_SLOWDOWN_4B_4C]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C14_EV_DEMAND_SLOWDOWN_4B_4C]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: EV 수요 둔화·재고·가격인하·고객 CAPEX 축소가 계약·가동률·현금의 thesis break로 이어지는지 판정하는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"OEM/배터리 issuer·계약 정정/취소·재고/가동률·실적/impairment·정부/산업 수요자료","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q01` — EV 판매·배터리 출하·재고·가격·가동률 둔화가 issuer에 직접 귀속되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: ev_demand_slowdown, inventory_spike, price_cut_risk, thesis_break_confirmed
   ...
   - false_positive_guards: ["산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다."]
2. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q02` — 고객 CAPEX·공장 ramp·call-off·계약 변경이 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다."]
3. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q03` — margin·working capital·cash burn·impairment 영향은 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다."]
4. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q04` — 일시적 재고조정과 구조적 고객/제품 상실을 구분했는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다."]
5. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q05` — 과거 slowdown이 현재 해소·재개·악화됐는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["산업 판매 둔화나 주가 하락만으로 hard 4C를 만들지 않는다."]
6. `C14_EV_DEMAND_SLOWDOWN_4B_4C_Q06` — 4B watch와 hard 4C를 가르는 취소·permanent loss·신뢰 이슈가 있는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
