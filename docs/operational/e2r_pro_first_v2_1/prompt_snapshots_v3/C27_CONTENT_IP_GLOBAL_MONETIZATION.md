...
- job_id: `PROMPT-V3-SNAPSHOT-C27_CONTENT_IP_GLOBAL_MONETIZATION`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C27_CONTENT_IP_GLOBAL_MONETIZATION`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C27_CONTENT_IP_GLOBAL_MONETIZATION]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C27_CONTENT_IP_GLOBAL_MONETIZATION]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 콘텐츠/IP ownership·global launch·retention·라이선스/반복매출이 hit risk와 비용을 넘어 cash로 이어지는 메커니즘
- required_bridge_axes: software_retention, customer, margin
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer/플랫폼/배급사·권리/계약·사용자/매출 KPI·실적/비용/FCF·IP 법적자료·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q01` — IP ownership·권리지역·수익배분·partner/platform 조건은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: ip_monetization_visible
   ...
   - false_positive_guards: ["공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다."]
2. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q02` — global launch·사용자/시청/판매·retention·repeat engagement가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: global_launch_conversion, repeat_revenue, user_retention
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다."]
3. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q03` — 광고·구독·IAP·license·royalty·merchandise monetization이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다."]
4. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q04` — content amortization·개발/마케팅 비용·margin·FCF가 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다."]
5. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q05` — hit concentration·platform dependency·release delay·IP dispute·token/theme hype counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: token_or_theme_hype_risk
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: earnings_visibility, information_confidence
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   ...
   - false_positive_guards: ["공개/출시/조회수·유명 IP·코인/테마를 반복 monetization으로 자동 승격하지 않는다."]
6. `C27_CONTENT_IP_GLOBAL_MONETIZATION_Q06` — pipeline·sequel·catalog durability·revision·valuation은 무엇인가
   - mandatory_for_full_thesis: `true`
