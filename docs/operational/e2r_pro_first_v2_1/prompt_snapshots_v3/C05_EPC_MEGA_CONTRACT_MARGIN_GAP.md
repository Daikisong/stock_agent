...
- job_id: `PROMPT-V3-SNAPSHOT-C05_EPC_MEGA_CONTRACT_MARGIN_GAP`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C05_EPC_MEGA_CONTRACT_MARGIN_GAP`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C05_EPC_MEGA_CONTRACT_MARGIN_GAP]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C05_EPC_MEGA_CONTRACT_MARGIN_GAP]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 대형 EPC 계약의 규모와 기간이 공정률·원가·지체상금·운전자본을 거쳐 이익과 현금으로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"단일판매공급계약/사업보고서·발주처·프로젝트 update·실적 note·현금흐름·분쟁/법원·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q01` — 계약금액·기간·대상·고객·기존 매출 대비 규모와 회사 귀속분은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: delivery_schedule
   ...
   - false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
2. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q02` — 설계·조달·시공 범위와 공정률·매출 인식·대금 지급 milestone은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: contract_amount_to_prior_sales
   ...
   - false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
3. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q03` — 가격 조정·원자재/인건비 pass-through·보증·지체상금 조건은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
4. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q04` — 프로젝트 mix가 gross margin·OPM·FCF에 미치는 영향이 실제로 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: margin_bridge_visible
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING
   - preferred_source_families: ISSUER_EARNINGS, OFFICIAL_FILING
   - affected_component_ids: bottleneck_pricing, eps_fcf_explosion, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
5. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q05` — 선수금·미청구공사·매출채권·운전자본과 현금 회수 상태는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION, FINANCIAL_CASH_CONVERSION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING
   - preferred_source_families: ISSUER_EARNINGS, OFFICIAL_FILING
   - affected_component_ids: earnings_visibility, information_confidence
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
6. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q06` — cost overrun·재공사·분쟁·납기 지연·고객 신용 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["계약금액만으로 이익·현금·Stage를 올리지 않는다."]
7. `C05_EPC_MEGA_CONTRACT_MARGIN_GAP_Q07` — revision과 valuation은 headline contract와 margin gap을 구분하는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION, EXPECTATION_VALUATION
   - required_primitives: contract_duration_months
