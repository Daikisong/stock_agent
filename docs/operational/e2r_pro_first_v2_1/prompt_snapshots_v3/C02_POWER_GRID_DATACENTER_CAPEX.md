...
- job_id: `PROMPT-V3-SNAPSHOT-C02_POWER_GRID_DATACENTER_CAPEX`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C02_POWER_GRID_DATACENTER_CAPEX`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C02_POWER_GRID_DATACENTER_CAPEX]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C02_POWER_GRID_DATACENTER_CAPEX]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 전력망·데이터센터 CAPEX가 고객 주문, 긴 lead time, 공급제약, 가격결정력과 매출로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"공식 수주/IR·유틸리티/클라우드 고객 공식자료·공장/lead-time 공시·실적·산업 설비 공급자료·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C02_POWER_GRID_DATACENTER_CAPEX_Q01` — 데이터센터·전력망·유틸리티 고객이 대상 회사의 직접 고객으로 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: datacenter_customer
   ...
   - false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
2. `C02_POWER_GRID_DATACENTER_CAPEX_Q02` — backlog·book-to-bill·lead time·납품 일정과 현재 증설 전 saleable capacity는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: order_backlog_to_sales, lead_time_extended, capacity_constraint
   ...
   - false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
3. `C02_POWER_GRID_DATACENTER_CAPEX_Q03` — 공급제약이 단순 업계 shortage인지 대상 회사의 제품·공장·고객 주문에 직접 귀속되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
4. `C02_POWER_GRID_DATACENTER_CAPEX_Q04` — 실현 ASP·가격 인상·제품 mix·마진 전환이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: pricing_power_confirmed
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
5. `C02_POWER_GRID_DATACENTER_CAPEX_Q05` — 증설 CAPEX의 가동시점·수율·고객 qualification·현금회수 경로는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: DIRECT_PREDICATE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
6. `C02_POWER_GRID_DATACENTER_CAPEX_Q06` — 고객 CAPEX 둔화·경쟁 증설·원재료·프로젝트 지연·정책 변화 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - preferred_source_families: REGULATOR_OFFICIAL, ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - affected_component_ids: earnings_visibility, information_confidence
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["데이터센터 산업 성장이나 전력수요 전망만으로 대상 회사 수주를 만들지 않는다."]
7. `C02_POWER_GRID_DATACENTER_CAPEX_Q07` — revision·FCF·valuation이 현재 backlog와 공급제약을 선반영했는가
   - mandatory_for_full_thesis: `true`
