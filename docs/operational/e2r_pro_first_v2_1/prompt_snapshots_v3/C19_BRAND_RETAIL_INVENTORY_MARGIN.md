...
- job_id: `PROMPT-V3-SNAPSHOT-C19_BRAND_RETAIL_INVENTORY_MARGIN`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C19_BRAND_RETAIL_INVENTORY_MARGIN`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C19_BRAND_RETAIL_INVENTORY_MARGIN]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C19_BRAND_RETAIL_INVENTORY_MARGIN]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 브랜드 수요·sell-through·재고·가격결정력이 할인/원가를 거쳐 margin과 cash로 전환되는 메커니즘
- required_bridge_axes: consumer_sell_through, margin, customer
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer retail/채널 KPI·유통사/플랫폼·재고/markdown·실적/margin/FCF·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q01` — 매장/온라인 sell-through·same-store·제품/지역 mix가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: sell_through_confirmed
   ...
   - false_positive_guards: ["매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다."]
2. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q02` — 회사·유통채널 inventory·재고일수·markdown·반품 상태는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: inventory_spike
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   ...
   - false_positive_guards: ["매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다."]
3. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q03` — 가격 인상·promotion·원가/물류·gross margin bridge가 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다."]
4. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q04` — reorder·채널 확장·브랜드 customer diversification이 확인되는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다."]
5. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q05` — working capital·marketing·매장 CAPEX·FCF 전환은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["매장수·검색량·브랜드 언급만으로 sell-through와 pricing power를 만들지 않는다."]
6. `C19_BRAND_RETAIL_INVENTORY_MARGIN_Q06` — fad·재고 spike·가격저항·원가 상승·채널 의존 counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: raw_material_cost_risk
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   - preferred_source_families: ISSUER_OFFICIAL, OFFICIAL_FILING
   - affected_component_ids: earnings_visibility, information_confidence
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
