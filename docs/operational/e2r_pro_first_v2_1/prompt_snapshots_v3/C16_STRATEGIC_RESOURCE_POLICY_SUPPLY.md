...
- job_id: `PROMPT-V3-SNAPSHOT-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C16_STRATEGIC_RESOURCE_POLICY_SUPPLY]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 전략자원 shortage·정책지원·offtake·permit가 실제 생산·판매·현금으로 이어지는 메커니즘
- required_bridge_axes: margin, contract, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer filing/technical report·정부 permit/정책·offtake 고객·프로젝트 금융·생산/원가·시장 supply data","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q01` — offtake 계약의 고객·최소물량·가격·기간·조건은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: offtake_contract
   ...
   - false_positive_guards: ["자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다."]
2. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q02` — 매장량/자원량과 실제 permit·건설·commissioning·production 상태를 분리했는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: permit_status
   ...
   - false_positive_guards: ["자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다."]
3. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q03` — 정책/보조금·정부 승인·인프라·환경/지역사회 조건은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE
   - required_primitives: policy_supply_support
   - required_source_roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   - preferred_source_families: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다."]
4. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q04` — 생산원가·CAPEX·funding·dilution·cash runway와 직접 cash route는 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다."]
5. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q05` — 대체 공급·가격 하락·permit 지연·기술/수율·정치 risk counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   - required_primitives: supply_shortage
   - required_source_roles: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   - preferred_source_families: REGULATOR_OFFICIAL, ISSUER_OFFICIAL
   - affected_component_ids: earnings_visibility, information_confidence
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["자원량·정책 headline·MOU를 생산량·매출·현금으로 자동 변환하지 않는다."]
6. `C16_STRATEGIC_RESOURCE_POLICY_SUPPLY_Q06` — revision·valuation이 pre-production optionality를 어느 정도 반영했는가
   - mandatory_for_full_thesis: `true`
