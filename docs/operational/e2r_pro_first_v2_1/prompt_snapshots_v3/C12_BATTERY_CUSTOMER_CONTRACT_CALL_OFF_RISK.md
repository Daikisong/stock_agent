...
- job_id: `PROMPT-V3-SNAPSHOT-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 배터리 고객 계약의 실제 call-off·volume·고객 CAPEX 변화와 취소/지연 위험을 판정하는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer/고객 계약·OEM CAPEX/판매·JV 자료·실적/가동률·공시 정정/취소·현금흐름","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q01` — 현재 유효한 고객 계약과 call-off 권리·최소물량·가격·기간이 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: customer_contract, call_off_risk
   ...
   - false_positive_guards: ["고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다."]
2. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q02` — 고객 EV 판매·공장 CAPEX·platform 일정과 issuer volume visibility가 일치하는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, LIFECYCLE_SUPERSESSION
   - required_primitives: customer_capex_decline, contract_cancelled_or_delayed, volume_visibility
   - required_source_roles: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - preferred_source_families: ISSUER_OFFICIAL, CUSTOMER_PARTNER_OFFICIAL
   - affected_component_ids: bottleneck_pricing, earnings_visibility, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다."]
3. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q03` — 계약 축소·pushout·취소·가동률 하락·재고 증가가 현재 OPEN인가
   - mandatory_for_full_thesis: `true`
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다."]
4. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q04` — 가격 인하·원재료·고정비 흡수·margin·cash impact가 무엇인가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다."]
5. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q05` — 과거 지연이 최신 공시에서 해소·재개·악화됐는가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK, LIFECYCLE_SUPERSESSION
   - required_primitives: DIRECT_PREDICATE
   - required_source_roles: ISSUER_OFFICIAL, OFFICIAL_FILING
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["고객 EV 둔화 일반론을 대상 계약 취소로 확대하지 않는다."]
6. `C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK_Q06` — Stage hard break와 단순 watch를 구분할 직접 source가 있는가
   - mandatory_for_full_thesis: `true`
