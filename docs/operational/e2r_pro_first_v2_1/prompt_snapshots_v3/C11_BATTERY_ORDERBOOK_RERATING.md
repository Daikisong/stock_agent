...
- job_id: `PROMPT-V3-SNAPSHOT-C11_BATTERY_ORDERBOOK_RERATING`
- run_id: `PROMPT-V3-SNAPSHOT-RUN-C11_BATTERY_ORDERBOOK_RERATING`
...
최종 응답에는 `[[E2R_PRO_RUN_ID:PROMPT-V3-SNAPSHOT-RUN-C11_BATTERY_ORDERBOOK_RERATING]]`, `[[E2R_PRO_JOB_ID:PROMPT-V3-SNAPSHOT-C11_BATTERY_ORDERBOOK_RERATING]]`, `[[E2R_PRO_PASS_ID:TO_BE_BOUND_BY_ORCHESTRATOR]]`, `[[E2R_PRO_PARENT_PASS_ID:NONE]]` marker를 각각 정확히 한 번 출력한다.
...
- economic_mechanism: 배터리 orderbook이 고객 call-off·ramp·ex-credit margin·FCF로 전환되는 메커니즘
- required_bridge_axes: margin, backlog, contract, customer, guard_risk
- source_role_policy: `{"llm_only_inference_is_diagnostic":true,"official_first":true,"recommended_routes":"issuer 계약/IR·OEM/JV 고객 공식자료·공장 ramp·실적/AMPC note·FCF·revision","source_proxy_is_non_scoring":true}`
- false_positive_guards: ["수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다."]
- score_authority: `false`
- stage_authority: `false`

Mandatory question families:

1. `C11_BATTERY_ORDERBOOK_RERATING_Q01` — orderbook/수주잔고의 고객·제품·기간·구속력·취소/콜오프 조건은 무엇인가
   ...
   - false_positive_guards: ["수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다."]
2. `C11_BATTERY_ORDERBOOK_RERATING_Q02` — 고객 공장·차종·platform ramp와 volume schedule이 확인되는가
   - mandatory_for_full_thesis: `true`
   ...
   - false_positive_guards: ["수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다."]
3. `C11_BATTERY_ORDERBOOK_RERATING_Q03` — ASP·원재료 pass-through·AMPC 제외 margin·mix가 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   - required_primitives: ex_credit_margin
   - required_source_roles: ISSUER_EARNINGS, OFFICIAL_FILING
   - preferred_source_families: ISSUER_EARNINGS, OFFICIAL_FILING
   - affected_component_ids: earnings_visibility, eps_fcf_explosion, information_confidence, market_mispricing, valuation_rerating
   - allowed_terminal_statuses: SUPPORTED_SCORING, PARTIALLY_SUPPORTED_SCORING, SUPPORTED_NON_SCORING, COUNTER_SUPPORTED, EVALUATED_ABSENT_AFTER_ADEQUATE_SEARCH, LIKELY_NONPUBLIC, FUTURE_EVENT_ONLY, NOT_APPLICABLE_WITH_REASON
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다."]
4. `C11_BATTERY_ORDERBOOK_RERATING_Q04` — CAPEX·utilization·yield·working capital·FCF 전환은 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, FINANCIAL_CASH_CONVERSION
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`1`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다."]
5. `C11_BATTERY_ORDERBOOK_RERATING_Q05` — 고객 capex 축소·call-off·계약 지연/취소·재고·price cut counter는 무엇인가
   - mandatory_for_full_thesis: `true`
   - question_roles: ECONOMIC_BRIDGE, COUNTER_HARD_BREAK
   ...
   - adequate_search: official-first=`true`, minimum_distinct_source_routes=`2`, independent_no_new_route_confirmations=`2`
   - false_positive_guards: ["수주총액·LOI·장기 협력만으로 고정 volume과 ex-credit 수익성을 가정하지 않는다."]
6. `C11_BATTERY_ORDERBOOK_RERATING_Q06` — revision·consensus·valuation이 orderbook headline을 어느 정도 반영했는가
   - mandatory_for_full_thesis: `true`
