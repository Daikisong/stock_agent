...
- **exact-head CI:** [Pro PR 36077141865](https://github.com/Daikisong/stock_agent/actions/runs/36077141865), [Pro push 36077138635](https://github.com/Daikisong/stock_agent/actions/runs/36077138635), [V6 36077141902](https://github.com/Daikisong/stock_agent/actions/runs/36077141902)는 모두 `SUCCESS`. Pro PR/push/V6 각각 7,952 tests, failure/error 0, skipped 38. Pro PR core-unit 300, browser-mock-e2e 105, Reviewer A–H, compile/diff-check 모두 PASS. V6 Gate 1 receipt 4/4, Phase100 15/15, production static audit critical 0.
- **이번 P112 문서 push 후 CI:** 문서만 바뀌어도 workflow가 실행됨을 이전 두 commit에서 확인했다. 이 기록 자체를 push하면 새로운 resulting SHA의 세 run이 발생할 수 있다. 따라서 P112 base SHA의 위 green 상태와 다음 문서 head의 CI를 구분하고, next exact head가 모두 green되기 전에는 P10/final verdict를 선언하지 않는다.
- **BrowserUse:** WSL preflight exit 0. persistent Node REPL의 BrowserUse `extension` 연결 성공, agent tab list 1 / user open tabs 4. 정확한 ChatGPT root 탭 1개를 claim하고 같은 객체에서 read-only 확인: `https://chatgpt.com/`, 로그인 prompt 없음, composer 모델 label `6 Pro`, 입력 텍스트 비어 있음. 다른 ChatGPT 탭은 없고 `research_packet(20260924-172107).json` 첨부 및 remove action이 남아 있었다. hash는 C15 packet과 비교 검증되지 않았다. 타일 제거, 새 대화, download/upload, 입력, submit/capture는 전부 하지 않았다.
- **C15 durable state:** SQLite URI `mode=ro`, `PRAGMA query_only=ON` 확인. `PROJOB-df15a37c58ae7583924e58c0` / S-Oil `010950` / `2026-08-23`, `USER_ATTENTION_REQUIRED` v26, packet hash `fa5845a055661c99c2ab1eb9cfb65f66fb84d2c85b267b3cde33b54843c320df`, submit/capture `0/0`, approval/browser/conversation binding 없음, successor 없음. last error `BrowserUIIncompatible: the exact BrowserUse packet file/hash was not visible in the claimed tab`; latest event `DRAFT_PREPARATION_OR_UNKNOWN`, `safe_unprepared_resume=false`, `new_chat_route_required=true`.
- **전체 목표:** P9 필수 canary C06/C17/C28은 `1/3` (C06 PASS, C17/C28 미완료). C15는 별도 same-job 재개 작업이며 unresolved. 기존 첨부 제거에 대한 구체 허가는 확인되지 않았다. exact CI green은 live canary를 대체하지 않는다.
- **이번 P112 범위:** CI와 탭/DB read-only 재확인뿐. 새 research/query/fetch, new job/pass, other archetype, score/Stage change, composer mutation, submit/capture, PR draft 해제/merge는 없다.
- **다음 단계:** 사용자의 기존 첨부를 제거해도 된다는 구체 허가가 오면, 먼저 C15 durable state와 같은 기존 로그인 tab을 실행 직전에 다시 검증한 뒤 exact packet filename/hash와 actual Pro selection을 모두 확인한다. 불일치 또는 허가 부재면 보내지 않는다.

P112 상태 영수증: [p112_existing_session_revalidation_and_exact_head_ci_receipt.json](p112_existing_session_revalidation_and_exact_head_ci_receipt.json).

## 과거 기준선 — P111, 2026-09-25 09:15 KST (P112에서 대체됨)
...
## 과거 기준선 — P100, 2026-09-25 03:56 KST (P109에서 대체됨)
...
- P76 focused BrowserUse bridge + fresh orchestration 회귀 **94/94 PASS**, `node --check` 및 `git diff --check` PASS였다. P76 exact-head 전체 CI도 위 세 GitHub Actions에서 SUCCESS로 끝났다.
- 2026-09-24 12:07 KST에 persistent `mcp__node_repl__js`의 동일 BrowserUse extension과 기존 claimed tab만 재확인했다. `browser.user.openTabs()`는 1개, origin/title은 `https://chatgpt.com` / `ChatGPT`; 새 browser/window/tab/profile, CDP, relogin, navigation/input/upload/submit/capture는 0이다. 실제 Pro 선택·빈 composer·첨부 0은 P76의 같은 탭 read-only 증거다.
- 준비 순서의 주의점: `setupBrowserRuntime()`이 반환한 Agent를 `globalThis.agent`에 보관하고 `globalThis.browser = await globalThis.agent.browsers.get("extension")`로 extension Promise를 기다린다. 이후 `browser.user.openTabs()`에서 정확한 기존 탭을 고르고, `claimTab()`이 반환한 실제 tab 객체를 유지한다. bridge와 Python worker는 한 `mcp__node_repl__js` 호출 안에서 실행하고 `await bridge.runUntil(workerPromise)`를 사용해야 한다.
- **다음 한 단계:** 정확한 동일 C15 R6 job만 위 기존 로그인 탭에서 재개한다. packet hash/approval/session/conversation/submit/capture 상태를 실행 직전 재확인하고, bridge handshake가 실패하면 UI 입력 전 단계에서 멈춰 원인·범위를 기록한다. 새 browser/session/job은 만들지 않는다.
- 자세한 CI, 브라우저 연결 순서, job 상태는 [P77 progress](#p77--기존-browseruse-로그인-탭-재확인과-p76-exact-head-ci-정정-2026-09-24-1207-kst), [P77 receipt](p77_browseruse_existing_session_docs_ci_receipt.json), [BrowserUse handoff](browseruse_existing_session_handoff.md)을 본다.
...
## P75 — BrowserUse DOM evaluate callback 계약 보정과 동일 로그인 탭 smoke (2026-09-24 10:42 KST)

### 작업 범위와 기존 세션 사용

- 이 단계는 P74 exact-head CI 성공 뒤, 같은 미전송 C15 R6를 파이프라인 read-only preflight까지 안전하게 재개할 수 있는지 확인하는 작업이다.
  새 조사 자료, source query/fetch, 다른 archetype, 점수/Stage 변경은 범위에 넣지 않았다.
- 사용자의 반복 지시대로 BrowserUse가 필요한 검사는 사용자가 이미 로그인한 Chrome BrowserUse `extension` 세션에 열려 있던
  기존 ChatGPT 탭을 그대로 사용했다. 새 브라우저/창/탭/프로필, CDP 대체 연결, 재로그인은 전혀 만들지 않았다.
- 직접 읽기 전용 관찰은 `https://chatgpt.com/`, title `ChatGPT`, composer 1개와 빈 composer를 확인했다. 이어 같은 tab object에
  BrowserUse bridge를 붙여 Python `ProBrowserWorker`의 `inspect_state()`를 실행했다. 반환은 editor ready, prompt blank, send disabled,
  stop absent였고 `deep_research_ready=true`였다. 이 legacy field는 코드상 **일반 Chat + 실제 Pro** readiness를 뜻하며
  Work/legacy Deep Research는 비활성인 조건이다. 실제 Pro gate도 통과했다.
- smoke의 첫 harness는 BrowserUse RPC 전에 WSL Python `sys.path`에 worktree `src`가 없어 `ModuleNotFoundError: No module named 'e2r'`로
  끝났다. 이것은 브라우저 오류나 로그인 문제가 아니며 UI/내구 상태를 바꾸지 않았다. 같은 기존 탭과 ephemeral smoke job으로
  `sys.path`만 바로잡아 재실행한 결과 PASS다. 수정된 호출은 저장소 경로를 명시한다.

### 재현된 blocker와 수정

- 같은 C15 R6 read-only resume를 재시도했을 때, packet attach나 사용자 입력 전 `div.ProseMirror[contenteditable="true"] >> nth=0`의
  `locator.evaluate()`가 3초 뒤 `Playwright selector deadline exceeded`로 실패했다. SQLite event는 이를
  `READ_ONLY_BROWSER_PREFLIGHT`로 기록했고 `safe_unprepared_resume=true`다.
- root cause: BrowserUse extension의 `locator.evaluate()` / `page.evaluate()`는 실제 JS function callback을 받는데 bridge는
  표현식을 문자열 호출식으로 합성해 전달하고 있었다. 동일 탭에서 function callback 직접 호출은 읽기 결과를 돌려주지만 문자열 호출식은 timeout했다.
- trusted Node runtime은 `new Function`/`eval` 문자열 코드 생성이 차단되어 있다. 우회로 동적 컴파일을 켜지 않았다. 대신 bridge가 쓰는
  승인된 read-only DOM 표현식을 **static callback function allowlist**로 매핑하고 arguments를 별도로 전달하게 했다.
  미등록 callback은 “reviewed read-only callback 없음”으로 닫는다. DOM click/mutation 표현식은 기존 guard에서 계속 거부한다.
- regression은 tag/value/body text, user-turn marker, tail, citation registry, turn id, local download container, assistant identity/snapshot,
  editor exact text, selected input file의 실제 adapter callback 표현식이 allowlist에 매핑되는지 확인하고, mutation/미등록 callback을 거부한다.
  임의 표현식 실행 경로는 추가하지 않았다.

### 검증 결과와 실제 C15 상태

- 다음 focused suite **91/91 PASS**:

  ```bash
  PYTHONPATH=src LD_LIBRARY_PATH=/home/eorb915/.cache/e2r-playwright-libs/usr/lib/x86_64-linux-gnu \
    python -m unittest tests.test_e2r_pro_first_browseruse_extension_bridge \
    tests.test_e2r_pro_first_v2_1_fresh_orchestration -q
  ```

- `node --check src/e2r/pro_first/browser/browseruse_extension_bridge.mjs`: PASS; `git diff --check`: PASS.
- 같은 기존 로그인 탭을 거친 end-to-end Python worker read-only smoke 결과:
  `editor_ready=true`, `deep_research_ready=true`, `prompt_ready=false`, `send_ready=false`, `stop_visible=false`,
  `state=DEEP_RESEARCH_MODE_READY`. packet upload/click/submit/capture/navigation은 수행하지 않았다. `packet_uploaded=false`는 이번 fresh
  smoke worker가 packet upload를 하지 않았다는 상태값이지, 계정의 모든 대화·첨부를 검색했다는 주장은 아니다.
- 중앙 SQLite를 URI `mode=ro` 및 `PRAGMA query_only=ON`으로 읽었다. 유일한 허용 C15 R6 job
  `PROJOB-df15a37c58ae7583924e58c0` / `010950` / `C15_MATERIAL_SPREAD_SUPERCYCLE`는 `USER_ATTENTION_REQUIRED`, version 8이며
  packet hash `fa5845a055661c99c2ab1eb9cfb65f66fb84d2c85b267b3cde33b54843c320df`가 그대로다. approval field, browser session, conversation은
  모두 미결박이고 submit/capture `0/0`; successor job은 없다. 마지막 오류 문자열과 상태는 [P75 receipt](p75_browseruse_evaluate_callback_receipt.json)에 보존한다.
- P75 동안 prompt input/upload/submit/capture `0/0/0/0`, 새 source query/fetch `0/0`, score/Stage 변경 `0/0`이다.

### CI 경계와 한 단계 다음

- GitHub에서 재확인한 P74 head `8b86d2db6479c40d4bc0463354dc9a3de711087e`의 Pro push run
  [35940763725](https://github.com/Daikisong/stock_agent/actions/runs/35940763725), Pro PR run
  [35940765268](https://github.com/Daikisong/stock_agent/actions/runs/35940765268), V6 PR run
  [35940765227](https://github.com/Daikisong/stock_agent/actions/runs/35940765227)은 모두 SUCCESS다. Pro regression은 7,920 tests,
  38 skipped, failure/error 0이었다. 이는 P75 코드에는 적용되지 않는다.
- P75 작업 트리는 현재 P74 head 위의 local diff이며 새 exact-head CI는 아직 없다. 따라서 기존 C15 R6 runner를 지금 재개하지 않는다.
...
## P77 — 기존 BrowserUse 로그인 탭 재확인과 P76 exact-head CI 정정 (2026-09-24 12:07 KST)

### 이번 갱신의 범위

이 단계는 운영 문서와 증거를 최신화했다. 새 리서치나 새 browser session/job을 만들지 않았다. 기존 사용자 로그인 세션 사용 규칙은 이미 있었지만, 최상단 handoff와 진행 요약이 P76 CI를 `pending`으로 남겨 둔 것이 현재 GitHub 결과와 어긋났다. 외부 검토자가 최신 SHA, CI run, 다음 한 단계와 브라우저 경계를 바로 찾도록 상태를 정정하고 P77 receipt를 추가했다.

### exact head와 GitHub 검증

```text
branch       feature/e2r-pro-first-browser-platform-20260822
local head   d294f395738254da1f616d361038a35548fa6c4e
origin head  d294f395738254da1f616d361038a35548fa6c4e
PR #7        OPEN / DRAFT / MERGEABLE; main 미병합
```

같은 head에 연결된 모든 필수 run을 GitHub에서 다시 열어 확인했다.

| workflow | run | 결과 | 주요 결과 |
|---|---:|---|---|
| Pro push | [35947967714](https://github.com/Daikisong/stock_agent/actions/runs/35947967714) | SUCCESS | full regression 7,923, skipped 38, failure/error 0; Reviewer A–H PASS |
| Pro PR | [35947970682](https://github.com/Daikisong/stock_agent/actions/runs/35947970682) | SUCCESS | full regression 7,923, skipped 38, failure/error 0 |
| V6 PR | [35947970654](https://github.com/Daikisong/stock_agent/actions/runs/35947970654) | SUCCESS | Gate 1 receipt 4/4, Phase100 15/15, production static audit critical 0, full unittest failure/error 0 |

따라서 P76의 “exact-head CI 대기”는 당시에는 맞았지만 현재 상태가 아니다. P76 receipt는 당시 상태의 역사 기록으로 유지하고, 최신 상태는 이 P77 기록을 기준으로 삼는다.

### BrowserUse: 항상 사용자의 기존 로그인 세션·기존 탭

2026-09-24 12:07 KST에 persistent `mcp__node_repl__js`에서 앞서 claim한 BrowserUse `extension` tab 객체를 그대로 사용해 읽기 전용 확인했다. `browser.user.openTabs()` 결과 1개, 기존 claim 객체의 origin/title `https://chatgpt.com` / `ChatGPT`였다. 이 확인에서 navigation, prompt 입력, file attach, submit, response capture는 하지 않았다. 새 창·탭·프로필, CDP 대체 연결, relogin도 없었다. 실제 선택 Pro, 빈 composer, selected file 0은 P76의 같은 기존 탭 read-only 확인에 남아 있다.

다음 작업자는 이 순서를 그대로 따른다.

```js
// persistent node_repl 안에서만. 이미 만들어 둔 BrowserUse globals를 재사용한다.
globalThis.browser = await globalThis.agent.browsers.get("extension");
```

`setupBrowserRuntime()`의 반환 Agent를 `globalThis.agent`로 보존하고, `browsers.get("extension")`의 Promise를 await한다. 그런 다음 `browser.user.openTabs()`로 사용자 탭을 열거하고, 현재 작업과 정확히 일치하는 descriptor 하나를 선택해 `browser.user.claimTab()`에 전달한다. descriptor는 tab 제어 객체가 아니므로 claim이 반환한 실제 tab 객체만 이후 작업에 넘긴다. 이미 claim한 tab 객체가 있으면 다시 claim/reset하지 않고 보존한다. 외부 사용자 탭을 찾을 때 `browser.tabs.list()`로 대신하지 않는다. persistent REPL의 기존 globals를 지우거나 reset하지 않는다.

C15 R6 pipeline 재개 시 bridge와 Python worker는 반드시 **하나의 active `mcp__node_repl__js` tool call 안에서** 함께 실행하고 `await bridge.runUntil(workerPromise)`를 수행한다. background로 남긴 bridge를 나중의 다른 REPL call에서 이어 실행하면 `BRIDGE_OPERATION_FAILED: node_repl exec context not found`가 날 수 있다. 이는 로그인 실패가 아니며, 새 세션·새 창·재로그인으로 우회하지 않는다.

### 현재 미완료 job과 범위

P76 SQLite read-only receipt 기준 active job `PROJOB-df15a37c58ae7583924e58c0` (symbol `010950`, C15 R6)은 `USER_ATTENTION_REQUIRED` version 10이다. packet hash는 유지됐고 approval/browser/conversation binding 없음, submit/capture `0/0`, successor 없음이다. 이 기록을 새로 만든 job이나 새 session으로 대체하지 않는다.
...
## P80 — 기존 로그인 BrowserUse 세션 사용 및 same-job read-only preflight PASS (2026-09-24 14:08 KST, P81에서 후속)

사용자가 로그인된 브라우저 작업은 **이미 로그인된 BrowserUse `extension` 세션의 기존 작업 탭에서 하라**고 다시 명확히 지시했다. 규칙 자체는 앞선 handoff에 있었지만, 실행에서 반복적으로 빗나간 점을 반영해 `browseruse_existing_session_handoff.md` 첫 화면에 실행 순서를 올렸다. 다음 작업자는 이를 배경 참고가 아니라 필수 절차로 따라야 한다.

- 반드시 `setupBrowserRuntime()` → `agent.browsers.get("extension")` → `browser.user.openTabs()` → 기존 사용자 탭 descriptor 확인 → `browser.user.claimTab()` 순서로 같은 로그인 탭을 붙잡는다.
- 기존 응답·JSON·첨부가 있으면 같은 탭에서 먼저 확인하고 그 결과를 회수한다. 새 요청·새 대화·재전송은 필요한 경우에만 같은 탭 안에서 한다.
- extension 또는 기존 탭 연결이 실패하면 실제 오류와 확인 범위를 기록하고 멈춘다. 새 브라우저·창·프로필·CDP 대체 세션·재로그인으로 우회하지 않는다. 기술 오류를 보안 정책 거부로 잘못 분류하지 않는다.
### exact-head CI 뒤 기존 탭에서 한 검증

- P79 exact head의 Pro push/Pro PR/V6 PR 세 run은 모두 SUCCESS로 끝났다. 그 뒤 canonical BrowserUse runtime의 `extension`에서 사용자 탭을 열거하고, 기존 ChatGPT 탭 하나(`chatgpt.com` home)를 정확히 claim했다. 새 window/tab/profile/CDP, relogin, navigation은 없었다.
- 같은 탭의 read-only DOM 판독: 로그인 prompt 없음, composer 1개, composer 바로 옆 실제 선택 `Pro` control, home 화면, 대화/첨부 없음.
- bridge와 WSL Python worker를 같은 active `mcp__node_repl__js` 호출에서 `await bridge.runUntil(workerPromise)`로 실행했다. Worker는 `ProBrowserWorker(BROWSER_USE_EXTENSION).open(job_id=기존 C15 R6)` 후 `ensure_logged_in()`과 `inspect_state()`만 수행했다. `READ_ONLY_PREFLIGHT_COMPLETE`; login/state `DEEP_RESEARCH_MODE_READY`, `editor_ready=true`, `pro_mode_ready=true`, `packet_uploaded=false`, `prompt_ready=false`, `send_ready=false`, `stop_visible=false`, conversation 미결박. 수정된 bounded evaluate 경로에서 이전 3초 selector timeout은 이번 동일 탭 smoke에서 재현되지 않았다. 다만 단독 root cause라는 주장은 하지 않는다.
- 실행 후 SQLite mode=ro + `PRAGMA query_only=ON`: same job `PROJOB-df15a37c58ae7583924e58c0`, version 12, packet hash unchanged, status `USER_ATTENTION_REQUIRED`, browser/conversation/approval 미결박, submit/capture `0/0`, successor 없음. `last_error` 값은 과거 timeout을 보존하지만 새 read-only preflight는 성공했다.
- 이 브라우저 smoke는 읽기 전용이었다. input/upload/download/submit/capture `0/0/0/0/0`, source query/fetch `0/0`, score/Stage change `0/0`. 사용자 기존 탭을 닫거나 초기화하지 않았다.
...
|---|---|
| PR #7 | OPEN / DRAFT / MERGEABLE, main 미병합 |
| exact head | `281354cfc6d8e88f21c2bd1b32db72fea6f2f155` |
| Pro push run [35955888505](https://github.com/Daikisong/stock_agent/actions/runs/35955888505) | SUCCESS |
| Pro PR run [35955892468](https://github.com/Daikisong/stock_agent/actions/runs/35955892468) | SUCCESS |
| V6 PR run [35955892448](https://github.com/Daikisong/stock_agent/actions/runs/35955892448) | SUCCESS |

P80 당시 계획된 다음 단계는 same-job 재개였다. 그 재개는 P81에서 기존 탭의 read-only selector timeout으로 입력 전 중단됐으므로, 이 계획은 superseded됐다. 현재 실행 순서는 아래 P81 checkpoint와 [BrowserUse existing-session handoff](browseruse_existing_session_handoff.md)을 따른다.

## P81 — BrowserUse selector read-path timeout 보강과 same-job 실패 경계 기록 (2026-09-24 14:23 KST; 문서 최신화 14:27 KST)

### 우선 규칙: 로그인 세션은 사용자의 기존 BrowserUse 탭이다

사용자가 다시 명시한 대로, 로그인된 서비스 작업은 BrowserUse Chrome `extension`이 붙인 **사용자 기존 로그인 세션 및 그 안의 정확한 작업 탭**에서 수행한다. 새 Chrome/창/탭/프로필/CDP 세션을 만들어 대체하거나 다시 로그인하지 않는다. 새 대화가 필요해도 그 기존 탭 안에서 시작한다. 이미 생성된 응답이나 JSON이 보이면 같은 탭에서 확인·다운로드하고 중복 전송하지 않는다. extension 연결이나 탭 claim이 실패하면 정확한 오류·확인 범위를 문서화하고 멈춘다.

필수 순서는 `setupBrowserRuntime()` → 반환 Agent를 `globalThis.agent`에 보존 → `agent.browsers.get("extension")` → `browser.user.openTabs()` → 정확한 기존 descriptor `browser.user.claimTab()` → **반환된 동일 tab 객체 사용**이다. 입력·첨부·전송 직전에는 같은 탭의 URL/대화, 로그인, 실제 Pro 모드, 초안/파일 상태를 다시 확인한다. 상세 절차는 [BrowserUse handoff](browseruse_existing_session_handoff.md) 첫머리에 있다.

### P81 재개 시 실제 결과

- 사용자가 앞서 실제 전송을 허용했지만, 2026-09-24 14:23 KST same-job 재개는 기존 로그인 탭의 **read-only browser preflight** 단계에서 멈췄다. 같은 BrowserUse `extension` 사용자 탭은 ChatGPT home, 로그인 prompt 없음, composer 1개, 실제 `Pro` 선택 상태였다. 새 window/tab/profile/CDP, 재로그인, navigation은 없었다.
- 정확한 오류:
...
- 정지 지점은 `first_visible()`의 selector 조회다. prompt 입력, file upload/download, submit, response capture 전에 실패했으며, 같은 기존 탭은 home/빈 composer 상태로 보존됐다. `tab.dev.logs()`는 0건. `tab.capabilities.get("cdp")`의 정확한 결과는 `Capability is not available: cdp`이며 이를 정책 거부로 분류하지 않는다.
- 중앙 SQLite는 mode=ro 및 `PRAGMA query_only=ON`으로 확인했다. 동일 C15 R6 job `PROJOB-df15a37c58ae7583924e58c0`, status `USER_ATTENTION_REQUIRED`, version 14, 기존 packet hash 유지, approval/browser/conversation 결박 없음, submit/capture `0/0`, successor 없음이다. 최신 이벤트는 `READ_ONLY_BROWSER_PREFLIGHT` / `safe_unprepared_resume=true`다. 새 job을 만들거나 기존 job에 전송하지 않았다.
- 실패한 실행에서 새 source query/fetch, 점수/Stage 변경은 모두 0이다. 브라우저 작업도 navigation, prompt input, file operation, submit, capture 모두 0이다.

### 원인 범위와 local patch

P79는 read-only `evaluate`에 bounded timeout을 전달했지만, 설치된 BrowserUse API의 `locator.count()`, `isVisible()`, `isEnabled()` 호출에는 같은 timeout option이 노출되지 않는다. E2R selector helper 및 attachment lookup이 이 raw method들을 직접 부르므로 3초 selector deadline이 남아 있을 수 있다. 이번 timeout을 만든 단일 API call은 따로 계측되지 않아 원인을 그보다 좁게 단정하지 않는다.

P81 local diff는 count/visibility/enabled 및 text/attribute/value를 reviewed bounded `evaluate/evaluateAll` read-only 경로에 모으고 기본 10초 deadline과 명시적 timeout 전달을 적용한다. Attachment lookup도 같은 dispatcher를 사용한다. 실제 action인 click/fill/press/submit은 이 read-only 경로로 우회시키지 않았다.
...
- 사용자는 인증이 필요한 BrowserUse 작업을 사용자가 이미 로그인해 둔 바로 그 세션에서 수행하라고 재확인했다. 다음 UI 작업은 `extension → openTabs() → 정확한 기존 대상 descriptor claimTab() → claim 반환 동일 tab 객체` 절차만 사용한다.
- 새 브라우저·창·탭·프로필·별도 CDP, 재로그인, 다른 대화에서 재전송으로 우회하지 않는다. 기존 탭을 연결/확인하지 못하면 거기서 중단하고 실제 오류와 확인 범위만 기록한다. 새 Chat이 필요해도 같은 로그인 탭 안에서 시작한다.
- P93은 상태 문서와 read-only runtime DB 점검만 했다. 브라우저 연결/조작, 입력·첨부·다운로드·submit/capture, 새 query/fetch/job/pass, 점수/Stage 변경은 0이다. 실제 세션 상태는 다음 인증 작업 직전에 다시 확인해야 한다.

### exact-head Actions 상태

2026-09-24 20:31 KST, PR #7은 `OPEN/DRAFT`, head `100346caf0a31886e9aff6d1a0ef65e22e6820b6`, `mergeStateStatus=UNSTABLE`이다. exact-head run은 아직 전부 완료되지 않았다.

| Workflow | Run | 당시 상태 | 진행 단계 |
|---|---:|---|---|
| Pro-first push | [35992521406](https://github.com/Daikisong/stock_agent/actions/runs/35992521406) | `in_progress` | core-unit, browser-mock-e2e, static-security 성공; full regression 실행 중 |
| Pro-first PR | [35992525796](https://github.com/Daikisong/stock_agent/actions/runs/35992525796) | `in_progress` | full regression, browser-mock-e2e, core-unit jobs 실행 중 |
| V6 verification | [35992525826](https://github.com/Daikisong/stock_agent/actions/runs/35992525826) | `in_progress` | 전체 unit-test suite 실행 중 |

이들은 exact P93 문서 시점의 상태이며 `SUCCESS`로 표현하지 않는다. live BrowserUse 작업 재개 또는 goal 완료 전에는 이 세 run의 결론을 확인해야 한다.

### C06 / 000660 receipt authority/status 정합성 미확인 — 최신 판정 보류

2026-09-24 20:25 KST에 중앙 runtime DB를 `mode=ro`와 `PRAGMA query_only=ON`으로 조회했다. job `PROJOB-287556cc59c10f124d615c4d`는 `FINAL`, pass 19개지만 receipt 계층별 authority/status의 연결 관계가 문서만으로 설명되지 않는다.

- 19개 pass row는 모두 `score_valid=0`, `publication_withheld=1`; 마지막 `SATURATION_AUDIT`는 `COMPLETE`인데 detail은 `research_status=RESEARCH_RUNNING`.
- score receipt 상위 수준은 `score_valid=true`, `production_score_authority=true`, score `23.275`, nested scorer `full_score_valid=true`로 기록한다.
- 그 receipt 안 `full_thesis_eligibility`는 `score_valid=false`, `score_authority=false`, `stage_authority=false`, `FULL_THESIS_SCORE_PENDING`, `WITHHELD_UNTIL_DETERMINISTIC_STAGECOURT`를 기록한다.
- StageCourt receipt는 `decision_status=FINAL`, canonical stage `0`이며 `production_score_authority=false`, `production_stage_authority=true`라고 기록한다.

이 필드가 서로 다른 authority 계층을 뜻할 수 있으므로 이것만으로 코드 결함이라고 단정하지 않는다. 다만 row-level `FINAL`, standalone `score_valid=true`, StageCourt `FINAL`만으로는 full-thesis canary PASS가 입증되지 않는다. 코드/receipt 생성 순서에서 각 authority와 terminal status의 대응을 확인할 때까지 C06은 **미증명/판정 보류**다. DB를 수정하거나 점수·Stage를 변경하지 않았다. 앞선 P92 snapshot의 C06 라벨보다 본 P93 판정이 우선한다.

### 다음 한 단계

1. head `100346ca`의 Pro push, Pro PR, V6 workflows 완료와 결론 확인.
...
## P108 — exact composer tile 복구 흐름 구현 및 검증 경계 기록 (2026-09-25 07:42 KST, P109에서 당시 상태 기록을 supersede)
...
## P109 — 기존 로그인 세션 지시 및 exact-head CI 인계 갱신 (2026-09-25 07:49 KST)

### 사용자 지시

사용자는 BrowserUse가 필요한 로그인 작업은 사용자가 이미 로그인해 둔 **그 세션**에서 하라고 재확인했다. 작업 직전에 `extension → browser.user.openTabs() → 정확한 기존 작업 탭 확인 → claimTab() → claim 반환 동일 Tab 객체` 순서로 진행한다. 새 Chrome/창/탭/프로필/CDP/relogin으로 옮기지 않는다. 새 대화가 필요한 경우에도 기존 로그인 탭 안에서만 시작한다. 연결 또는 대상 확인이 안 되면 입력·첨부·다운로드·전송 전에 멈추고 실제 오류/확인 범위만 남긴다.

### 현재 GitHub/검증 상태

- PR #7은 [OPEN/DRAFT](https://github.com/Daikisong/stock_agent/pull/7), head `617e204b4c0b4f9dce15f4185eff589af9042920`, `UNSTABLE`이다. 이 확인 시점에 local branch, origin feature branch, PR head가 같았고 worktree는 clean이었다. main에는 병합되지 않았다.
- 해당 head의 [Pro PR run 36069234793](https://github.com/Daikisong/stock_agent/actions/runs/36069234793), [Pro push run 36069233229](https://github.com/Daikisong/stock_agent/actions/runs/36069233229), [V6 run 36069234853](https://github.com/Daikisong/stock_agent/actions/runs/36069234853)은 모두 `IN_PROGRESS`다. Pro PR/push의 `static-security`는 `SUCCESS`; `full-regression`, `core-unit`, `browser-mock-e2e`, V6 `offline-contract`는 진행 중이다. 전체 테스트 결론은 아직 없다.
- 이번 P109는 진행 문서와 handoff를 고치기 위해 Git/PR/Actions를 읽기 전용으로 확인했다. BrowserUse 연결/탭 열거/claim, C15 DB 조회, UI 조작은 하지 않았다. 입력·첨부·다운로드·전송·capture, query/fetch, 새 job/pass, 다른 archetype, 점수/Stage 변경은 0.
- C15의 마지막 기록 증거는 P107 read-only receipt 기준 `PROJOB-df15a37c58ae7583924e58c0`, `USER_ATTENTION_REQUIRED` v26, submit/capture `0/0`, canonical packet hash `fa5845a055661c99c2ab1eb9cfb65f66fb84d2c85b267b3cde33b54843c320df`, approval/browser/conversation binding 없음이다. P109에서 재조회하지 않았으므로 live 재개 전 최신 durable state를 다시 읽는다.
- 전체 master goal은 미완료다. 기록상 P9 live full-thesis Pro canary `1/3`: C06만 완료, C17/C28 미완료. PR #7은 draft/open 유지하고 main에 병합하지 않는다.
