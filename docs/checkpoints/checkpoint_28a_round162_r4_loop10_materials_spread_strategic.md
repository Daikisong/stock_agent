# Checkpoint 28A Round 162 R4 Loop 10 Materials / Spread / Strategic Resources

## 목적

`docs/round/round_162.md`의 R4 Loop 10 내용을 별도 calibration pack으로 반영했다.
...
- `Mosaic phosphate`처럼 제품 가격환경이 좋아도 sulfuric acid, sulfur, ammonia, urea 비용이 더 세면 4C-watch로 봐야 한다.

## 반영 파일

- `src/e2r/sector/round162_r4_loop10_materials_spread_strategic.py`
- `src/e2r/cli/build_round162_r4_loop10_report.py`
- `tests/test_round162_r4_loop10_materials_spread_strategic.py`
- `data/e2r_case_library/cases_r4_loop10_round162.jsonl`
- `data/sector_taxonomy/score_weight_profiles_round162_r4_loop10_v10.csv`
- `output/e2r_round162_r4_loop10_materials_spread_strategic/`
...
- 4C/RedTeam 후보: `Korea Zinc share issuance/probe`, `LG Chem/Lotte Chemical oversupply`, `Lithium -86%`, `Indonesia HPAL sulfur input-cost squeeze`, `Mosaic phosphate sulfuric-acid input-cost squeeze`
...
- price floor, offtake, 생산 ramp-up, realized price, AISC, cash cost, FCF, 자본환원, dilution, governance event, processing/fertilizer input cost를 실제 field로 확인해야 한다.
...
```bash
PYTHONPATH=src python -m unittest tests/test_round162_r4_loop10_materials_spread_strategic.py -v
PYTHONPATH=src python -m e2r.cli.build_round162_r4_loop10_report
```

결과:

- Round 162 전용 테스트 12개 통과
- canonical target 26개 생성. 그중 round_162 원문 target 22개와 helper/adjacent overlay 4개를 분리 기록
- case candidate 20개 생성. `FERTILIZER_INPUT_COST_SULFURIC_ACID` / Mosaic 4C-watch 케이스 추가
