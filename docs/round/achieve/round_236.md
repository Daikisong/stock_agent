순서상 이번은 **R6 Loop 10 — 금융·자본배분·디지털금융 가격경로 검증 라운드**다.

이번 R6는 **은행 밸류업, 증권주 거래대금 cycle, 지주 NAV discount, 보험 NAV capital release, 디지털자산 지분투자, crypto platform M&A, 인터넷은행 governance, stablecoin policy overheat**를 같이 본다.
...
이번 환경에서는 KRX/Naver/Yahoo/Stooq 원시 수정주가 일봉 OHLC를 안정적으로 직접 확보하지 못했다. 대신 Reuters / FT / public company profile에 남은 **이벤트 수익률, 섹터 수익률, 거래금액, 지분율, 순이익, buyback/cancel, 법적 리스크 지표**로 계산 가능한 값만 계산했다.
...
R6의 Stage 3는 “저PBR이다”, “밸류업이다”, “스테이블코인이다”가 아니다. **ROE·자본비율·credit cost·실제 소각·배당·규제수익·지분법 이익이 PBR 프레임을 바꾸는 순간**이다.
...
- buyback cancellation / payout policy
- PBR 1x breakthrough
...
- capital-release / regulatory share sale
- K-ICS / CSM / shareholder return

디지털자산:
- Hana Bank / Dunamu
- NAVER Financial / Dunamu
- Upbit market share
- exchange trust / abnormal withdrawal

인터넷은행:
- K Bank IPO
- KakaoBank governance risk
- founder legal risk / major shareholder suitability
...
## Case A — KB금융 중심 은행 value-up basket `success_candidate`
...
Stage 2:
2025~2026
- Big 4 financial groups 순이익 nearly 18조 원
- KB Financial 2025 net profit 5.84조 원, +15.1%
- financial groups index rally on KOSPI bull market
- buyback / dividend / PBR rerating 기대

Stage 3:
보류
- ROE, CET1, credit cost, 반복 소각, 배당성향 확인 필요
- 저PBR과 value-up 기대만으로 Green 금지

Stage 4B:
PBR rerating이 실제 ROE/소각보다 먼저 가격에 반영되면 후보

Stage 4C:
PF credit cost, CET1 약화, 주주환원 후퇴, 세제 shock, 경기둔화 credit shock 시 후보
```

KB금융·신한·하나·우리 같은 대형 금융지주는 R6에서 좋은 Stage 2 후보지만, 아직 “저PBR이라 오른다”만으로 Stage 3를 줄 수는 없다. 공개 회사정보 기준 2025년 국내 Big 4 금융지주 합산 순이익은 nearly 18조 원이고, KB금융은 순이익 5.84조 원, +15.1%로 가장 강한 성장률을 보인 것으로 정리된다. 다만 이 숫자는 public profile 기반이라 `source_confidence = medium_low`로 둔다. 금융지주 자체의 Green은 ROE, CET1, credit cost, 반복적인 소각·배당 실행으로 확인해야 한다. ([위키백과][1])

### 실제 가격경로 검증

```text
price_data_source:
public company profile + Reuters sector rally anchor
...
source_confidence:
medium_low for company-level 2025 net-profit figures

financial_groups_event_MFE_1D:
+4.2% on 2026-05-06 KOSPI 7,000 event

KOSPI_same_context:
+6.45%

financial_groups_relative_underperformance:
4.2 - 6.45
= -2.25pp
...
reason:
- Reuters source gives sector move, not each bank’s raw OHLC.
...
```text
symbols = securities basket / Mirae Asset / NH / Samsung Securities / Kiwoom-like exposure
case_type = cyclical_success + 4B-watch
archetype = SECURITIES_CAPITAL_MARKET_BOOM
```
...
- foreign net purchase 3.1조 원

Stage 3:
없음
- 증권주 rally는 market-cycle revenue Stage 2
- brokerage revenue, IB revenue, ROE, risk controls 확인 전 Green 금지

Stage 4B:
2026-05-06
- securities basket +13.5%
- 시장 호황 기대가 실적 확인보다 먼저 가격에 반영
```

2026년 5월 6일 KOSPI는 7,000을 넘었고, 종가 기준 6.45% 상승했다. 같은 날 증권업종은 13.5%, 금융지주는 4.2% 올랐으며, 외국인은 3.1조 원을 순매수했다. 이건 증권주에 좋은 Stage 2/cyclical_success지만, Green은 아니다. 거래대금 호황이 위탁매매·IB·운용손익·ROE로 실제 반영되는지 확인해야 한다. ([Reuters][2])
...
reason:
- Reuters gives sector event return, but not each securities firm’s OHLC path.
...
- 반복 소각
- discount narrowing
- SK Hynix NAV 상승이 주주환원으로 연결되는지 확인 필요
...
SK스퀘어는 R6에서 가장 깔끔한 holding NAV discount Stage 2 후보 중 하나다. 회사는 2024년 11월 1,000억 원 자사주 소각과 추가 1,000억 원 매입·소각 계획을 발표했고, SK하이닉스 지분 20%를 보유하고 있다. Reuters는 SK스퀘어의 시장가치가 보유 중인 SK하이닉스 지분가치 180억 달러의 절반보다 낮다고 보도했다. ([Reuters][3])
...
reason:
- Reuters confirms capital-return/NAV evidence but not SK Square raw OHLC path.
- KRX/Naver/Yahoo/Stooq 원시 수정주가 일봉 OHLC 직접 확보 실패.
```
...
삼성생명은 2026년 3월 삼성전자 지분 1.3조 원어치를 매각한다고 공시했다. 회사는 금융회사 지배구조 관련 규제 리스크 해소를 이유로 들었다. 이건 보험 NAV capital-release Stage 2 후보지만, 매각대금이 실제 주주환원·자본정책·K-ICS 개선으로 이어지는지 확인하기 전에는 Stage 3가 아니다. ([Reuters][4])
...
reason:
- Reuters gives transaction/regulatory anchor but not Samsung Life raw OHLC path.
- KRX/Naver/Yahoo/Stooq raw OHLC unavailable in this pass.
```
...
## Case E — 하나은행 / Dunamu `success_candidate / regulated digital-asset equity option`
...
- transaction value 1조 원 / about $700M
...
하나은행은 Dunamu 지분 6.55%를 1조 원에 인수하기로 했다. Dunamu는 Upbit을 운영하고, Upbit은 국내 가상자산 거래량 80% 이상을 처리하는 거래소로 보도됐다. 이건 R6에서 강한 Stage 2다. 하지만 은행 Green은 지분법 이익, 규제수익, 자본비율 영향, 디지털 송금·stablecoin revenue로 확인되어야 한다. ([Reuters][5])

### 실제 가격경로 검증

```text
price_data_source:
Reuters transaction anchor
...
reason:
- Reuters gives transaction metrics but not Hana Financial raw OHLC.
- KRX/Naver/Yahoo/Stooq raw OHLC unavailable in this pass.
```
...
NAVER Financial은 Dunamu를 15.13조 원 규모 all-stock deal로 인수하기로 했다. Upbit은 국내 crypto exchange 시장 약 70%를 차지하는 플랫폼으로 묘사됐다. 하지만 인수 뉴스 직후 NAVER 주가가 7% 넘게 올랐다가, Upbit에서 540억 원 규모 abnormal withdrawal이 발생했다는 보도 후 -4.2%로 전환했다. 이건 R6에서 “digital-asset platform Stage 2 + exchange-trust 4C-watch”가 동시에 뜨는 케이스다. ([Reuters][6])
...
K Bank는 인터넷은행 profitability Stage 2 후보로 볼 수 있다. IPO 계획은 최대 9,840억 원 조달, 공모가 9,500~12,000원, valuation 최대 5조 원으로 제시됐고, 2024년 상반기 영업이익은 867억 원으로 전년 대비 3배 이상 증가했다. 다만 상장 전이므로 가격경로 검증은 불가능하고, Stage 3는 상장 이후 ROE/NIM/credit cost로 봐야 한다. ([Reuters][7])

반대로 KakaoBank 쪽은 성장성보다 governance RedTeam이 먼저다. Kakao 창업자 김범수는 SM Entertainment 인수 과정의 주가조작 혐의로 구속됐고, Reuters는 Kakao 주가가 3.4% 하락했으며, 연초 대비 24% 하락한 상태였다고 보도했다. 유죄 확정 시 금융범죄자의 은행 지분 10% 초과 보유 제한 때문에 KakaoBank 지배력 문제가 생길 수 있다는 점이 핵심 4C-watch다. ([Reuters][8])
...
## Case H — Kakao Pay / stablecoin basket `overheat / price_moved_without_evidence`
...
원화 stablecoin basket은 R6의 대표적 price_moved_without_evidence다. FT는 Kakao Pay가 한 달 동안 2배 이상, LG CNS가 약 70%, Aton이 80%, ME2ON이 3배 상승했다고 보도했다. 하지만 같은 시기 규제체계는 확정되지 않았고, Bank of Korea는 비은행 stablecoin 발행이 통화정책과 자본흐름에 혼란을 줄 수 있다고 우려했다. ([Financial Times][9])
...
| ------------------ | --------------------- | ------------------------------------------------------------------: | -------------- |
| KB 중심 은행 value-up  | success_candidate     |        Big4 NP nearly 18T; KB 5.84T, +15.1%; financial groups +4.2% | Stage 2        |
| 증권주 basket         | cyclical_success + 4B |                   KOSPI +6.45%, securities +13.5%, foreign buy 3.1T | cyclical       |
| SK스퀘어              | success_candidate     | 200B won buyback/cancel programme, SKH stake 20%, NAV discount >50% | NAV value-up   |
| 삼성생명               | success_candidate     |                             Samsung Electronics stake sale 1.3T won | insurance NAV  |
| 하나은행 / Dunamu      | success_candidate     |           1T won for 6.55%, implied Dunamu value 15.27T, Upbit >80% | digital equity |
| NAVER / Dunamu     | event + 4C-watch      |                   15.13T deal, +7% → -4.2%, abnormal withdrawal 54B | trust watch    |
| K Bank / KakaoBank | success + 4C-watch    |     K Bank IPO max 984B; Kakao -3.4%, YTD -24%, bank ownership risk | mixed          |
...
- PF credit cost / CET1 risk for banks
...
SK스퀘어는 실제 소각과 추가 매입·소각 계획이 있고, 하나은행은 1조 원을 들여 Dunamu 지분을 취득해 regulated digital-asset equity option을 만들었다. 삼성생명은 삼성전자 지분 매각으로 capital release 후보가 됐다. 하지만 모두 Stage 3는 **ROE·자본비율·반복 주주환원·규제수익·지분법 이익**이 확인될 때다.
...
privacy_or_exchange_trust_break -5
...
8. privacy / exchange trust / governance hard risk 없음
...
이번 R6 Loop 10에서는 hard 4C를 확정하지 않는다. 다만 **Kakao/KakaoBank legal risk**, **Upbit abnormal withdrawal**, **stablecoin non-bank issuer uncertainty**는 강한 4C-watch다.
...
## docs/round/round_164.md 요약

```md
# R6 Loop 10. Financial / Capital Allocation / Digital Finance Price Validation

이번 라운드는 R6 Loop 10 price-validation 라운드다.

핵심 결론:
- Bank value-up is Stage 2, not Green. KB-led financial groups have profit and low-PBR rerating evidence, but Stage 3 requires ROE, CET1, credit cost and repeated shareholder return execution.
- Securities basket is cyclical_success and 4B-watch. On 2026-05-06, KOSPI rose +6.45%, securities firms +13.5%, financial groups +4.2%, with foreign net purchase of 3.1T won.
...
```md
# Checkpoint 28A Round 164 R6 Loop 10 Financial Capital Digital Price Validation

## 반영 내용
- R6 Loop 10 price-validation 라운드를 추가했다.
...
- Reuters/FT/public-company profile anchors로 가능한 MFE/MAE 및 transaction/valuation/policy metrics를 계산했다.
...
R6는 Stage 2 후보가 많지만, **저PBR·밸류업·디지털자산 기대를 너무 쉽게 Green으로 올리면 바로 망가진다.**

```text
1. KB 중심 은행주는 value-up Stage 2 후보가 될 수 있다.
...
3. SK스퀘어와 삼성생명은 NAV discount / capital-release 후보지만,
   NAV가 주주환원·capital release로 연결되는지 확인해야 Stage 3다.

4. 하나은행의 Dunamu 지분 인수는 강한 Stage 2다.
...
> **R6는 디지털자산/스테이블코인 테마가 가격을 너무 빨리 앞질러 가므로, event premium과 governance/exchange-trust 4C gate를 강하게 둬야 한다.**
