...
fine_archetype_id: DUKOVANY_FINAL_CONTRACT_NO_BACKFILL_HOLDOUT_V116_PREFERRED_BIDDER_SUPPLIER_THEME_SERVICE_OM_SPLIT
selection_basis:
...
web_raw_fetch_status_this_turn:
  ...
  uncached_symbol_shards:
    ...
    - 083650/2024: cache_miss_observed
    - 105840/2024: cache_miss_observed
...
This file continues the local C04 sequence after `R1/C04 loop 115`; selected loop is therefore `116`.

This is a **dedupe-aware holdout validation / cache-miss TODO** MD. It does not claim fresh independent stock-web evidence because additional 2024/2025 nuclear candidate shards returned cache miss in this execution. The trigger rows below reuse current-session stock-web-derived C04 rows that already contain complete 30D/90D/180D MFE and MAE from `Songdaiki/stock-web` tradable OHLC. Exact duplicate `same_entry_group_id` rows should be deduped during batch ingest. No production scoring is changed.
...
The trigger-family distinction is therefore mandatory:
...
reason:
  ...
  - uncached additional 2024/2025 C04 candidate shards returned cache miss in this execution
...
The preferred-bidder event had real project potential, but the accounting bridge was not final. High MAE confirms it should not be Green.
...
```text
C04_FINAL_CONTRACT_NO_BACKFILL_REQUIREMENT_V116_HELD_OUT
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---:|
| L1_INDUSTRIALS_INFRA_DEFENSE_GRID | C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY | DUKOVANY_FINAL_CONTRACT_NO_BACKFILL_HOLDOUT_V116 | 1 | 3 | 2 | 1 | 0 | 4 | 4 | 0 | 3 | false | false | 24 |
...
no_new_signal_reason: all rows reused holdout validation after additional C04 candidate shards returned cache miss
loop_contribution_label: duplicate_low_value_loop_with_future_trigger_todo
do_not_propose_new_weight_delta: true
```

```jsonl
{"row_type":"residual_contribution","round":"R1","loop":"116","large_sector_id":"L1_INDUSTRIALS_INFRA_DEFENSE_GRID","canonical_archetype_id":"C04_NUCLEAR_POLICY_PROJECT_LEGAL_DELAY","new_independent_case_count":0,"reused_case_count":4,"new_symbol_count":0,"new_trigger_family_count":0,"narrative_only_future_trigger_count":2,"tested_existing_calibrated_axes":["stage2_required_bridge","local_4b_watch_guard","hard_4c_thesis_break_routes_to_4c","no_backfill_later_evidence"],"residual_error_types_found":["preferred_bidder_without_final_contract","direct_scope_without_legal_cash_bridge","supplier_theme_price_only_spike","final_contract_backfill_risk"],"loop_contribution_label":"duplicate_low_value_loop_with_future_trigger_todo","do_not_propose_new_weight_delta":true}
```
...
```text
Do not patch production scoring from this MD alone. Treat this R1/C04 loop 116 as holdout validation only. Batch it with C04 loops 113~115 and R13 accounting-trust / Stage2 false-positive / high-MAE / 4B-4C files. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C04 final-contract no-backfill gate and supplier-theme hard 4C guard. When stock-web shards are accessible, compute separate 30D/90D/180D windows from 2025-06-04 for the Dukovany final-contract event; do not backfill that later evidence into 2024 preferred-bidder triggers. Also reprice additional C04 candidates such as 비에이치아이(083650) and 우진(105840) before counting them as new independent evidence.
```
