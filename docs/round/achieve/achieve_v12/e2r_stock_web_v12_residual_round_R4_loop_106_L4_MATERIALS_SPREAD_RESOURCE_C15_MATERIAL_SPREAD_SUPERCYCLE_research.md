...
fine_archetype_id: MATERIAL_LABEL_TO_ASP_MARGIN_FCF_HOLDOUT_VALIDATION_VS_FOIL_ALUMINIUM_CHEMICAL_PRICE_SPIKE_DECAY
selection_basis:
...
loop_objective:
  - holdout_validation
  - material_label_to_margin_cash_bridge_gate
...
`C15_MATERIAL_SPREAD_SUPERCYCLE` remains Priority 0 in the latest no-repeat index: 6 representative rows and 24 rows short of the 30-row minimum. The v12 scheduler maps C15~C17 to `R4 / L4_MATERIALS_SPREAD_RESOURCE`.

This file continues the local C15 sequence after `R4/C15 loop 105`; selected loop is therefore `106`.

This is a **holdout validation / rule-consolidation** MD. It does not claim fresh independent price discovery in this turn. The trigger rows below reuse current-session stock-web-derived C15/C16/C17 rows that already contain complete 30D/90D/180D MFE and MAE from `Songdaiki/stock-web` tradable OHLC. Exact duplicate trigger keys should be deduped during batch ingest. No production scoring is changed.

---

## 1. Research thesis

C15 is not `material stock + commodity story`.

It is the material spread conversion bridge:

```text
metal / chemical / battery foil / silicone / copper / aluminium label
→ ASP, volume, utilization, inventory gain, margin, FCF, revision
→ price path validation
```

The model’s repeated failure mode is to pay for the word `material` before the company converts the raw input into margin or cash.
...
dual-use metal label
≠ FCF unless order and margin refresh
```

This loop validates five C15 routes:
...
reused_price_rows_from_current_session:
  ...
  - R13 high-MAE / accounting-trust / Stage2 false-positive / 4B-4C material guardrails
reason:
  - all reused rows were already calculated from Songdaiki/stock-web tradable raw 1D OHLC
  - this file validates the C15 material-label-to-margin-cash bridge gate
...
### 5.1 KCC / 002380 — material margin positive control

KCC remains the C15 control. The material thesis reached company-level margin conversion.
...
### 5.2 Sam-A Aluminium / 006110 — high-MFE foil label local 4B

The battery-foil label had price response, but it did not sustain without utilization and margin refresh.
...
### 5.3 Choil Aluminium / 018470 — aluminium beta hard 4C

This is a clean commodity-label false positive.
...
### 5.4 Hyosung Chemical / 298000 — petrochemical label hard 4C

The petrochemical/PP label failed to become product-spread repair or balance-sheet relief.
...
### 5.5 Poongsan / 103140 — processing bridge local 4B

Poongsan keeps the model from over-blocking real processing bridges, but high MAE requires order and margin refresh.
...
| 006110 | foil local 4B | +28.34 / -47.55 | +28.34 / -53.58 | label needs utilization/margin refresh |
...
The C15 gate held in holdout form:

```text
company-specific material margin bridge -> keep Stage2
material label with high MFE but severe MAE -> local 4B until refresh
commodity beta with low MFE / deep MAE -> hard 4C
chemical label without spread repair -> hard 4C
processing bridge with high MAE -> local 4B
```

### Rule candidate retained, not newly proposed

```text
C15_MATERIAL_LABEL_TO_ASP_MARGIN_FCF_BRIDGE_REQUIREMENT_V106_HELD_OUT
...
one_line_summary: "This loop adds 0 new independent cases and validates the C15 material-label-to-margin-cash bridge gate across margin positives, local 4B material spikes, and hard 4C commodity/chemical labels."
```
...
```text
Do not patch production scoring from this MD alone. Treat this R4/C15 loop 106 as holdout validation only. Batch it with C15 loops 101~105, C16/C17 adjacent material rows, and R13 high-MAE/accounting-trust guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C15 material-label-to-ASP-margin-FCF gate, but do not create a new weight delta from this loop because no new independent case was added.
```
