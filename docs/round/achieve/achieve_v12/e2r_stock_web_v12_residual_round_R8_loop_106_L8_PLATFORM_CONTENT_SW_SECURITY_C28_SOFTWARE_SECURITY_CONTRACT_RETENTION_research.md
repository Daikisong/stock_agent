...
fine_archetype_id: CONTRACT_RETENTION_ARR_RENEWAL_MARGIN_HOLDOUT_V106_SECURITY_SAAS_AI_THEME_RECLASSIFICATION_SPLIT
selection_basis:
...
web_raw_fetch_status_this_turn:
  ...
  direct_stock_web_shards:
    - 012510/2024: cache_miss_observed_this_turn
    - 018260/2024: cache_miss_or_not_recomputed_this_turn
    - 053800/2024: cache_miss_or_not_recomputed_this_turn
    - 030520/2024: cache_miss_or_not_recomputed_this_turn
    - 047560/2024: cache_miss_or_not_recomputed_this_turn
    - 150900/2024: cache_miss_or_not_recomputed_this_turn
    - 263860/2024: cache_miss_or_not_recomputed_this_turn
    - 042510/2024: cache_miss_or_not_recomputed_this_turn
...
This file follows the local R8 software/platform sequence after `R8/C26 loop 105`; because no current-session C28 file has been produced, selected loop is set to `106`.

This is a **dedupe-aware holdout validation / source-proxy TODO** MD. Direct fresh stock-web shard recomputation for the C28 symbols was unavailable in this execution, so every trigger row is marked `source_proxy_only=true`, `batch_reverification_required=true`, and `independent_evidence_weight=0.0`. The rows include full 30D/90D/180D MFE/MAE fields for schema compatibility, but they should be reverified against `Songdaiki/stock-web` before any weight change. No production scoring is changed.
...
row_provenance:
  ...
  reason:
    - direct fresh C28 symbol-year shards were unavailable or cache-missed in this execution
...
```text
C28_CONTRACT_RETENTION_ARR_RENEWAL_MARGIN_GATE_V106_HELD_OUT
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---:|
| L8_PLATFORM_CONTENT_SW_SECURITY | C28_SOFTWARE_SECURITY_CONTRACT_RETENTION | CONTRACT_RETENTION_ARR_RENEWAL_MARGIN_HOLDOUT_V106 | 2 | 6 | 2 | 2 | 0 | 8 | 8 | 0 | 6 | false | false | 12 |
...
no_new_signal_reason: all rows are source_proxy_only because direct fresh C28 stock-web candidate shards were unavailable or cache-missed
loop_contribution_label: duplicate_low_value_loop_with_cache_miss_todo
do_not_propose_new_weight_delta: true
```

```jsonl
{"row_type":"residual_contribution","round":"R8","loop":"106","large_sector_id":"L8_PLATFORM_CONTENT_SW_SECURITY","canonical_archetype_id":"C28_SOFTWARE_SECURITY_CONTRACT_RETENTION","new_independent_case_count":0,"reused_case_count":8,"new_symbol_count":0,"new_trigger_family_count":0,"narrative_only_future_todo_count":1,"source_proxy_only_count":8,"batch_reverification_required_count":8,"tested_existing_calibrated_axes":["stage2_required_bridge","local_4b_watch_guard","hard_4c_thesis_break_routes_to_4c","C28_contract_retention_ARR_renewal_margin_gate","dominant_driver_reclassification_guard"],"residual_error_types_found":["AI_software_theme_without_retained_revenue","security_label_without_renewal_metrics","enterprise_IT_watch_not_green","digital_ID_security_policy_label_false_positive"],"loop_contribution_label":"duplicate_low_value_loop_with_cache_miss_todo","do_not_propose_new_weight_delta":true}
```
...
```text
Do not patch production scoring from this MD alone. Treat this R8/C28 loop 106 as holdout validation only. Batch it with existing C28 rows and R13 accounting-trust / Stage2 false-positive / high-MAE / 4B-4C guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C28 contract-retention/ARR/renewal/margin gate and AI-theme reclassification guard, but do not create a new weight delta from this loop because every trigger row is source_proxy_only and requires batch reverification. Future research should reprice 더존비즈온(012510), 삼성에스디에스(018260), 안랩(053800), 지니언스(263860), 한글과컴퓨터(030520), 이스트소프트(047560), 파수(150900), 라온시큐어(042510), 알서포트(131370), 민앤지(214180), 브레인즈컴퍼니(099390) when stock-web shards are accessible.
```
