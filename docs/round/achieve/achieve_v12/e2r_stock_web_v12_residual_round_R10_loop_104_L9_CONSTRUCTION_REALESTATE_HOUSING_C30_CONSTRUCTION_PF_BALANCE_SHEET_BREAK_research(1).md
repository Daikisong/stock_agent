...
fine_archetype_id: PF_REFINANCING_ISSUER_CASH_BRIDGE_HOLDOUT_V104_DEDUPED_LOW_VALUE_LOOP
selection_basis:
...
web_raw_fetch_status_this_turn:
  ...
  uncached_symbol_shards:
    ...
    - 375500: cache_miss_observed
    - 013580: cache_miss_observed
    - 005960: cache_miss_observed
...
`C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK` remains Priority 0 in the no-repeat index. It maps to `R10 / L9_CONSTRUCTION_REALESTATE_HOUSING` under the v12 scheduler.

This file continues the local C30 sequence after `R10/C30 loop 103`; selected loop is therefore `104`.

This is a **holdout validation / dedupe-aware low-value loop**. It does not claim new independent evidence because additional construction-symbol shards returned cache miss during this turn. The trigger rows below reuse current-session stock-web-derived C30/C31/R13 construction rows that already contain complete 30D/90D/180D MFE and MAE from `Songdaiki/stock-web` tradable OHLC. Exact duplicate `same_entry_group_id` rows should be deduped during batch ingest. No production scoring is changed.

---

## 1. Research thesis

C30 should not reward a generic PF policy headline.

C30 should reward an issuer-specific balance-sheet bridge:

```text
PF stress / liquidity support / construction rebound / low-PBR builder label
→ refinancing, maturity rollover, guarantee relief, presale improvement, debt-service capacity, receivables quality, cash conversion
...
+ builder/low-PBR label
+ short local rebound
```

This can create MFE, but the company-specific balance sheet may still leak. A construction name is not fixed when the headline arrives; it is fixed when refinancing and cash conversion reach the issuer.

This holdout loop validates six C30 routes:

1. **Large-builder survivor watch**
   - Low MAE means not a hard block.
   - Modest MFE and incomplete cash bridge mean not Green.

2. **Delayed rebound**
   - Later MFE can be real.
   - Do not backfill it into the original trigger date.

3. **Weak-liquidity PF-relief label**
   - Low MFE and deep MAE without issuer cash bridge should hard block.

4. **Price-only rebound overlay**
   - A local spike without non-price confirmation stays local 4B.

5. **Delayed housing/PF soft landing**
   - Can stay local 4B.
   - Requires timestamped issuer bridge before Actionable.

6. **Workout/restructuring stress-control**
   - Informative, but not clean equity aggregate.
...
reason:
  ...
  - uncached additional C30 candidate shards returned cache miss in this turn
...
Low drawdown but weak upside. Stable is not Green.
...
The later MFE must not be backfilled into the original weak trigger.
...
### 5.3 Kumho E&C / 002990 — weak-liquidity hard 4C

Policy did not reach the issuer cash bridge.
...
### 5.4 Daewoo E&C / 047040 — price-only 4B

Local spike but no non-price bridge.
...
### 5.5 HDC Hyundai Development / 294870 — delayed soft-landing 4B

Later validation is useful, but entry-date evidence is incomplete.
...
### 5.6 HL D&I / 014790 — delayed support 4B

Shallow drawdown and delayed MFE are useful, but need timestamped issuer bridge.
...
### 5.7 Taeyoung E&C / 009410 — workout stress-control only

Workout is not clean equity recovery.
...
```jsonl
{"row_type":"score_simulation","symbol":"000720","raw_policy_label":3,"raw_issuer_cash_bridge":2,"raw_debt_service_visibility":2,"raw_margin_bridge":1,"raw_price_validation":1,"raw_label_only_risk":2,"stage2_actionable_bonus_before":2.0,"stage2_actionable_bonus_after":1.0,"simulated_route":"Stage2Watch_Survivor"}
{"row_type":"score_simulation","symbol":"047040","raw_policy_label":3,"raw_issuer_cash_bridge":1,"raw_debt_service_visibility":1,"raw_margin_bridge":1,"raw_price_validation":1,"raw_label_only_risk":3,"stage2_actionable_bonus_before":2.0,"stage2_actionable_bonus_after":1.0,"simulated_route":"DelayedLocal4B_NoBackfill"}
{"row_type":"score_simulation","symbol":"002990","raw_policy_label":4,"raw_issuer_cash_bridge":0,"raw_debt_service_visibility":0,"raw_margin_bridge":0,"raw_price_validation":0,"raw_label_only_risk":5,"stage2_actionable_bonus_before":2.0,"stage2_actionable_bonus_after":0.0,"simulated_route":"Hard4C_WeakLiquidity"}
{"row_type":"score_simulation","symbol":"047040","raw_policy_label":2,"raw_issuer_cash_bridge":0,"raw_debt_service_visibility":0,"raw_margin_bridge":0,"raw_price_validation":1,"raw_label_only_risk":5,"stage2_actionable_bonus_before":1.0,"stage2_actionable_bonus_after":0.0,"simulated_route":"PriceOnlyLocal4B"}
{"row_type":"score_simulation","symbol":"294870","raw_policy_label":3,"raw_issuer_cash_bridge":2,"raw_debt_service_visibility":2,"raw_margin_bridge":1,"raw_price_validation":3,"raw_label_only_risk":2,"stage2_actionable_bonus_before":2.0,"stage2_actionable_bonus_after":1.0,"simulated_route":"DelayedLocal4B"}
{"row_type":"score_simulation","symbol":"014790","raw_policy_label":3,"raw_issuer_cash_bridge":2,"raw_debt_service_visibility":2,"raw_margin_bridge":1,"raw_price_validation":3,"raw_label_only_risk":2,"stage2_actionable_bonus_before":2.0,"stage2_actionable_bonus_after":1.0,"simulated_route":"DelayedLocal4B"}
{"row_type":"score_simulation","symbol":"009410","raw_policy_label":5,"raw_issuer_cash_bridge":3,"raw_debt_service_visibility":4,"raw_margin_bridge":0,"raw_price_validation":0,"raw_label_only_risk":3,"workout_contamination_risk":5,"stage2_actionable_bonus_before":2.0,"stage2_actionable_bonus_after":0.0,"simulated_route":"StressControlOnly"}
```
...
The C30 gate held again:

```text
low-MAE survivor -> Watch, not Green
delayed rebound -> local 4B, no backfill
weak-liquidity PF label -> hard 4C
price-only rebound -> local 4B only
workout -> stress-control, not clean aggregate
```

### Rule candidate retained, not newly proposed

```text
C30_PF_REFINANCING_TO_ISSUER_CASH_BRIDGE_REQUIREMENT_V104_HELD_OUT
...
one_line_summary: "This loop adds 0 new independent cases and validates the C30 PF/refinancing-to-issuer-cash bridge gate across survivor watch, delayed rebound, weak-liquidity hard 4C, price-only 4B, and workout stress-control rows."
```
...
```text
Do not patch production scoring from this MD alone. Treat this R10/C30 loop 104 as holdout validation only. Batch it with C30 loops 1~3 and 100~103 plus C31/R13 construction/PF guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C30 PF-refinancing-to-issuer-cash bridge gate, but do not create a new weight delta from this loop because uncached additional C30 symbol shards returned cache miss and no new independent case was added.
```
