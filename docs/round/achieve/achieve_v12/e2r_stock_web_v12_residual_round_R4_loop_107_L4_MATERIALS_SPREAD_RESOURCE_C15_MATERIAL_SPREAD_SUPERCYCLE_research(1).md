...
fine_archetype_id: MATERIAL_LABEL_TO_ASP_MARGIN_FCF_HOLDOUT_V107_FOIL_COPPER_ALUMINIUM_CHEMICAL_SPLIT
selection_basis:
...
This file continues the local C15 sequence after `R4/C15 loop 106`; selected loop is therefore `107`.
...
```jsonl
{"row_type":"narrative_only_future_trigger_todo","selected_round":"R4","selected_loop":107,"large_sector_id":"L4_MATERIALS_SPREAD_RESOURCE","canonical_archetype_id":"C15_MATERIAL_SPREAD_SUPERCYCLE","fine_archetype_id":"C15_NEW_MATERIALS_REPRICE_TODO_AFTER_CACHE_MISS","candidate_symbols":["010130","005490"],"candidate_names":["고려아연","POSCO홀딩스"],"why_not_trigger_row_now":"uncached 2024 stock-web symbol shards returned cache miss in this execution; no fresh 30D/90D/180D MFE/MAE computed here","calibration_usable":false,"score_alignment":"future run should compute stock-web windows before counting these as new C15 evidence"}
```
...
```text
C15_MATERIAL_LABEL_TO_ASP_MARGIN_FCF_BRIDGE_REQUIREMENT_V107_HELD_OUT
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---:|
| L4_MATERIALS_SPREAD_RESOURCE | C15_MATERIAL_SPREAD_SUPERCYCLE | MATERIAL_LABEL_TO_ASP_MARGIN_FCF_HOLDOUT_V107 | 2 | 3 | 2 | 2 | 0 | 5 | 5 | 0 | 4 | false | false | 24 |
...
```text
Do not patch production scoring from this MD alone. Treat this R4/C15 loop 107 as holdout validation only. Batch it with C15 loops 101~106, C16/C17 adjacent material rows, and R13 high-MAE/accounting-trust/Stage2 false-positive/4B-4C guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C15 material-label-to-ASP-margin-FCF gate, but do not create a new weight delta from this loop because no new independent case was added. Future research should reprice 고려아연(010130) and POSCO홀딩스(005490) when stock-web shards are accessible.
```
