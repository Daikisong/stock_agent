...
fine_archetype_id: CONTRACT_RETENTION_ARR_RENEWAL_MARGIN_HOLDOUT_V108_SECURITY_SAAS_AI_THEME_RECLASSIFICATION_SPLIT
selection_basis:
...
web_raw_fetch_status_this_turn:
  ...
  direct_stock_web_shards:
    - 012510/2024: source_proxy_reverify_required_from_loop_107
    - 018260/2024: source_proxy_reverify_required_from_loop_107
    - 053800/2024: source_proxy_reverify_required_from_loop_107
    - 030520/2024: source_proxy_reverify_required_from_loop_107
    - 047560/2024: source_proxy_reverify_required_from_loop_107
    - 150900/2024: source_proxy_reverify_required_from_loop_107
    - 263860/2024: source_proxy_reverify_required_from_loop_107
    - 042510/2024: source_proxy_reverify_required_from_loop_107
    - 131370/2024: not_recomputed_this_turn_future_remote_support_retention_candidate
    - 214180/2024: not_recomputed_this_turn_future_security_identity_contract_candidate
    - 099390/2024: not_recomputed_this_turn_future_ITSM_contract_retention_candidate
    - 232140/2024: not_recomputed_this_turn_noise_check
    - 041020/2024: not_recomputed_this_turn_future_office_AI_SaaS_boundary
    - 032850/2024: not_recomputed_this_turn_future_healthcare_SW_boundary
    - 290270/2024: not_recomputed_this_turn_future_security_cloud_candidate
    - 234340/2024: not_recomputed_this_turn_future_SaaS_platform_boundary
...
This file continues the local R8 software/platform sequence after `R8/C28 loop 107`; selected loop is therefore `108`.

This is a **dedupe-aware holdout validation / source-proxy TODO** MD. Direct fresh C28 software/security contract-retention shard recomputation was not completed in this execution. The file reuses loop 107 source-proxy rows as dedupe holdout evidence, so every trigger row remains marked `source_proxy_only=true`, `batch_reverification_required=true`, and `independent_evidence_weight=0.0`. The rows include full 30D/90D/180D MFE/MAE fields for schema compatibility, but they should be reverified against `Songdaiki/stock-web` before any weight change. No production scoring is changed.
...
loop_108_note:
  ...
  reason: C28 remains under-covered, but this run reuses prior C28 source-proxy rows and must not create a new weight delta.
```
...
row_provenance:
  ...
  reason:
    - C28 loop 108 intentionally reuses loop 107 source-proxy rows as a dedupe holdout
...
```text
C28_CONTRACT_RETENTION_ARR_RENEWAL_MARGIN_GATE_V108_HELD_OUT
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---:|
| L8_PLATFORM_CONTENT_SW_SECURITY | C28_SOFTWARE_SECURITY_CONTRACT_RETENTION | CONTRACT_RETENTION_ARR_RENEWAL_MARGIN_HOLDOUT_V108 | 2 | 6 | 2 | 2 | 0 | 8 | 8 | 0 | 6 | false | false | 12 |
...
```text
Do not patch production scoring from this MD alone. Treat this R8/C28 loop 108 as source-proxy holdout validation only. Batch it with C28 loops 106~107, existing C28 rows and R13 accounting-trust / Stage2 false-positive / high-MAE / 4B-4C guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C28 contract-retention/ARR/renewal/margin gate and AI-theme reclassification guard, but do not create a new weight delta from this loop because every trigger row is source_proxy_only and requires batch reverification. Future research should directly reprice 더존비즈온(012510), 삼성에스디에스(018260), 안랩(053800), 지니언스(263860), 한글과컴퓨터(030520), 이스트소프트(047560), 파수(150900), 라온시큐어(042510), 알서포트(131370), 민앤지(214180), 브레인즈컴퍼니(099390), 폴라리스오피스(041020), 비트컴퓨터(032850), 엑셈(205100), 케이사인(192250), 이글루(067920) when stock-web shards are accessible.
```
