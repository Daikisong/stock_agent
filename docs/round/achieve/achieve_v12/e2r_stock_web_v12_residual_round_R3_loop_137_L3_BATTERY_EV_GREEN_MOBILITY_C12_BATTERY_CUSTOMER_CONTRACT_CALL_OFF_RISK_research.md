...
fine_archetype_id: CUSTOMER_CONTRACT_TO_CALL_OFF_UTILIZATION_MARGIN_HOLDOUT_V137_CELL_SEPARATOR_FOIL_MATERIAL_PROXY_SPLIT
selection_basis:
...
web_raw_fetch_status_this_turn:
  ...
  direct_stock_web_shards:
    ...
    - 006400/2024: cache_miss_observed_this_turn
    - 247540/2024: cache_miss_observed_this_turn
    - 361610/2024: reused_from_prior_local_file
...
This file continues the local R3 battery sequence after `R3/C13 loop 136`; selected loop is therefore `137` for this C12 holdout.

This is a **dedupe-aware holdout validation / cache-miss TODO** MD. It does not claim fresh independent stock-web evidence because direct stock-web symbol-year shard recomputation for several C12 candidates returned cache miss or was unavailable in this execution. The trigger rows below reuse current-session stock-web-derived C13/C31/C15/C16 rows that already contain complete 30D/90D/180D MFE and MAE from `Songdaiki/stock-web` tradable OHLC, then reinterpret them through the C12 customer-call-off lens. Exact duplicate `same_entry_group_id` rows should be deduped during batch ingest. No production scoring is changed.
...
```text
C12_CUSTOMER_CONTRACT_TO_CALLOFF_UTILIZATION_MARGIN_GATE_V137_HELD_OUT
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---:|
| L3_BATTERY_EV_GREEN_MOBILITY | C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK | CUSTOMER_CONTRACT_TO_CALLOFF_HOLDOUT_V137 | 1 | 4 | 2 | 2 | 0 | 5 | 5 | 0 | 4 | false | false | 14 |
...
```text
Do not patch production scoring from this MD alone. Treat this R3/C12 loop 137 as holdout validation only. Batch it with existing C12 rows, C13 AMPC/JV rows, C15/C16/C17 material/resource rows, and R13 accounting-trust / Stage2 false-positive / high-MAE / 4B-4C guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C12 customer-contract-to-call-off/utilization/margin gate and dominant-driver reclassification guard, but do not create a new weight delta from this loop because no new independent case was added. Future research should reprice 삼성SDI(006400), SK이노베이션(096770), 포스코퓨처엠(003670), 에코프로비엠(247540), 나노신소재(121600), 천보(278280), 솔브레인(357780), 엔켐(348370) when stock-web shards are accessible.
```
