...
fine_archetype_id: PF_REFINANCING_ISSUER_CASH_BRIDGE_HOLDOUT_V105_CACHE_MISS_TODO
selection_basis:
...
`C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK` remains Priority 0 in the no-repeat index. The v12 scheduler maps C30 to `R10 / L9_CONSTRUCTION_REALESTATE_HOUSING`.

This file continues the local C30 sequence after `R10/C30 loop 104`; selected loop is therefore `105`.
...
C30 should not reward a generic PF-relief headline. It should reward the moment a stressed builder’s balance sheet stops leaking.

```text
PF stress / liquidity support / construction rebound / low-PBR builder label
→ issuer-specific refinancing, maturity rollover, guarantee relief, presale improvement, debt-service capacity, receivables quality, cash conversion
→ equity price validation
```

The false positive usually looks attractive first:
...
But a rebound is only 4B until the company-specific financing bridge is visible. The rule remains:

```text
issuer cash bridge visible
→ Stage2 can survive
...
```jsonl
{"row_type":"narrative_only_future_trigger_todo","selected_round":"R10","selected_loop":105,"large_sector_id":"L9_CONSTRUCTION_REALESTATE_HOUSING","canonical_archetype_id":"C30_CONSTRUCTION_PF_BALANCE_SHEET_BREAK","fine_archetype_id":"CACHE_MISS_C30_NEW_SYMBOL_REPRICE_TODO","candidate_symbols":["006360","375500","013580","005960"],"candidate_names":["GS건설","DL이앤씨","계룡건설","동부건설"],"why_not_trigger_row_now":"uncached 2024 stock-web symbol shards returned cache miss in this execution; no fresh 30D/90D/180D MFE/MAE computed here","calibration_usable":false,"score_alignment":"future run should compute stock-web windows for these candidates before counting them as new C30 evidence"}
```
...
Stable large-builder rows are not automatically positive. Low MAE helps, but modest MFE and incomplete cash conversion keep this at Watch.
...
The 180D rebound is real enough to watch, but it cannot be backfilled to the original trigger.
...
Policy vocabulary did not reach issuer cashflow.
...
Local spike without non-price issuer bridge remains 4B only.
...
Delayed price validation can remain in 4B, but the entry-date evidence was incomplete.
...
The row has delayed validation, not immediate balance-sheet proof.
...
Workout is not clean equity recovery. It should teach caution, not promotion.
...
```text
C30_PF_REFINANCING_TO_ISSUER_CASH_BRIDGE_REQUIREMENT_V105_HELD_OUT
...
one_line_summary: "This loop adds 0 new independent cases and validates the C30 PF/refinancing-to-issuer-cash bridge gate while recording cache-miss TODO candidates for future repricing."
```
