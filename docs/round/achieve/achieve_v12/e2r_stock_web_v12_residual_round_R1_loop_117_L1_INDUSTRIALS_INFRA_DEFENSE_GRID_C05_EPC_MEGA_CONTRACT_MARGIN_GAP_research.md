...
fine_archetype_id: EPC_MEGA_CONTRACT_TO_MARGIN_WORKING_CAPITAL_CASH_BRIDGE_HOLDOUT_V117_SAUDI_GAS_CONSTRUCTION_REBOUND_PF_ADJACENT_SPLIT
selection_basis:
...
web_raw_fetch_status_this_turn:
  ...
  direct_stock_web_shards:
    - 028050/2024: reused_from_prior_local_C05_loop_116_pure_EPC_row
    - 006360/2024: reused_from_prior_local_C05_loop_116_EPC_boundary_4B
    ...
    - 010130/2024: not_recomputed_this_turn_C32_governance_boundary_noise
    - 034020/2024: not_recomputed_this_turn_C04_nuclear_project_policy_boundary
    - 009540/2024: not_recomputed_this_turn_shipbuilding_EPC_boundary_noise
    - 051600/2024: not_recomputed_this_turn_nuclear_service_boundary_noise
    - 010620/2024: not_recomputed_this_turn_future_EPC_order_backlog_candidate
    - 028670/2024: not_recomputed_this_turn_future_plant_EPC_candidate
    - 100090/2024: not_recomputed_this_turn_future_EPC_component_candidate
    - 097230/2024: not_recomputed_this_turn_future_infra_EPC_boundary
    - 011930/2024: not_recomputed_this_turn_future_EPC_margin_gap_candidate
...
This file continues the local C05 sequence after `R1/C05 loop 116`; selected loop is therefore `117`.

This is a **dedupe-aware holdout validation / cache-miss TODO** MD. It does not claim fresh independent stock-web evidence because direct fresh C05 EPC/plant/construction candidate shards were not recomputed in this execution. The trigger rows below reuse current-session stock-web-derived C05/C30 rows that already contain complete 30D/90D/180D MFE and MAE from `Songdaiki/stock-web` tradable OHLC. Exact duplicate `same_entry_group_id` rows should be deduped during batch ingest. No production scoring is changed.
...
loop_117_note:
  ...
  reason: C05 remains under-covered, but this run reuses prior C05/C30 boundary rows and must not create a new weight delta.
```
...
reason:
  ...
  - direct fresh C05 EPC/plant/construction candidate shards were not recomputed in this execution
...
```jsonl
{"row_type":"narrative_only_future_trigger_todo","selected_round":"R1","selected_loop":117,"large_sector_id":"L1_INDUSTRIALS_INFRA_DEFENSE_GRID","canonical_archetype_id":"C05_EPC_MEGA_CONTRACT_MARGIN_GAP","fine_archetype_id":"C05_NEW_EPC_CONTRACT_REPRICE_TODO_AFTER_CACHE_MISS","candidate_symbols":["028050","006360","000720","047040","375500","010130","034020"],"candidate_names":["삼성E&A","GS건설","현대건설","대우건설","DL이앤씨","고려아연-EPC_CAPEX_noise_check","두산에너빌리티-project_margin_check"],"why_not_trigger_row_now":"fresh stock-web symbol-year shards were not recomputed in this execution; no fresh 30D/90D/180D MFE/MAE computed here","calibration_usable":false,"score_alignment":"future run should compute fresh stock-web windows before counting new C05 evidence; distinguish EPC contract margin bridge from C30 PF/housing balance-sheet beta or C04/C16 project-policy noise"}
```
...
```text
C05_EPC_CONTRACT_SIZE_TO_MARGIN_WORKING_CAPITAL_CASH_GATE_V117_HELD_OUT
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---:|
| L1_INDUSTRIALS_INFRA_DEFENSE_GRID | C05_EPC_MEGA_CONTRACT_MARGIN_GAP | EPC_CONTRACT_TO_MARGIN_CASH_HOLDOUT_V117 | 4 | 4 | 4 | 2 | 0 | 8 | 8 | 0 | 5 | false | false | 17 |
...
no_new_signal_reason: all rows reused holdout validation after direct fresh C05 EPC/plant/construction candidate shards were not recomputed
loop_contribution_label: duplicate_low_value_loop_with_cache_miss_todo
do_not_propose_new_weight_delta: true
```

```jsonl
{"row_type":"residual_contribution","round":"R1","loop":"117","large_sector_id":"L1_INDUSTRIALS_INFRA_DEFENSE_GRID","canonical_archetype_id":"C05_EPC_MEGA_CONTRACT_MARGIN_GAP","new_independent_case_count":0,"reused_case_count":8,"new_symbol_count":0,"new_trigger_family_count":0,"narrative_only_future_todo_count":1,"tested_existing_calibrated_axes":["stage2_required_bridge","local_4b_watch_guard","hard_4c_thesis_break_routes_to_4c","C05_contract_to_margin_working_capital_cash_gate","no_backfill_later_evidence","construction_PF_reclassification_cap"],"residual_error_types_found":["contract_size_without_margin_cash_bridge","delayed_rebound_backfill_risk","construction_sector_beta_misfile","quality_label_without_project_cash_bridge","large_builder_support_false_positive"],"loop_contribution_label":"duplicate_low_value_loop_with_cache_miss_todo","do_not_propose_new_weight_delta":true}
```
...
```text
Do not patch production scoring from this MD alone. Treat this R1/C05 loop 117 as holdout validation only. Batch it with C05 loops 111, 114, 115 and 116, C30 PF/construction boundary rows, and R13 accounting-trust / Stage2 false-positive / high-MAE / 4B-4C guardrails. If exact same_entry_group_id rows already exist, dedupe them. Preserve the C05 EPC-contract-size-to-margin-working-capital-cash bridge gate and the delayed-rebound no-backfill guard, but do not create a new weight delta from this loop because no new independent case was added. Future research should directly reprice 삼성E&A(028050), GS건설(006360), 현대건설(000720), 대우건설(047040), DL이앤씨(375500), HDC현대산업개발(294870), 한신공영(004960), SGC이테크건설(016250), 한미글로벌(053690), 도화엔지니어링(002150), 유신(054930), 현대엔지니어링-unlisted boundary exclusions, and additional EPC/plant/project cases when stock-web shards are accessible.
```
