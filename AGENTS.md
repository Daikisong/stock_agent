...
- 기존 로그인 세션이 필요한 작업은 BrowserUse Chrome plugin의 `extension` 연결로 사용자의 기존 Chrome 세션·작업 탭을 먼저 찾아 그 안에서 수행한다. `brower` CDP나 Codex가 띄운 Chrome은 별도 backend이며, 로그인 세션이 필요한 BrowserUse 작업의 대체품이 아니다.
...
- BrowserUse 연결 뒤 기존 탭 목록을 읽고 대상 서비스 주소·계정·작업 대화를 확인해 정확한 tab ID에 결박한다. 다른 자동화 브라우저/디버깅 포트에 탭이 없다는 이유로 사용자의 기존 로그인 세션도 없다고 단정하지 않는다.
