...
## BrowserUse — 로그인된 기존 세션 사용

- 로그인된 사용자 세션이 필요한 UI 작업은 반드시 BrowserUse Chrome plugin의 `extension` 연결로 사용자의 기존 Chrome 세션·작업 탭을 먼저 찾아 그 안에서 수행한다. 같은 계정·구독이거나 같은 사이트가 열렸다는 이유로 `brower` CDP, Codex가 띄운 Chrome, 다른 프로필로 바꾸지 않는다.
- 사용자 Chrome 탭은 `browser.user.openTabs()`로 열거하고, 거기서 반환된 정확한 탭 객체를 `browser.user.claimTab(tab)`으로 결박한다. `browser.tabs.list()`는 BrowserUse가 만든 agent tab 목록이므로 비어 있어도 사용자의 외부 Chrome 탭이 없다는 뜻이 아니다.
- 작업 내내 claim한 동일 tab 객체를 유지한다. 주소·작업 대화·로그인 상태·미전송 초안·응답 생성 여부를 필요한 최소 범위에서 읽기 전용으로 확인한다. 대화 본문은 대상 응답을 확인하는 데 필요한 경우에만 읽는다.
- BrowserUse 탭 목록이 비거나 대상 탭을 찾지 못하면 이를 연결/대상 확인 문제로 기록하고 읽기 전용 진단에서 멈춘다. CDP 포트·`C:\ChromeDebug`·다른 프로필에 같은 서비스가 열린 것만으로 동일 로그인 세션이라고 간주하거나 대체 ChatGPT 탭을 만들지 않는다.
- 새 창, 새 브라우저 프로필, 임시/시크릿/헤드리스 브라우저를 만들어 대신 로그인하지 않는다. 기존 탭을 확인하기 전에 재로그인을 요청하거나 쿠키·세션을 다른 프로필로 복사하지 않는다.
- 브라우저 설정의 활성화, 현재 에이전트 세션의 실제 호출 도구, BrowserUse 연결, 기존 대상 탭의 로그인 상태를 각각 확인한다. 설정 파일이나 preflight 성공만으로 연결 성공을 선언하지 않는다.
- BrowserUse 연결 뒤 `browser.user.openTabs()` 결과에서 대상 서비스 주소·계정·작업 대화를 확인해 정확한 반환 객체에 결박한다. tab ID를 추측하거나 수기로 만들지 않는다. 다른 자동화 브라우저/디버깅 포트에 탭이 없다는 이유로 사용자의 기존 로그인 세션도 없다고 단정하지 않는다.
- 다른 backend로 전환할 수 있는 것은 그것이 같은 Chrome instance와 동일한 기존 tab/session에 실제로 결박됨을 확인했을 때뿐이다. 같은 계정, 구독 등급, 또는 비슷한 화면은 같은 세션의 증거가 아니다.
- 새 대화가 필요한 경우에도 로그인된 기존 작업 탭 안에서 새 대화를 시작한다. 새 대화와 새 브라우저 세션을 혼동하지 않는다. 사용자 초안·진행 중 응답이 있으면 덮어쓰거나 이동하지 않는다.
- 입력·첨부·전송 직전에 대상 tab ID, 현재 URL/대화, 입력창을 다시 확인한다. 다른 작업 탭이나 터미널에 전역 키 입력·붙여넣기를 하지 않는다. 화면이 예상과 달라지면 입력을 중단하고 읽기 전용으로 확인한다.
- ChatGPT Pro 작업은 일반 Chat의 실제 Pro 모드를 확인한다. Pro 구독 표시만으로 모드 선택을 인정하거나 Deep research로 대체하지 않는다.
- E2R 파이프라인이 별도 CDP 탭만 제어할 수 있고 그 탭이 claim한 BrowserUse tab과 동일함을 증명하지 못하면, 다른 세션에서 수동 전송하거나 성공 영수증을 꾸며내지 않는다. 파이프라인이 정확한 BrowserUse 탭을 제어하도록 연결하기 전까지 해당 live 단계를 보류한다.
- Chrome BrowserUse 호출이 실패하면 실제 오류와 확인 범위를 기록한다. 기존 탭을 보존하고, 연결 실패를 새 창·재로그인·재전송으로 우회하지 않는다. 다른 backend도 동일 로그인 세션과 정확한 탭을 보존할 수 있어야 하며, 불가능하면 중단하고 필요한 조건만 보고한다.
- 작업 종료·보류 시 사용자의 기존 창·탭·로그인을 닫거나 초기화하지 않는다. 인수인계에는 마지막 확인 시각, 작업 대상, 실제 전송 여부, 정확한 오류, 다음 한 단계만 남기며 인증 토큰·쿠키는 기록하지 않는다.

쉬운 예: 이미 로그인된 ChatGPT 탭 A에서 JSON을 받아야 하면 BrowserUse로 탭 A를 찾아 그 응답을 확인해 다운로드한다.
BrowserUse가 탭 A를 못 찾으면 새 Chrome B나 CDP 창을 대신 열지 않고 연결 상태를 보고한다. 새 연구 대화가 필요하더라도 허용된 범위에서 탭 A 안의 새 대화를 사용한다.
