...
| local_4b_watch_guard | C12_BATTERY_CUSTOMER_CONTRACT_CALL_OFF_RISK | apply_next_patch | Type1_safety_guardrail | True | none | implement_defensive_guardrail |
...
| stage2_bonus_candidate_delta | C22_INSURANCE_RATE_CYCLE_RESERVE | blocked_by_logic_risk | Type2_stage2_yellow_observation | False | bad_stage2_or_high_mae_rate_too_high | keep_green_restricted_and_add_red_team_guard |
...
## Blockers
- evidence_url_pending: 525 / v12 rows can support shadow analysis but need exact public evidence URLs before default promotion. / needed: Attach verified DART/IR/report/news URLs or mark rows as evidence-verified.
- source_proxy_only: 523 / Some evidence is source-name-level historical event proxy, not verified production evidence. / needed: Replace proxy rows with verified as-of evidence records.
- rejected_rows: 2956 / Some rows failed validation or were not usable for shadow calibration. / needed: Fix missing sector/archetype IDs, price fields, or evidence flags.
