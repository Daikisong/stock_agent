...
from e2r.cli.build_round121_r2_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round121_r2_loop7_ai_semiconductor import (
    ROUND121_BASE_SCORE_WEIGHTS,
    ROUND121_CASE_CANDIDATES,
    ROUND121_PRICE_FIELDS,
    ROUND121_SCORE_TARGETS,
    ROUND121_SCORE_STAGE_PRICE_ALIGNMENT,
    ROUND121_STAGE_CAPS,
    render_round121_green_guardrail_markdown,
    render_round121_loop7_risk_overlay_markdown,
    render_round121_price_validation_plan_markdown,
    render_round121_score_stage_price_alignment_markdown,
    render_round121_summary_markdown,
    round121_base_score_weight_rows,
    round121_case_candidate_rows,
    round121_case_records,
    round121_price_field_rows,
    round121_score_profile_rows,
    round121_score_stage_price_alignment_rows,
    round121_stage_cap_rows,
    round121_stage_date_rows,
    round121_summary,
    round121_target_for,
    write_round121_r2_loop7_reports,
)


class Round121R2Loop7AISemiconductorTests(unittest.TestCase):
    def test_round121_targets_cover_loop7_ai_semiconductor_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND121_SCORE_TARGETS}
    ...
    def test_loop7_weights_distinguish_hbm_optical_server_and_neocloud(self):
        hbm = round121_target_for("MEMORY_HBM_CAPACITY")
        hbm_lta = round121_target_for("MEMORY_HBM_LTA_PREPAYMENT")
        hbm_catchup = round121_target_for("HBM_CATCHUP_EXECUTION")
        hbm_catchup_risk = round121_target_for("HBM_CATCHUP_EXECUTION_RISK")
        supply_reallocation = round121_target_for("MEMORY_SUPPLY_REALLOCATION")
        ai_storage = round121_target_for("AI_STORAGE_NAND_SHORTAGE")
        equipment = round121_target_for("SEMI_EQUIPMENT_AI_CAPEX")
        custom_asic = round121_target_for("CUSTOM_AI_ASIC_HYPERSCALER")
        custom_margin = round121_target_for("CUSTOM_AI_ASIC_MARGIN_CONCENTRATION")
        optical = round121_target_for("OPTICAL_NETWORKING_AI_DATACENTER")
        networking = round121_target_for("AI_NETWORKING_SWITCHING_INFRA")
        photonics = round121_target_for("PHOTONICS_AI_DATACENTER_CHIPS")
        server = round121_target_for("AI_SERVER_ODM_EMS_SUPPLY_CHAIN")
        neocloud = round121_target_for("NEOCLOUD_GPU_RENTAL")
        capex_overlay = round121_target_for("AI_CAPEX_CROWDING_OVERLAY")
        circular = round121_target_for("CIRCULAR_AI_FINANCING_OVERLAY")
        disclosure_cap = round121_target_for("DISCLOSURE_CONFIDENCE_CAP")
        ...
        self.assertIn("customer_price_resistance", hbm.loop7_penalty_axes)
    ...
    def test_accounting_trust_overlay_is_hard_gate(self):
        overlay = round121_target_for("REDTEAM_ACCOUNTING_TRUST_OVERLAY")
    ...
    def test_required_round121_cases_are_present_with_loop7_stage_markers(self):
        rows = {row["case_id"]: row for row in round121_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND121_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop7_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND121_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round121_weight_table(self):
        rows = {row["target_id"]: row for row in round121_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round121_stage_date_rows()}
        fields = {row["field"] for row in round121_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND121_PRICE_FIELDS))

    def test_loop7_base_score_weights_stage_caps_and_alignment_are_explicit(self):
        weights = {row["component"]: row for row in round121_base_score_weight_rows()}
        caps = {row["stage_band"]: row for row in round121_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round121_score_stage_price_alignment_rows()}
        alignment_md = render_round121_score_stage_price_alignment_markdown()

        self.assertEqual(len(ROUND121_BASE_SCORE_WEIGHTS), 7)
        ...
        self.assertEqual(len(ROUND121_STAGE_CAPS), 5)
        ...
        self.assertEqual(len(ROUND121_SCORE_STAGE_PRICE_ALIGNMENT), 12)
        ...
        self.assertIn("R2 v7 Base Score Weights", alignment_md)
        self.assertIn("Stage Caps", alignment_md)
        self.assertIn("Samsung HBM4 shipped", alignment_md)

    def test_summary_and_markdown_explain_loop7_ai_distinctions(self):
        summary = round121_summary()
        summary_md = render_round121_summary_markdown()
        guardrails = render_round121_green_guardrail_markdown()
        overlays = render_round121_loop7_risk_overlay_markdown()
        price_plan = render_round121_price_validation_plan_markdown()
        ...
        self.assertIn("R2 Loop 7", summary_md)
        self.assertIn("Do not apply R2 Loop-7 v7.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round121_r2_loop7_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r2_loop7_round121.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round121_r2_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND121_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round121_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round121_r2_loop7_ai_semiconductor", text)
