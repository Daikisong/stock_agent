...
from e2r.cli.build_round82_r3_loop4_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round82_r3_loop4_battery_ev_green import (
    ROUND82_CASE_CANDIDATES,
    ROUND82_PRICE_FIELDS,
    ROUND82_SCORE_TARGETS,
    render_round82_green_guardrail_markdown,
    render_round82_price_validation_plan_markdown,
    render_round82_risk_overlay_markdown,
    render_round82_summary_markdown,
    round82_case_candidate_rows,
    round82_case_records,
    round82_price_field_rows,
    round82_score_profile_rows,
    round82_stage_date_rows,
    round82_summary,
    round82_target_for,
    write_round82_r3_loop4_reports,
)


class Round82R3Loop4BatteryEVGreenTests(unittest.TestCase):
    def test_round82_targets_cover_r3_loop4_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND82_SCORE_TARGETS}
    ...
    def test_ess_and_waste_are_green_possible_while_materials_solar_and_health_are_guarded(self):
        ess = round82_target_for("ESS_LFP_GRID_STORAGE")
        waste = round82_target_for("WASTE_RECYCLING_ENVIRONMENT")
        materials = round82_target_for("BATTERY_MATERIALS_CAPEX_OVERHEAT")
        solar = round82_target_for("SOLAR_TARIFF_SUPPLYCHAIN")
        health = round82_target_for("BATTERY_SOH_SECOND_LIFE_TRANSPARENCY")
        disclosure = round82_target_for("BATTERY_CONTRACT_DISCLOSURE_CONFIDENCE")
        fire = round82_target_for("EV_FIRE_BESS_SAFETY_OVERLAY")

        for target in (ess, waste, materials, solar, health, disclosure, fire):
    ...
    def test_lithium_is_cycle_overlay_not_structural_green_by_default(self):
        lithium = round82_target_for("LITHIUM_ESS_DEMAND_CYCLE")
        ...
        self.assertIn("sodium_ion", lithium.loop4_penalty_axes)

    def test_required_round82_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round82_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND82_CASE_CANDIDATES))
        self.assertEqual(rows["lg_energy_lfp_4_3b_contract_case"]["target_id"], "BATTERY_CONTRACT_DISCLOSURE_CONFIDENCE")
        self.assertEqual(rows["lg_energy_lfp_4_3b_contract_case"]["stage2_date"], "2025-07-30")
    ...
    def test_case_records_validate_and_keep_loop4_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND82_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["lg_energy_lfp_4_3b_contract_case"].score_price_alignment, "evidence_good_but_price_failed")
    ...
    def test_score_profile_rows_match_round82_weight_table(self):
        rows = {row["target_id"]: row for row in round82_score_profile_rows()}

        self.assertEqual(rows["BATTERY_MATERIALS_CAPEX_OVERHEAT"]["eps_fcf"], "16")
        self.assertEqual(rows["BATTERY_MATERIALS_CAPEX_OVERHEAT"]["structural_visibility"], "11")
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round82_stage_date_rows()}
        fields = {row["field"] for row in round82_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND82_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r3_loop4_guardrails(self):
        summary = round82_summary()
        summary_md = render_round82_summary_markdown()
        guardrails = render_round82_green_guardrail_markdown()
        overlays = render_round82_risk_overlay_markdown()
        price_plan = render_round82_price_validation_plan_markdown()
        ...
        self.assertIn("R3 Loop 4", summary_md)
        self.assertIn("Do not apply R3 Loop-4 v4.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round82_r3_loop4_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r3_loop4_round82.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round82_r3_loop4_v4.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND82_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round82_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round82_r3_loop4_battery_ev_green", text)
