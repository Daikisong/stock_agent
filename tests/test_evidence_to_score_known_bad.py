...
class EvidenceToScoreKnownBadTests(unittest.TestCase):
    ...
    def test_exact_25_goal_mutations_are_detected(self) -> None:
        audit = compile_evidence_to_score_known_bad_audit()
        self.assertEqual([case.case_id for case in CASES], [f"KB-{i:02d}" for i in range(1, 26)])
        self.assertEqual(audit["status"], PASS_STATUS)
        self.assertEqual(audit["case_count"], 25)
    ...
    def test_operational_audit_is_recomputed_from_real_detectors(self) -> None:
        ...
        artifact = json.loads(
            (self.ROOT / "docs/operational/e2r_evidence_to_score_known_bad_audit.json").read_text(encoding="utf-8")
        )
