...
from e2r.cli.build_round86_r7_loop4_report import build_parser
...
class Round86R7Loop4BiotechHealthcareDeviceTests(unittest.TestCase):
    def test_round86_targets_cover_r7_loop4_archetypes(self):
        labels = {target.target_id for target in ROUND86_SCORE_TARGETS}
    ...
    def test_cdmo_glp1_and_devices_are_green_possible_but_guardrailed(self):
        ...
        robot = target_for("SURGICAL_ROBOT_INSTALLED_BASE")
    ...
    def test_pre_revenue_ai_telehealth_and_diagnostics_are_redteam_first(self):
        ...
        glp1_channel = target_for("GLP1_TELEHEALTH_CHANNEL")
    ...
    def test_redteam_overlays_are_gate_only(self):
        ...
        compounded = target_for("COMPOUNDED_GLP1_REGULATORY_RISK")
        patent = target_for("BIOSIMILAR_PATENT_LITIGATION")
        device = target_for("DEVICE_SAFETY_COUNTERFEIT_OVERLAY")
    ...
    def test_required_round86_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round86_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND86_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round86_guardrails(self):
        records = round86_case_records()

        self.assertEqual(len(records), len(ROUND86_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round86_weight_table(self):
        rows = {row["target_id"]: row for row in round86_score_profile_rows()}
        ...
        self.assertEqual(rows["OBESITY_GLP1_COMMERCIALIZATION"]["valuation"], "9")
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round86_stage_date_rows()}
        fields = {row["field"] for row in round86_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND86_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r7_loop4_guardrails(self):
        summary = round86_summary()
        summary_md = render_round86_summary_markdown()
        guardrails = render_round86_green_guardrail_markdown()
        overlays = render_round86_risk_overlay_markdown()
        price_plan = render_round86_price_validation_plan_markdown()
        ...
        self.assertIn("R7 Loop 4", summary_md)
        self.assertIn("Do not apply R7 Loop-4 v4.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round86_r7_loop4_reports(
                ...
                cases_path=Path(tmp) / "cases_r7_loop4_round86.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round86_r7_loop4_v4.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND86_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round86_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round86_r7_loop4_biotech_healthcare_device", text)
