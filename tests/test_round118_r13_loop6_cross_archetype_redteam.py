...
from e2r.cli.build_round118_r13_loop6_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10ThemePosture
from e2r.sector.round118_r13_loop6_cross_archetype_redteam import (
...
class Round118R13Loop6CrossArchetypeRedTeamTests(unittest.TestCase):
    def test_round118_targets_cover_loop6_overlays(self):
        labels = {target.target_id for target in ROUND118_OVERLAY_TARGETS}
    ...
    def test_round118_new_canonical_archetypes_exist(self):
        expected = (
    ...
    def test_round118_green_and_hard_gate_rules_are_explicit(self):
        structural = round118_target_for("STRUCTURAL_SUCCESS_ALIGNED")
        sector_4b = round118_target_for("STRUCTURAL_SUCCESS_BUT_4B_WATCH")
        accounting = round118_target_for("REDTEAM_ACCOUNTING_TRUST_OVERLAY")
        stablecoin = round118_target_for("STABLECOIN_CONVERTIBILITY_RISK")
        policy_success = round118_target_for("SECTOR_SUCCESS_BUT_POLICY_SHOCK_WATCH")
        policy_shock = round118_target_for("POLICY_MARKET_SHOCK_EVENT")
        circular = round118_target_for("CIRCULAR_AI_FINANCING_WATCH")
        event_contract = round118_target_for("EVENT_TO_CONTRACT_ESCALATION")
        capex_affo = round118_target_for("CAPEX_AFFO_DILUTION_RISK")
        disclosure_cap = round118_target_for("DISCLOSURE_CONFIDENCE_CAPPED")
    ...
    def test_required_round118_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round118_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND118_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_green_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND118_CASE_CANDIDATES))
        for record in records:
            record.validate()
            self.assertEqual(record.large_sector, ROUND118_LARGE_SECTOR)
    ...
    def test_score_profile_rows_are_overlay_not_production_scoring(self):
        rows = round118_score_profile_rows()

        self.assertEqual(len(rows), len(ROUND118_OVERLAY_TARGETS))
        for row in rows:
    ...
    def test_stage_date_and_price_fields_cover_loop6_needs(self):
        stage_rows = {row["target_id"]: row for row in round118_stage_date_rows()}
        price_fields = {row["field"] for row in round118_price_field_rows()}
    ...
    def test_summary_and_markdown_explain_loop6_validation_layer(self):
        summary = round118_summary()
        summary_md = render_round118_summary_markdown()
        gate_plan = render_round118_redteam_gate_plan_markdown()
        price_plan = render_round118_price_validation_plan_markdown()
        ...
        self.assertIn("Do not apply Round118 overlay symbols", gate_plan)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round118_r13_loop6_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r13_loop6_round118.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round118_r13_loop6_v6.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND118_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round118_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round118_r13_loop6_cross_archetype_redteam", text)
