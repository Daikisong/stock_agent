...
from e2r.cli.build_round222_r5_loop9_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector
from e2r.sector.round222_r5_loop9_consumer_retail_brand_price_validation import (
    ROUND222_CASE_CANDIDATES,
    ROUND222_GREEN_FORBIDDEN_PATTERNS,
    ROUND222_GREEN_REQUIRED_FIELDS,
    ROUND222_HARD_4C_GATES,
    ROUND222_PRICE_VALIDATION_FIELDS,
    ROUND222_REQUIRED_TARGET_ALIASES,
    ROUND222_SHADOW_WEIGHT_ROWS,
    ROUND222_STAGE4B_WATCH_TRIGGERS,
    render_round222_green_gate_review_markdown,
    render_round222_stage4b_4c_review_markdown,
    round222_audit_payload,
    round222_case_records,
    round222_case_rows,
    round222_shadow_weight_rows,
    round222_summary,
    write_round222_r5_loop9_reports,
)


class Round222R5Loop9ConsumerRetailBrandPriceValidationTests(unittest.TestCase):
    def test_targets_are_existing_canonical_archetypes(self) -> None:
        canonical_values = {item.value for item in E2RArchetype}

        self.assertGreaterEqual(len(ROUND222_REQUIRED_TARGET_ALIASES), 13)
        self.assertTrue(set(ROUND222_REQUIRED_TARGET_ALIASES.values()).issubset(canonical_values))
        self.assertEqual(
            ROUND222_REQUIRED_TARGET_ALIASES["K_FOOD_EXPORT_RECURRING"],
            E2RArchetype.EXPORT_RECURRING_CONSUMER.value,
        )
        self.assertEqual(
            ROUND222_REQUIRED_TARGET_ALIASES["K_BEAUTY_DEVICE_GLOBAL_BRAND"],
            E2RArchetype.BEAUTY_DEVICE_EXPORT.value,
        )
        self.assertEqual(
            ROUND222_REQUIRED_TARGET_ALIASES["TOURISM_RETAIL_DUTYFREE_EVENT"],
            E2RArchetype.TOURISM_POLICY_EVENT.value,
        )

    def test_case_records_validate_and_remain_calibration_only(self) -> None:
        records = round222_case_records()
    ...
    def test_samyang_is_stage3_candidate_with_regulatory_watch(self) -> None:
        case = _case("r5_loop9_samyang_buldak_export_regulatory_watch")
    ...
    def test_nongshim_and_olive_young_are_stage2_watch_until_sellthrough(self) -> None:
        nongshim = _case("r5_loop9_nongshim_global_staple_toomba")
        olive = _case("r5_loop9_cj_olive_young_retail_platform")
        ...
        self.assertEqual(nongshim.extra_price_metrics["toomba_first_3_month_sales_mn_units"], 17.0)
    ...
    def test_apr_is_structural_success_with_4b_watch(self) -> None:
        case = _case("r5_loop9_apr_medicube_structural_4b")

        self.assertEqual(case.primary_archetype, E2RArchetype.BEAUTY_DEVICE_EXPORT)
        self.assertEqual(case.stage2_price_anchor, 158300.0)
        self.assertEqual(case.stage3_date.isoformat(), "2025-10-20")
        self.assertEqual(case.stage4b_date.isoformat(), "2025-10-20")
        self.assertEqual(case.stage4b_status, "watch")
        self.assertEqual(case.extra_price_metrics["market_cap_mfe_july_to_oct_pct"], 42.9)
    ...
    def test_dalba_tourism_and_fnf_are_event_or_overheat_not_green(self) -> None:
        dalba = _case("r5_loop9_dalba_silicon2_indie_kbeauty_watch")
        tourism = _case("r5_loop9_tourism_retail_china_visa_event")
        fnf = _case("r5_loop9_fnf_taylormade_mna_optionality")
    ...
    def test_amorepacific_is_china_exposure_thesis_break_watch(self) -> None:
        case = _case("r5_loop9_amorepacific_legacy_china_exposure")
        ...
        self.assertEqual(case.stage4c_date.isoformat(), "2024-08-01")
        self.assertEqual(case.mae_1d, -25.0)
    ...
    def test_green_gate_and_stage4_rules_are_explicit(self) -> None:
        required = set(ROUND222_GREEN_REQUIRED_FIELDS)
        forbidden = set(ROUND222_GREEN_FORBIDDEN_PATTERNS)
        green_markdown = render_round222_green_gate_review_markdown()
        stage_review = render_round222_stage4b_4c_review_markdown()
        ...
        self.assertIn("food_safety_recall_expands", ROUND222_HARD_4C_GATES)
        self.assertIn("tourism_policy_day_basket_rally", ROUND222_STAGE4B_WATCH_TRIGGERS)
        self.assertIn("Do not apply these weights to production scoring yet.", green_markdown)
        self.assertIn("r5_loop9_tourism_retail_china_visa_event", stage_review)

    def test_shadow_weights_cover_r5_loop9_targets(self) -> None:
        shadow_text = "\n".join(str(row) for row in round222_shadow_weight_rows())

        self.assertEqual(len(ROUND222_SHADOW_WEIGHT_ROWS), 8)
    ...
    def test_price_validation_fields_include_round222_anchor_fields(self) -> None:
        fields = set(ROUND222_PRICE_VALIDATION_FIELDS)
    ...
    def test_summary_and_audit_payload_are_calibration_only(self) -> None:
        audit = round222_audit_payload()

        self.assertEqual(audit["source_round"], "docs/round/round_222.md")
        self.assertEqual(audit["large_sector"], Round10LargeSector.CONSUMER_RETAIL_BRAND.value)
        self.assertFalse(audit["summary"]["production_scoring_changed"])
        self.assertFalse(audit["summary"]["candidate_generation_input"])
        self.assertTrue(audit["summary"]["shadow_weight_only"])
        self.assertIn("do_not_use_round222_cases_as_candidate_generation_input", audit["what_not_to_change"])
    ...
    def test_cli_parser_and_writer_outputs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            paths = write_round222_r5_loop9_reports(
            ...
            rows = round222_case_rows()
            self.assertEqual(len(records), len(ROUND222_CASE_CANDIDATES))
            self.assertEqual(len(rows), len(ROUND222_CASE_CANDIDATES))
            ...
            self.assertIn("TOURISM_POLICY_EVENT", paths["shadow_weights"].read_text(encoding="utf-8"))
            self.assertIn("ff_investment_vs_possible_value_pct", paths["price_validation_fields"].read_text(encoding="utf-8"))


def _case(case_id: str):
    return next(case for case in ROUND222_CASE_CANDIDATES if case.case_id == case_id)
