...
from e2r.cli.build_round73_r7_loop3_report import build_parser
...
class Round73R7Loop3BiotechHealthcareDeviceTests(unittest.TestCase):
    def test_round73_targets_cover_r7_loop3_archetypes(self):
        labels = {target.target_id for target in ROUND73_SCORE_TARGETS}
    ...
    def test_required_round73_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round73_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND73_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["intuitive_surgical_q1_2026_procedure_growth_case"]["stage2_date"], "2026-04-01")
    ...
    def test_case_records_validate_and_keep_round73_guardrails(self):
        records = round73_case_records()

        self.assertEqual(len(records), len(ROUND73_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round73_weight_table(self):
        rows = {row["target_id"]: row for row in round73_score_profile_rows()}
        ...
        self.assertEqual(rows["OBESITY_GLP1_COMMERCIALIZATION"]["valuation"], "10")
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round73_stage_date_rows()}
        fields = {row["field"] for row in round73_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND73_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r7_loop3_guardrails(self):
        summary = round73_summary()
        summary_md = render_round73_summary_markdown()
        guardrails = render_round73_green_guardrail_markdown()
        overlays = render_round73_risk_overlay_markdown()
        price_plan = render_round73_price_validation_plan_markdown()
        ...
        self.assertIn("R7 Loop 3", summary_md)
        self.assertIn("Do not apply R7 Loop-3 v3.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round73_r7_loop3_reports(
                ...
                cases_path=Path(tmp) / "cases_r7_loop3_round73.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round73_r7_loop3_v3.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND73_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round73_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round73_r7_loop3_biotech_healthcare_device", text)
