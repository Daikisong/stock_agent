...
from e2r.cli.build_round113_r8_loop6_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round113_r8_loop6_platform_content_sw_security import (
    ROUND113_CASE_CANDIDATES,
    ROUND113_PRICE_FIELDS,
    ROUND113_SCORE_TARGETS,
    render_round113_green_guardrail_markdown,
    render_round113_price_validation_plan_markdown,
    render_round113_risk_overlay_markdown,
    render_round113_summary_markdown,
    round113_case_candidate_rows,
    round113_case_records,
    round113_price_field_rows,
    round113_score_profile_rows,
    round113_stage_date_rows,
    round113_summary,
    round113_target_for,
    write_round113_r8_loop6_reports,
)


class Round113R8Loop6PlatformContentSwSecurityTests(unittest.TestCase):
    def test_round113_targets_cover_r8_loop6_archetypes(self):
        labels = {target.target_id for target in ROUND113_SCORE_TARGETS}
    ...
    def test_green_possible_targets_are_repeat_revenue_guardrailed(self):
        b2b = round113_target_for("B2B_SAAS_ERP_WORKFLOW")
        enterprise = round113_target_for("ENTERPRISE_AI_ONTOLOGY_WORKFLOW")
        government = round113_target_for("GOVERNMENT_AI_PROGRAM_OF_RECORD")
        cloud = round113_target_for("CLOUD_AI_SOFTWARE_INFRA")
        edge = round113_target_for("EDGE_AI_CLOUD_INFRASTRUCTURE")
        legacy_reinforcement = round113_target_for("LEGACY_SAAS_AI_REINFORCEMENT")
        cyber = round113_target_for("CYBERSECURITY_AI_THREAT_DEMAND")
        ...
        self.assertIn("ai_cost", cloud.loop6_penalty_axes)
        ...
        self.assertIn("capex", edge.loop6_penalty_axes)
    ...
    def test_ai_game_and_ugc_themes_are_not_green_by_default(self):
        ai = round113_target_for("AI_SOFTWARE_APPLICATION")
        game = round113_target_for("GAME_CONTENT_IP")
        single_ip = round113_target_for("SINGLE_IP_RELEASE_EVENT_PREMIUM")
        ugc = round113_target_for("UGC_GAME_PLATFORM_SAFETY")
        nft = round113_target_for("METAVERSE_NFT_THEME")
    ...
    def test_overlay_targets_are_gate_only(self):
        for target_id in (
            ...
            target = round113_target_for(target_id)
            assert target is not None
        ...
        trust = round113_target_for("SECURITY_OPERATIONAL_TRUST_OVERLAY")
        ad_trust = round113_target_for("PLATFORM_AD_TRUST_OVERLAY")
        privacy = round113_target_for("PLATFORM_PRIVACY_YOUTH_SAFETY_OVERLAY")
        observability_risk = round113_target_for("OBSERVABILITY_GUIDANCE_RISK")
        legacy = round113_target_for("LEGACY_SAAS_AI_DISRUPTION_OVERLAY")
    ...
    def test_disclosure_confidence_cap_is_cap_only(self):
        cap = round113_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round113_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round113_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND113_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["cisco_ai_infrastructure_orders_case"]["target_id"], "CLOUD_AI_SOFTWARE_INFRA")
    ...
    def test_case_records_validate_and_keep_round113_guardrails(self):
        records = round113_case_records()

        self.assertEqual(len(records), len(ROUND113_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["cisco_ai_infrastructure_orders_case"].primary_archetype.value, "CLOUD_AI_SOFTWARE_INFRA")
        self.assertIn(
    ...
    def test_score_profile_rows_match_round113_weight_table(self):
        rows = {row["target_id"]: row for row in round113_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round113_stage_date_rows()}
        fields = {row["field"] for row in round113_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND113_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r8_loop6_guardrails(self):
        summary = round113_summary()
        summary_md = render_round113_summary_markdown()
        guardrails = render_round113_green_guardrail_markdown()
        overlays = render_round113_risk_overlay_markdown()
        price_plan = render_round113_price_validation_plan_markdown()
        ...
        self.assertIn("Round 113", summary_md)
        self.assertIn("Do not apply R8 Loop-6 v6.0 weights", guardrails)
        ...
        self.assertIn("AI_NETWORKING_INFRA_STAGE2_RESTRUCTURING_WATCH", overlays)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round113_r8_loop6_reports(
                ...
                cases_path=Path(tmp) / "cases_r8_loop6_round113.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round113_r8_loop6_v6.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND113_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round113_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round113_r8_loop6_platform_content_sw_security", text)
