...
from e2r.cli.build_round155_r10_loop9_report import build_parser
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round155_r10_loop9_construction_real_estate_materials import (
...
class Round155R10Loop9ConstructionRealEstateMaterialsTests(unittest.TestCase):
    def test_round155_targets_cover_r10_loop9_archetypes(self):
        labels = {target.target_id for target in ROUND155_SCORE_TARGETS}
    ...
    def test_required_round155_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round155_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND155_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round155_guardrails(self):
        records = round155_case_records()

        self.assertEqual(len(records), len(ROUND155_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round155_weight_table(self):
        rows = {row["target_id"]: row for row in round155_score_profile_rows()}
    ...
    def test_base_score_axes_match_round155_v9_weight_table(self):
        rows = {row["axis_id"]: row for row in round155_base_score_axis_rows()}

        self.assertEqual(len(rows), 7)
        self.assertEqual(len(ROUND155_BASE_SCORE_AXES), 7)
    ...
    def test_stage_caps_make_round155_stage_boundaries_explicit(self):
        rows = {row["cap_id"]: row for row in round155_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND155_STAGE_CAPS), 4)
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round155_stage_date_rows()}
        fields = {row["field"] for row in round155_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND155_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r10_loop9_guardrails(self):
        summary = round155_summary()
        summary_md = render_round155_summary_markdown()
        guardrails = render_round155_green_guardrail_markdown()
        overlays = render_round155_risk_overlay_markdown()
        price_plan = render_round155_price_validation_plan_markdown()
        ...
        self.assertIn("Round 155", summary_md)
        self.assertIn("R10 v9 Base Score Axes", summary_md)
        self.assertIn("eps_fcf_affo_noi_conversion: 24", summary_md)
        self.assertIn("R10 v9 Stage Caps", summary_md)
        self.assertIn("stage1_headline_cap: 45", summary_md)
        self.assertIn("Do not apply R10 Loop-9 v9.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round155_r10_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r10_loop9_round155.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round155_r10_loop9_v9.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND155_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round155_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round155_r10_loop9_construction_real_estate_materials", text)
