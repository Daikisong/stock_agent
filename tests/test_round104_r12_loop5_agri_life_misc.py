...
from e2r.cli.build_round104_r12_loop5_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round104_r12_loop5_agri_life_misc import (
...
class Round104R12Loop5AgriLifeMiscTests(unittest.TestCase):
    def test_round104_targets_cover_r12_loop5_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND104_SCORE_TARGETS}
    ...
    def test_new_round104_archetypes_exist_and_gates_are_explicit(self):
        for archetype in (
        ...
        for target in (edtech, repair, checkout_reg, pouch, disease, approval):
    ...
    def test_score_profile_rows_match_round104_weight_table(self):
        rows = {row["target_id"]: row for row in round104_score_profile_rows()}
    ...
    def test_required_round104_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round104_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND104_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["cnh_agri_machinery_demand_cycle_case"]["target_id"], "AGRI_MACHINERY_DEMAND_CYCLE")
        self.assertEqual(rows["bayer_crop_science_seed_license_case"]["target_id"], "AGRI_INPUT_SEED_CROP_PROTECTION")
    ...
    def test_case_records_validate_and_keep_round104_guardrails(self):
        records = round104_case_records()

        self.assertEqual(len(records), len(ROUND104_CASE_CANDIDATES))
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        stage_rows = {row["target_id"]: row for row in round104_stage_date_rows()}
        price_fields = {row["field"] for row in round104_price_field_rows()}
        ...
        self.assertEqual(len(price_fields), len(ROUND104_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r12_loop5_guardrails(self):
        summary = round104_summary()
        summary_md = render_round104_summary_markdown()
        guardrails = render_round104_green_guardrail_markdown()
        caps = render_round104_unit_economics_cap_markdown()
        price_plan = render_round104_price_validation_plan_markdown()
        ...
        self.assertIn("Round-104", summary_md)
        self.assertIn("Do not apply these R12 Loop-5 v5 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round104_r12_loop5_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r12_loop5_round104.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round104_r12_loop5_v5.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND104_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round104_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round104_r12_loop5_agri_life_misc", text)
