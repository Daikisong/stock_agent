...
from e2r.cli.build_round138_r6_loop8_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round138_r6_loop8_financial_capital_digital import (
...
class Round138R6Loop8FinancialCapitalDigitalTests(unittest.TestCase):
    def test_round138_targets_cover_r6_loop8_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND138_SCORE_TARGETS}
    ...
    def test_financial_insurance_stablecoin_and_exchange_policies_are_separated(self):
        financial = round138_target_for("FINANCIAL_SPREAD_BALANCE_SHEET")
        bank_holding = round138_target_for("BANK_HOLDING_VALUEUP_CAPITAL_RETURN")
        pf_overlay = round138_target_for("BANK_CREDIT_COST_PF_OVERLAY")
        insurance = round138_target_for("INSURANCE_UNDERWRITING_CYCLE")
        insurance_capital = round138_target_for("INSURANCE_CAPITAL_RELEASE_VALUEUP")
        superapp = round138_target_for("FINTECH_SUPERAPP_IPO_OPTION")
        krw_stablecoin = round138_target_for("KRW_STABLECOIN_INFRA_OPTION")
        stablecoin = round138_target_for("REGULATED_STABLECOIN_INFRA")
        ai_payment = round138_target_for("STABLECOIN_AI_AGENT_PAYMENT_OPTION")
        stablecoin_economics = round138_target_for("STABLECOIN_REGULATORY_ECONOMICS")
        algorithmic = round138_target_for("ALGORITHMIC_STABLECOIN_FAILURE")
        stablecoin_bank = round138_target_for("STABLECOIN_BANK_DEPOSIT_DISINTERMEDIATION")
        exchange = round138_target_for("DIGITAL_ASSET_EXCHANGE_CONSOLIDATION")
        bank_stake = round138_target_for("BANK_DIGITAL_ASSET_EQUITY_STAKE")
        exchange_security = round138_target_for("EXCHANGE_SECURITY_OPERATIONAL_RISK")
        fintech_ipo = round138_target_for("FINTECH_IPO_VALUATION_RISK")
    ...
    def test_valueup_execution_and_event_premium_are_not_the_same(self):
        valueup = round138_target_for("VALUE_UP_SHAREHOLDER_RETURN")
        treasury = round138_target_for("TREASURY_SHARE_CANCEL_EXECUTION")
        mandate = round138_target_for("TREASURY_CANCEL_MANDATE_POLICY")
        buyback_risk = round138_target_for("BUYBACK_CANCEL_BUT_BUSINESS_RISK")
        holding = round138_target_for("HOLDING_RESTRUCTURING_GOVERNANCE")
        event = round138_target_for("EVENT_PREMIUM_GOVERNANCE_BATTLE")
    ...
    def test_overlays_are_gate_only_redteam(self):
        ...
        stablecoin_bank = round138_target_for("STABLECOIN_BANK_DEPOSIT_DISINTERMEDIATION")
        exchange_security = round138_target_for("EXCHANGE_SECURITY_OPERATIONAL_RISK")
        tax = round138_target_for("TAX_POLICY_MARKET_SHOCK_OVERLAY")
        ai_shock = round138_target_for("AI_WINDFALL_CITIZEN_DIVIDEND_POLICY_SHOCK")
        governance = round138_target_for("GOVERNANCE_EXECUTION_FAILURE_OVERLAY")
        convertibility = round138_target_for("STABLECOIN_CONVERTIBILITY_OVERLAY")
        disclosure_cap = round138_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round138_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round138_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND138_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round138_guardrails(self):
        records = round138_case_records()

        self.assertEqual(len(records), len(ROUND138_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round138_weight_table(self):
        rows = {row["target_id"]: row for row in round138_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round138_stage_date_rows()}
        fields = {row["field"] for row in round138_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND138_PRICE_FIELDS))

    def test_base_weights_stage_caps_and_alignment_are_reported(self):
        base_rows = {row["component"]: row for row in round138_base_score_weight_rows()}
        cap_rows = {row["cap_id"]: row for row in round138_stage_cap_rows()}
        alignment_rows = {row["case_id"]: row for row in round138_score_stage_price_alignment_rows()}
        alignment_md = render_round138_score_stage_price_alignment_markdown()
    ...
    def test_summary_and_markdown_explain_r6_loop8_guardrails(self):
        summary = round138_summary()
        summary_md = render_round138_summary_markdown()
        guardrails = render_round138_green_guardrail_markdown()
        overlays = render_round138_risk_overlay_markdown()
        price_plan = render_round138_price_validation_plan_markdown()
        ...
        self.assertIn("R6 Loop 8", summary_md)
        self.assertIn("Do not apply R6 Loop-8 v8.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round138_r6_loop8_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r6_loop8_round138.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round138_r6_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND138_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round138_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round138_r6_loop8_financial_capital_digital", text)
