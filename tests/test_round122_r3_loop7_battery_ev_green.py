...
from e2r.cli.build_round122_r3_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round122_r3_loop7_battery_ev_green import (
    ROUND122_BASE_SCORE_WEIGHTS,
    ROUND122_CASE_CANDIDATES,
    ROUND122_PRICE_FIELDS,
    ROUND122_SCORE_TARGETS,
    ROUND122_SCORE_STAGE_PRICE_ALIGNMENT,
    ROUND122_STAGE_CAPS,
    render_round122_green_guardrail_markdown,
    render_round122_price_validation_plan_markdown,
    render_round122_risk_overlay_markdown,
    render_round122_score_stage_price_alignment_markdown,
    render_round122_summary_markdown,
    round122_base_score_weight_rows,
    round122_case_candidate_rows,
    round122_case_records,
    round122_price_field_rows,
    round122_score_stage_price_alignment_rows,
    round122_score_profile_rows,
    round122_stage_cap_rows,
    round122_stage_date_rows,
    round122_summary,
    round122_target_for,
    write_round122_r3_loop7_reports,
)


class Round122R3Loop7BatteryEVGreenTests(unittest.TestCase):
    def test_round122_targets_cover_r3_loop7_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND122_SCORE_TARGETS}
    ...
    def test_ess_and_waste_are_green_possible_while_materials_solar_and_health_are_guarded(self):
        ess = round122_target_for("ESS_LFP_GRID_STORAGE")
        waste = round122_target_for("WASTE_RECYCLING_ENVIRONMENT")
        materials = round122_target_for("BATTERY_MATERIALS_CAPEX_OVERHEAT")
        solar = round122_target_for("SOLAR_TARIFF_SUPPLYCHAIN")
        health = round122_target_for("BATTERY_SOH_SECOND_LIFE_TRANSPARENCY")
        disclosure = round122_target_for("BATTERY_CONTRACT_DISCLOSURE_CONFIDENCE")
        ev_cancel = round122_target_for("EV_CAPA_CONTRACT_CANCELLATION")
        fire = round122_target_for("EV_FIRE_BESS_SAFETY_OVERLAY")
        bess = round122_target_for("BESS_SAFETY_PERMITTING")
        graphite = round122_target_for("BATTERY_GRAPHITE_SUPPLYCHAIN_SECURITY")
        solid_state = round122_target_for("SOLID_STATE_COMMERCIALIZATION_LICENSE")
    ...
    def test_tesla_megapack_supply_chain_is_watch_until_ramp_and_margin_are_visible(self):
        tesla = round122_target_for("ESS_TESLA_MEGAPACK_SUPPLY_CHAIN")
    ...
    def test_lithium_is_cycle_overlay_not_structural_green_by_default(self):
        lithium = round122_target_for("LITHIUM_ESS_DEMAND_CYCLE")
        ...
        self.assertIn("sodium_ion", lithium.loop7_penalty_axes)

    def test_required_round122_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round122_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND122_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop7_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND122_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round122_weight_table(self):
        rows = {row["target_id"]: row for row in round122_score_profile_rows()}
    ...
    def test_round122_base_weights_stage_caps_and_alignment_are_explicit(self):
        base_rows = {row["component"]: row for row in round122_base_score_weight_rows()}
        caps = {row["cap_id"]: row for row in round122_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round122_score_stage_price_alignment_rows()}
        alignment_md = render_round122_score_stage_price_alignment_markdown()

        self.assertEqual(len(base_rows), len(ROUND122_BASE_SCORE_WEIGHTS))
        self.assertEqual(base_rows["eps_fcf_opm_transition"]["weight"], "24")
        self.assertEqual(base_rows["contract_visibility"]["weight"], "22")
        self.assertEqual(base_rows["safety_regulatory_disclosure_confidence"]["weight"], "10")
        self.assertEqual(len(caps), len(ROUND122_STAGE_CAPS))
        ...
        self.assertEqual(len(alignment), len(ROUND122_SCORE_STAGE_PRICE_ALIGNMENT))
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round122_stage_date_rows()}
        fields = {row["field"] for row in round122_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND122_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r3_loop7_guardrails(self):
        summary = round122_summary()
        summary_md = render_round122_summary_markdown()
        guardrails = render_round122_green_guardrail_markdown()
        overlays = render_round122_risk_overlay_markdown()
        price_plan = render_round122_price_validation_plan_markdown()
        ...
        self.assertIn("R3 Loop 7", summary_md)
        self.assertIn("Do not apply R3 Loop-7 v7.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round122_r3_loop7_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r3_loop7_round122.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round122_r3_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND122_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round122_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round122_r3_loop7_battery_ev_green", text)
