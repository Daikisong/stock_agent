...
from e2r.cli.build_round136_r4_loop8_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round136_r4_loop8_materials_spread_strategic import (
...
class Round136R4Loop8MaterialsSpreadStrategicTests(unittest.TestCase):
    def test_round136_targets_cover_r4_loop8_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND136_SCORE_TARGETS}
    ...
    def test_resource_targets_split_structural_offtake_from_export_control_event(self):
        rare = round136_target_for("RARE_METALS_PRICE_FLOOR_OFFTAKE")
        magnet = round136_target_for("RARE_EARTH_MAGNET_SUPPLY_CHAIN")
        export_control = round136_target_for("RARE_METALS_EXPORT_CONTROL_EVENT")
        rare_dilution = round136_target_for("RARE_EARTH_CAPITAL_RAISE_DILUTION")
        copper = round136_target_for("COPPER_AI_GRID_STRUCTURAL_DEMAND")
        copper_cost = round136_target_for("COPPER_PROCESSING_INPUT_COST_OVERLAY")
        nickel_cost = round136_target_for("NICKEL_SULFUR_HPAL_INPUT_COST")
        gold_jurisdiction = round136_target_for("GOLD_MINER_JURISDICTION_RERATING")
        trading = round136_target_for("GENERAL_TRADING_RESOURCE_INFRA")
        lng = round136_target_for("LNG_ENERGY_TRADING_DISTRIBUTION")
        packaging_remedy = round136_target_for("PACKAGING_CONSOLIDATION_REMEDY")
        chemical = round136_target_for("CHEMICAL_SPREAD")
        event = round136_target_for("EVENT_PREMIUM_GOVERNANCE_OVERLAY")
        disclosure = round136_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round136_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round136_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND136_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop8_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND136_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round136_weight_table(self):
        rows = {row["target_id"]: row for row in round136_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round136_stage_date_rows()}
        fields = {row["field"] for row in round136_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND136_PRICE_FIELDS))

    def test_round136_base_weights_stage_caps_and_alignment_are_explicit(self):
        base_rows = {row["component"]: row for row in round136_base_score_weight_rows()}
        caps = {row["cap_id"]: row for row in round136_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round136_score_stage_price_alignment_rows()}
        alignment_md = render_round136_score_stage_price_alignment_markdown()

        self.assertEqual(len(base_rows), len(ROUND136_BASE_SCORE_WEIGHTS))
        ...
        self.assertEqual(len(caps), len(ROUND136_STAGE_CAPS))
        ...
        self.assertEqual(len(alignment), len(ROUND136_SCORE_STAGE_PRICE_ALIGNMENT))
    ...
    def test_summary_and_markdown_explain_r4_loop8_guardrails(self):
        summary = round136_summary()
        summary_md = render_round136_summary_markdown()
        guardrails = render_round136_green_guardrail_markdown()
        overlays = render_round136_risk_overlay_markdown()
        price_plan = render_round136_price_validation_plan_markdown()
        ...
        self.assertIn("R4 Loop 8", summary_md)
        self.assertIn("price floor", summary_md)
        self.assertIn("Do not apply R4 Loop-8 v8.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round136_r4_loop8_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r4_loop8_round136.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round136_r4_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND136_CASE_CANDIDATES))
    ...
    def test_production_modules_do_not_import_round136_pack(self):
        ...
        for relative in (
            ...
            self.assertNotIn("round136_r4_loop8_materials_spread_strategic", text)
