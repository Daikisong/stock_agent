...
from e2r.cli.build_round130_r11_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round130_r11_loop7_policy_geopolitical_event import (
...
class Round130R11Loop7PolicyGeopoliticalEventTests(unittest.TestCase):
    def test_round130_targets_cover_r11_loop7_archetypes(self):
        labels = {target.target_id for target in ROUND130_SCORE_TARGETS}
    ...
    def test_required_round130_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round130_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND130_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round130_guardrails(self):
        records = round130_case_records()

        self.assertEqual(len(records), len(ROUND130_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round130_weight_table(self):
        rows = {row["target_id"]: row for row in round130_score_profile_rows()}
    ...
    def test_base_score_axes_match_round130_v7_weight_table(self):
        rows = {row["axis_id"]: row for row in round130_base_score_axis_rows()}

        self.assertEqual(len(rows), 7)
        self.assertEqual(len(ROUND130_BASE_SCORE_AXES), 7)
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round130_stage_date_rows()}
        fields = {row["field"] for row in round130_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND130_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r11_loop7_guardrails(self):
        summary = round130_summary()
        summary_md = render_round130_summary_markdown()
        guardrails = render_round130_green_guardrail_markdown()
        caps = render_round130_event_false_positive_caps_markdown()
        price_plan = render_round130_price_validation_plan_markdown()
        ...
        self.assertIn("Round-130", summary_md)
        self.assertIn("R11 v7 Base Score Axes", summary_md)
        self.assertIn("actual_contract_budget_order_financing_visibility: 28", summary_md)
        self.assertIn("Do not apply these R11 Loop-7 v7 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round130_r11_loop7_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r11_loop7_round130.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round130_r11_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND130_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round130_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round130_r11_loop7_policy_geopolitical_event", text)
