...
class CensusProviderPendingNotRedTests(unittest.TestCase):
    def test_provider_pending_never_red_or_reject(self):
        inst = instrument()
        scan = BaselineScanner(BaselineScanInputs(provider_failed_symbols={inst.symbol: ["OpenDART"]})).scan(inst, as_of_date="2026-07-01")
