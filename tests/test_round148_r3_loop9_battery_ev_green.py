...
from e2r.cli.build_round148_r3_loop9_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round148_r3_loop9_battery_ev_green import (
    ROUND148_BASE_SCORE_WEIGHTS,
    ROUND148_CASE_CANDIDATES,
    ROUND148_PRICE_FIELDS,
    ROUND148_SCORE_TARGETS,
    ROUND148_SCORE_STAGE_PRICE_ALIGNMENT,
    ROUND148_STAGE_CAPS,
    render_round148_green_guardrail_markdown,
    render_round148_price_validation_plan_markdown,
    render_round148_risk_overlay_markdown,
    render_round148_score_stage_price_alignment_markdown,
    render_round148_summary_markdown,
    round148_base_score_weight_rows,
    round148_case_candidate_rows,
    round148_case_records,
    round148_price_field_rows,
    round148_score_stage_price_alignment_rows,
    round148_score_profile_rows,
    round148_stage_cap_rows,
    round148_stage_date_rows,
    round148_summary,
    round148_target_for,
    write_round148_r3_loop9_reports,
)


class Round148R3Loop9BatteryEVGreenTests(unittest.TestCase):
    def test_round148_targets_cover_r3_loop9_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND148_SCORE_TARGETS}
    ...
    def test_ess_and_waste_are_green_possible_while_materials_solar_and_health_are_guarded(self):
        ess = round148_target_for("ESS_LFP_GRID_STORAGE")
        waste = round148_target_for("WASTE_RECYCLING_ENVIRONMENT")
        materials = round148_target_for("BATTERY_MATERIALS_CAPEX_OVERHEAT")
        solar = round148_target_for("SOLAR_TARIFF_SUPPLYCHAIN")
        health = round148_target_for("BATTERY_SOH_SECOND_LIFE_TRANSPARENCY")
        disclosure = round148_target_for("BATTERY_CONTRACT_DISCLOSURE_CONFIDENCE")
        ev_cancel = round148_target_for("EV_CAPA_CONTRACT_CANCELLATION")
        fire = round148_target_for("EV_FIRE_BESS_SAFETY_OVERLAY")
        bess = round148_target_for("BESS_SAFETY_PERMITTING")
        graphite = round148_target_for("BATTERY_GRAPHITE_SUPPLYCHAIN_SECURITY")
        solid_state = round148_target_for("SOLID_STATE_COMMERCIALIZATION_LICENSE")
    ...
    def test_tesla_megapack_supply_chain_is_watch_until_ramp_and_margin_are_visible(self):
        tesla = round148_target_for("ESS_TESLA_MEGAPACK_SUPPLY_CHAIN")
    ...
    def test_lithium_is_cycle_overlay_not_structural_green_by_default(self):
        lithium = round148_target_for("LITHIUM_ESS_DEMAND_CYCLE")
        ...
        self.assertIn("sodium_ion", lithium.loop9_penalty_axes)

    def test_required_round148_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round148_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND148_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop9_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND148_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round148_weight_table(self):
        rows = {row["target_id"]: row for row in round148_score_profile_rows()}
    ...
    def test_round148_base_weights_stage_caps_and_alignment_are_explicit(self):
        base_rows = {row["component"]: row for row in round148_base_score_weight_rows()}
        caps = {row["cap_id"]: row for row in round148_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round148_score_stage_price_alignment_rows()}
        alignment_md = render_round148_score_stage_price_alignment_markdown()

        self.assertEqual(len(base_rows), len(ROUND148_BASE_SCORE_WEIGHTS))
        ...
        self.assertEqual(len(caps), len(ROUND148_STAGE_CAPS))
        ...
        self.assertEqual(len(alignment), len(ROUND148_SCORE_STAGE_PRICE_ALIGNMENT))
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round148_stage_date_rows()}
        fields = {row["field"] for row in round148_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND148_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r3_loop9_guardrails(self):
        summary = round148_summary()
        summary_md = render_round148_summary_markdown()
        guardrails = render_round148_green_guardrail_markdown()
        overlays = render_round148_risk_overlay_markdown()
        price_plan = render_round148_price_validation_plan_markdown()
        ...
        self.assertIn("R3 Loop 9", summary_md)
        self.assertIn("Do not apply R3 Loop-9 v9.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round148_r3_loop9_reports(
                ...
                cases_path=Path(tmp) / "cases_r3_loop9_round148.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round148_r3_loop9_v9.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND148_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round148_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round148_r3_loop9_battery_ev_green", text)
