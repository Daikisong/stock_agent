...
from e2r.cli.build_round168_r10_loop10_report import build_parser
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round168_r10_loop10_construction_real_estate_materials import (
...
class Round168R10Loop10ConstructionRealEstateMaterialsTests(unittest.TestCase):
    def test_round168_targets_cover_r10_loop10_archetypes(self):
        labels = {target.target_id for target in ROUND168_SCORE_TARGETS}

        self.assertEqual(len(labels), 28)
        self.assertEqual(ROUND168_SOURCE_CANONICAL_TARGET_COUNT, 26)
        self.assertEqual(ROUND168_HELPER_OVERLAY_TARGET_COUNT, 2)
        self.assertTrue(set(ROUND168_SOURCE_CANONICAL_TARGET_IDS).issubset(labels))
        self.assertEqual(set(ROUND168_HELPER_OVERLAY_TARGET_IDS), {"LOW_CARBON_CEMENT_PREMIUM", "PF_CREDIT_REDTEAM_OVERLAY"})
        for label in (
    ...
    def test_pf_affo_power_water_and_ai_theme_overlays_are_gate_only(self):
        ...
        local_opposition = target_for("DATA_CENTER_LOCAL_OPPOSITION_OVERLAY")
        ...
        for target in (pf, affo, power_water, local_opposition, water_rights, ratepayer, capex_affo, cold_debt, no_revenue):
        ...
        assert local_opposition is not None
        self.assertIn("moratorium", local_opposition.stage4c_conditions)
    ...
    def test_required_round168_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round168_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND168_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["seattle_datacenter_moratorium_case"]["target_id"], "DATA_CENTER_LOCAL_OPPOSITION_OVERLAY")
    ...
    def test_case_records_validate_and_keep_round168_guardrails(self):
        records = round168_case_records()

        self.assertEqual(len(records), len(ROUND168_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round168_weight_table(self):
        rows = {row["target_id"]: row for row in round168_score_profile_rows()}
        ...
        self.assertEqual(rows["DATA_CENTER_LOCAL_OPPOSITION_OVERLAY"]["eps_fcf"], "gate")
    ...
    def test_base_score_axes_match_round168_v10_weight_table(self):
        axis_rows = round168_base_score_axis_rows()
        rows = {row["axis_id"]: row for row in axis_rows}

        self.assertEqual(len(rows), 7)
        self.assertEqual(len(ROUND168_BASE_SCORE_AXES), 7)
        self.assertEqual(
    ...
    def test_stage_caps_make_round168_stage_boundaries_explicit(self):
        rows = {row["cap_id"]: row for row in round168_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND168_STAGE_CAPS), 4)
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round168_stage_date_rows()}
        fields = {row["field"] for row in round168_price_field_rows()}
        ...
        self.assertIn("moratorium", rows["DATA_CENTER_LOCAL_OPPOSITION_OVERLAY"]["stage4c"])
        ...
        self.assertEqual(len(fields), len(ROUND168_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r10_loop10_guardrails(self):
        summary = round168_summary()
        summary_md = render_round168_summary_markdown()
        guardrails = render_round168_green_guardrail_markdown()
        overlays = render_round168_risk_overlay_markdown()
        price_plan = render_round168_price_validation_plan_markdown()
        ...
        self.assertIn("Round 168", summary_md)
        self.assertIn("source_canonical_target_count: 26", summary_md)
        self.assertIn("helper_overlay_target_count: 2", summary_md)
        self.assertIn("R10 v10 Base Score Axes", summary_md)
        self.assertIn("eps_fcf_affo_noi_conversion: 24", summary_md)
        self.assertIn("R10 v10 Stage Caps", summary_md)
        self.assertIn("stage1_headline_cap: 45", summary_md)
        self.assertIn("Do not apply R10 Loop-10 v10.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round168_r10_reports(
                ...
                cases_path=Path(tmp) / "cases_r10_loop10_round168.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round168_r10_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND168_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round168_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round168_r10_loop10_construction_real_estate_materials", text)
