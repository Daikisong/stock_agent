...
from e2r.cli.build_round84_r5_loop4_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round84_r5_loop4_consumer_retail_brand import (
...
class Round84R5Loop4ConsumerRetailBrandTests(unittest.TestCase):
    def test_round84_targets_cover_consumer_retail_brand_loop4(self):
        labels = {target.target_id for target in ROUND84_SCORE_TARGETS}
    ...
    def test_green_possible_watch_and_redteam_targets_are_separated(self):
        ...
        for target_id in green_targets:
            target = round84_target_for(target_id)
    ...
    def test_required_round84_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round84_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND84_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["coupang_post_breach_deleted_claim_case"]["stage4c_date"], "2025-12-25")
    ...
    def test_case_records_validate_and_keep_consumer_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND84_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["coupang_post_breach_deleted_claim_case"].rerating_result, "thesis_break")
    ...
    def test_score_profile_rows_match_round84_weight_table(self):
        rows = {row["target_id"]: row for row in round84_score_profile_rows()}

        self.assertEqual(rows["EXPORT_RECURRING_CONSUMER"]["eps_fcf"], "22")
        self.assertEqual(rows["K_FOOD_SINGLE_HERO_PRODUCT"]["valuation"], "11")
        ...
        self.assertEqual(rows["BEAUTY_DEVICE_EXPORT"]["valuation"], "10")
        ...
        self.assertEqual(rows["RETAIL_ECOMMERCE_LOGISTICS"]["eps_fcf"], "15")
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round84_stage_date_rows()}
        fields = {row["field"] for row in round84_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND84_PRICE_FIELDS))

    def test_summary_and_markdown_explain_round84_guardrails(self):
        summary = round84_summary()
        summary_md = render_round84_summary_markdown()
        guardrails = render_round84_green_guardrail_markdown()
        overlays = render_round84_risk_overlay_markdown()
        price_plan = render_round84_price_validation_plan_markdown()
        ...
        self.assertIn("R5 Loop 4", summary_md)
        self.assertIn("Do not apply R5 Loop-4 v4.0 weights", guardrails)
        self.assertIn("AFFILIATE_COMMERCE_MARGIN_WATCH", overlays)
        self.assertIn("DISCOUNT_PROMOTION_MARGIN_OVERLAY", guardrails)
        self.assertIn("coupang_post_breach_deleted_claim_case", price_plan)

    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round84_r5_loop4_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r5_loop4_round84.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round84_r5_loop4_v4.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND84_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round84_pack(self):
        ...
        for relative in (
            ...
            self.assertNotIn("round84_r5_loop4_consumer_retail_brand", text)
