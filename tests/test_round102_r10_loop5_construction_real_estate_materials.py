...
from e2r.cli.build_round102_r10_loop5_report import build_parser
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round102_r10_loop5_construction_real_estate_materials import (
...
class Round102R10Loop5ConstructionRealEstateMaterialsTests(unittest.TestCase):
    def test_round102_targets_cover_r10_loop5_archetypes(self):
        labels = {target.target_id for target in ROUND102_SCORE_TARGETS}
    ...
    def test_pf_affo_power_water_and_ai_theme_overlays_are_gate_only(self):
        ...
        for target in (pf, affo, power_water, local_moratorium, capex_affo, cold_debt, ai_theme):
    ...
    def test_required_round102_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round102_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND102_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["equinix_malaysia_ai_liquid_cooling_case"]["target_id"], "DATA_CENTER_REIT_INFRASTRUCTURE")
        self.assertEqual(rows["equinix_malaysia_ai_liquid_cooling_case"]["stage2_date"], "2026-05-12")
        self.assertEqual(rows["blackstone_data_center_reit_flat_debut_case"]["target_id"], "DATA_CENTER_REIT_IPO_NO_ASSET")
        self.assertEqual(rows["blackstone_data_center_reit_flat_debut_case"]["stage4b_date"], "2026-05-14")
        self.assertEqual(rows["fermi_ai_data_center_no_revenue_case"]["target_id"], "AI_DATA_CENTER_REAL_ASSET_DEVELOPMENT")
        self.assertEqual(rows["fermi_no_tenant_net_loss_case"]["target_id"], "AI_INFRA_REAL_ASSET_THEME_OVERLAY")
    ...
    def test_case_records_validate_and_keep_round102_guardrails(self):
        records = round102_case_records()

        self.assertEqual(len(records), len(ROUND102_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round102_weight_table(self):
        rows = {row["target_id"]: row for row in round102_score_profile_rows()}
        ...
        self.assertEqual(rows["AI_POWER_DATA_CENTER_CAMPUS"]["bottleneck_pricing"], "18")
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round102_stage_date_rows()}
        fields = {row["field"] for row in round102_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND102_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r10_loop5_guardrails(self):
        summary = round102_summary()
        summary_md = render_round102_summary_markdown()
        guardrails = render_round102_green_guardrail_markdown()
        overlays = render_round102_risk_overlay_markdown()
        price_plan = render_round102_price_validation_plan_markdown()
        ...
        self.assertIn("Round 102", summary_md)
        self.assertIn("Do not apply R10 Loop-5 v5.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round102_r10_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r10_loop5_round102.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round102_r10_loop5_v5.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND102_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round102_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round102_r10_loop5_construction_real_estate_materials", text)
