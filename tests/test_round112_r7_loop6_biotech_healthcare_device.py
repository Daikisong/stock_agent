...
from e2r.cli.build_round112_r7_loop6_report import build_parser
...
class Round112R7Loop6BiotechHealthcareDeviceTests(unittest.TestCase):
    def test_round112_targets_cover_r7_loop6_archetypes(self):
        labels = {target.target_id for target in ROUND112_SCORE_TARGETS}
    ...
    def test_cdmo_glp1_and_devices_are_green_possible_but_guardrailed(self):
        cdmo = round112_target_for("CDMO_HEALTHCARE_CONTRACT")
        glp1 = round112_target_for("OBESITY_GLP1_COMMERCIALIZATION")
        device = round112_target_for("MEDICAL_DEVICE_HEALTHCARE_EXPORT")
        implant = round112_target_for("MEDICAL_DEVICE_DENTAL_IMPLANT")
        robot = round112_target_for("SURGICAL_ROBOT_INSTALLED_BASE")
    ...
    def test_pre_revenue_ai_telehealth_and_diagnostics_are_redteam_first(self):
        for target_id in (
            ...
            target = round112_target_for(target_id)
            assert target is not None
            self.assertEqual(target.posture, Round10ThemePosture.REDTEAM_FIRST)

        gene = round112_target_for("GENE_THERAPY_RARE_DISEASE")
        ai_drug = round112_target_for("AI_DRUG_DISCOVERY_PLATFORM")
        diagnostics = round112_target_for("DIAGNOSTICS_INFECTIOUS_DISEASE")
        telehealth = round112_target_for("TELEHEALTH_BEHAVIORAL_HEALTH")
        glp1_channel = round112_target_for("GLP1_TELEHEALTH_CHANNEL")
    ...
    def test_redteam_overlays_are_gate_only(self):
        ...
        compounded = round112_target_for("COMPOUNDED_GLP1_REGULATORY_RISK")
        price_war = round112_target_for("GLP1_PRICE_WAR_OVERLAY")
        patent = round112_target_for("BIOSIMILAR_PATENT_LITIGATION")
        device = round112_target_for("DEVICE_SAFETY_COUNTERFEIT_OVERLAY")
    ...
    def test_disclosure_confidence_cap_is_cap_only_not_gate_only(self):
        cap = round112_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round112_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round112_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND112_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["cigna_humira_biosimilar_zero_copay_case"]["target_id"], "BIOSIMILAR_PBM_FORMULARY_SWITCH")
        self.assertEqual(rows["cigna_humira_biosimilar_zero_copay_case"]["stage2_date"], "2024-04-25")
        self.assertEqual(rows["unitedhealth_humira_preferred_list_case"]["target_id"], "BIOSIMILAR_PBM_FORMULARY_SWITCH")
        self.assertEqual(rows["unitedhealth_humira_preferred_list_case"]["stage2_date"], "2024-09-10")
    ...
    def test_case_records_validate_and_keep_round112_guardrails(self):
        records = round112_case_records()

        self.assertEqual(len(records), len(ROUND112_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["cigna_humira_biosimilar_zero_copay_case"].primary_archetype.value, "BIOSIMILAR_PBM_FORMULARY_SWITCH")
    ...
    def test_score_profile_rows_match_round112_weight_table(self):
        rows = {row["target_id"]: row for row in round112_score_profile_rows()}
        ...
        self.assertEqual(rows["BIOSIMILAR_PBM_FORMULARY_SWITCH"]["structural_visibility"], "17")
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round112_stage_date_rows()}
        fields = {row["field"] for row in round112_price_field_rows()}
        ...
        self.assertIn("prescription_conversion", rows["BIOSIMILAR_PBM_FORMULARY_SWITCH"]["stage2"])
        self.assertIn("weekly_scripts", rows["OBESITY_GLP1_COMMERCIALIZATION"]["stage2"])
        self.assertIn("weekly_scripts", rows["ORAL_GLP1_APPROVAL_COMMERCIALIZATION"]["stage2"])
        ...
        self.assertIn("subgroup_performance_issue", rows["MEDICAL_AI_SUBGROUP_GENERALIZATION_RISK"]["stage4c"])
        ...
        self.assertIn("glp1_procedure_impact", rows["SURGICAL_ROBOT_GLP1_PROCEDURE_MIX_OVERLAY"]["stage4c"])
        for field in (
            "stage2_price",
        ...
        self.assertEqual(len(fields), len(ROUND112_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r7_loop6_guardrails(self):
        summary = round112_summary()
        summary_md = render_round112_summary_markdown()
        guardrails = render_round112_green_guardrail_markdown()
        overlays = render_round112_risk_overlay_markdown()
        price_plan = render_round112_price_validation_plan_markdown()
        ...
        self.assertIn("R7 Loop 6", summary_md)
        self.assertIn("Do not apply R7 Loop-6 v6.0 weights", guardrails)
        ...
        self.assertIn("MEDICAL_AI_SUBGROUP_GENERALIZATION_RISK", overlays)
        self.assertIn("BIOSIMILAR_PATENT_LITIGATION", overlays)
        self.assertIn("BIOSIMILAR_PBM_FORMULARY_SWITCH", overlays)
        ...
        self.assertIn("SURGICAL_ROBOT_GLP1_PROCEDURE_MIX_OVERLAY", overlays)
        ...
        self.assertIn("cigna_humira_biosimilar_zero_copay_case", price_plan)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round112_r7_loop6_reports(
                ...
                cases_path=Path(tmp) / "cases_r7_loop6_round112.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round112_r7_loop6_v6.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND112_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round112_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round112_r7_loop6_biotech_healthcare_device", text)
