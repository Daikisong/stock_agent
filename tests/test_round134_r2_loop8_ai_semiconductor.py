...
from e2r.cli.build_round134_r2_loop8_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round134_r2_loop8_ai_semiconductor import (
    ROUND134_BASE_SCORE_WEIGHTS,
    ROUND134_CASE_CANDIDATES,
    ROUND134_PRICE_FIELDS,
    ROUND134_SCORE_TARGETS,
    ROUND134_SCORE_STAGE_PRICE_ALIGNMENT,
    ROUND134_STAGE_CAPS,
    render_round134_green_guardrail_markdown,
    render_round134_loop8_risk_overlay_markdown,
    render_round134_price_validation_plan_markdown,
    render_round134_score_stage_price_alignment_markdown,
    render_round134_summary_markdown,
    round134_base_score_weight_rows,
    round134_case_candidate_rows,
    round134_case_records,
    round134_price_field_rows,
    round134_score_profile_rows,
    round134_score_stage_price_alignment_rows,
    round134_stage_cap_rows,
    round134_stage_date_rows,
    round134_summary,
    round134_target_for,
    write_round134_r2_loop8_reports,
)


class Round134R2Loop8AISemiconductorTests(unittest.TestCase):
    def test_round134_targets_cover_loop8_ai_semiconductor_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND134_SCORE_TARGETS}
    ...
    def test_loop8_weights_distinguish_hbm_optical_server_and_neocloud(self):
        hbm = round134_target_for("MEMORY_HBM_CAPACITY")
        hbm_lta = round134_target_for("MEMORY_HBM_LTA_PREPAYMENT")
        hbm_catchup = round134_target_for("HBM_CATCHUP_EXECUTION")
        hbm_catchup_risk = round134_target_for("HBM_CATCHUP_EXECUTION_RISK")
        supply_reallocation = round134_target_for("MEMORY_SUPPLY_REALLOCATION")
        ai_storage = round134_target_for("AI_STORAGE_NAND_SHORTAGE")
        equipment = round134_target_for("SEMI_EQUIPMENT_AI_CAPEX")
        custom_asic = round134_target_for("CUSTOM_AI_ASIC_HYPERSCALER")
        custom_margin = round134_target_for("CUSTOM_AI_ASIC_MARGIN_CONCENTRATION")
        optical = round134_target_for("OPTICAL_NETWORKING_AI_DATACENTER")
        networking = round134_target_for("AI_NETWORKING_SWITCHING_INFRA")
        photonics = round134_target_for("PHOTONICS_AI_DATACENTER_CHIPS")
        server = round134_target_for("AI_SERVER_ODM_EMS_SUPPLY_CHAIN")
        neocloud = round134_target_for("NEOCLOUD_GPU_RENTAL")
        capex_overlay = round134_target_for("AI_CAPEX_CROWDING_OVERLAY")
        circular = round134_target_for("CIRCULAR_AI_FINANCING_OVERLAY")
        disclosure_cap = round134_target_for("DISCLOSURE_CONFIDENCE_CAP")
        ...
        self.assertIn("customer_price_resistance", hbm.loop8_penalty_axes)
    ...
    def test_accounting_trust_overlay_is_hard_gate(self):
        overlay = round134_target_for("REDTEAM_ACCOUNTING_TRUST_OVERLAY")
    ...
    def test_required_round134_cases_are_present_with_loop8_stage_markers(self):
        rows = {row["case_id"]: row for row in round134_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND134_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop8_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND134_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round134_weight_table(self):
        rows = {row["target_id"]: row for row in round134_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round134_stage_date_rows()}
        fields = {row["field"] for row in round134_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND134_PRICE_FIELDS))

    def test_loop8_base_score_weights_stage_caps_and_alignment_are_explicit(self):
        weights = {row["component"]: row for row in round134_base_score_weight_rows()}
        caps = {row["stage_band"]: row for row in round134_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round134_score_stage_price_alignment_rows()}
        alignment_md = render_round134_score_stage_price_alignment_markdown()

        self.assertEqual(len(ROUND134_BASE_SCORE_WEIGHTS), 7)
        ...
        self.assertEqual(len(ROUND134_STAGE_CAPS), 5)
        ...
        self.assertEqual(len(ROUND134_SCORE_STAGE_PRICE_ALIGNMENT), 12)
        ...
        self.assertIn("R2 v8 Base Score Weights", alignment_md)
        self.assertIn("Stage Caps", alignment_md)
        self.assertIn("Samsung HBM4 shipped", alignment_md)

    def test_summary_and_markdown_explain_loop8_ai_distinctions(self):
        summary = round134_summary()
        summary_md = render_round134_summary_markdown()
        guardrails = render_round134_green_guardrail_markdown()
        overlays = render_round134_loop8_risk_overlay_markdown()
        price_plan = render_round134_price_validation_plan_markdown()
        ...
        self.assertIn("R2 Loop 8", summary_md)
        self.assertIn("Do not apply R2 Loop-8 v8.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round134_r2_loop8_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r2_loop8_round134.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round134_r2_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND134_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round134_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round134_r2_loop8_ai_semiconductor", text)
