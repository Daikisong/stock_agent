...
from e2r.cli.build_round225_r8_loop9_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector
from e2r.sector.round225_r8_loop9_platform_content_sw_security_price_validation import (
    ROUND225_CASE_CANDIDATES,
    ROUND225_GREEN_FORBIDDEN_PATTERNS,
    ROUND225_GREEN_REQUIRED_FIELDS,
    ROUND225_HARD_4C_GATES,
    ROUND225_PRICE_VALIDATION_FIELDS,
    ROUND225_REQUIRED_TARGET_ALIASES,
    ROUND225_SCORE_ADJUSTMENTS,
    ROUND225_SHADOW_WEIGHT_ROWS,
    ROUND225_STAGE4B_WATCH_TRIGGERS,
    render_round225_green_gate_review_markdown,
    render_round225_stage4b_4c_review_markdown,
    round225_audit_payload,
    round225_case_records,
    round225_case_rows,
    round225_summary,
    write_round225_r8_loop9_reports,
)


class Round225R8Loop9PlatformContentSwSecurityPriceValidationTests(unittest.TestCase):
    def test_round225_targets_map_to_existing_canonical_archetypes(self) -> None:
        canonical_values = {item.value for item in E2RArchetype}

        self.assertGreaterEqual(len(ROUND225_REQUIRED_TARGET_ALIASES), 14)
        self.assertTrue(set(ROUND225_REQUIRED_TARGET_ALIASES.values()).issubset(canonical_values))
        self.assertEqual(
            ROUND225_REQUIRED_TARGET_ALIASES["AI_SOFTWARE_PARTNERSHIP_EVENT"],
            E2RArchetype.AI_SOFTWARE_APPLICATION.value,
        )
        self.assertEqual(
            ROUND225_REQUIRED_TARGET_ALIASES["GAME_IP_REPEAT_MONETIZATION"],
            E2RArchetype.GAME_CONTENT_IP_REPEAT_MONETIZATION.value,
        )
        self.assertEqual(
            ROUND225_REQUIRED_TARGET_ALIASES["GAME_IP_IPO_PREMIUM"],
    ...
    def test_douzone_and_shiftup_are_stage2_watch_not_green(self) -> None:
        by_id = {case.case_id: case for case in ROUND225_CASE_CANDIDATES}
        douzone = by_id["r8_loop9_douzone_eqt_b2b_saas_stage2_watch"]
        shiftup = by_id["r8_loop9_shiftup_single_ip_repeat_monetization_watch"]
    ...
    def test_ai_cloud_webtoon_and_kakao_events_are_not_green(self) -> None:
        by_id = {case.case_id: case for case in ROUND225_CASE_CANDIDATES}
        samsung_sds = by_id["r8_loop9_samsung_sds_ai_cloud_kkr_event_premium"]
        lg_cns = by_id["r8_loop9_lg_cns_ipo_cloud_ai_price_failed"]
        webtoon = by_id["r8_loop9_naver_webtoon_ipo_disney_ip_watch"]
        kakao = by_id["r8_loop9_kakao_openai_partnership_price_only_rally"]
        ...
        self.assertIn("net_loss_margin_11_3pct", webtoon.red_flag_fields)
    ...
    def test_security_and_governance_cases_feed_redteam_not_positive_scoring(self) -> None:
        by_id = {case.case_id: case for case in ROUND225_CASE_CANDIDATES}
        skt = by_id["r8_loop9_skt_data_breach_operational_trust_4c_watch"]
        hybe = by_id["r8_loop9_hybe_founder_legal_governance_4c_watch"]
    ...
    def test_green_gate_4b_4c_and_shadow_weights_are_explicit(self) -> None:
        required = set(ROUND225_GREEN_REQUIRED_FIELDS)
        forbidden = set(ROUND225_GREEN_FORBIDDEN_PATTERNS)
        review = render_round225_green_gate_review_markdown()
        stage_review = render_round225_stage4b_4c_review_markdown()
        weights = {row.archetype: row for row in ROUND225_SHADOW_WEIGHT_ROWS}
        ...
        self.assertIn("data_breach_revenue_cut", ROUND225_HARD_4C_GATES)
        self.assertIn("ai_partnership_spike_before_paid_usage", ROUND225_STAGE4B_WATCH_TRIGGERS)
        self.assertIn("Do not apply these weights to production scoring yet.", review)
        self.assertIn("r8_loop9_skt_data_breach_operational_trust_4c_watch", stage_review)
    ...
    def test_price_fields_and_score_axes_cover_r8_loop9(self) -> None:
        fields = set(ROUND225_PRICE_VALIDATION_FIELDS)
        axes = {item.axis for item in ROUND225_SCORE_ADJUSTMENTS}
    ...
    def test_audit_payload_marks_non_production_round(self) -> None:
        audit = round225_audit_payload()

        self.assertEqual(audit["source_round"], "docs/round/round_225.md")
        self.assertEqual(audit["large_sector"], Round10LargeSector.PLATFORM_CONTENT_SW_SECURITY.value)
        self.assertFalse(audit["summary"]["production_scoring_changed"])
        self.assertFalse(audit["summary"]["candidate_generation_input"])
        self.assertTrue(audit["summary"]["shadow_weight_only"])
        self.assertIn("do_not_use_round225_cases_as_candidate_generation_input", audit["what_not_to_change"])
    ...
    def test_cli_parser_and_writer_outputs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            paths = write_round225_r8_loop9_reports(
            ...
            rows = round225_case_rows()
            self.assertEqual(len(records), len(ROUND225_CASE_CANDIDATES))
            self.assertEqual(len(rows), len(ROUND225_CASE_CANDIDATES))
