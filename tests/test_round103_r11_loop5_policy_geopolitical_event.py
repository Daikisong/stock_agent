...
from e2r.cli.build_round103_r11_loop5_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round103_r11_loop5_policy_geopolitical_event import (
...
class Round103R11Loop5PolicyGeopoliticalEventTests(unittest.TestCase):
    def test_round103_targets_cover_r11_loop5_archetypes(self):
        labels = {target.target_id for target in ROUND103_SCORE_TARGETS}
    ...
    def test_required_round103_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round103_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND103_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["china_group_visa_tourism_policy_case"]["target_id"], "POLICY_LOCAL_THEME")
    ...
    def test_case_records_validate_and_keep_round103_guardrails(self):
        records = round103_case_records()

        self.assertEqual(len(records), len(ROUND103_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round103_weight_table(self):
        rows = {row["target_id"]: row for row in round103_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round103_stage_date_rows()}
        fields = {row["field"] for row in round103_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND103_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r11_loop5_guardrails(self):
        summary = round103_summary()
        summary_md = render_round103_summary_markdown()
        guardrails = render_round103_green_guardrail_markdown()
        caps = render_round103_event_false_positive_caps_markdown()
        price_plan = render_round103_price_validation_plan_markdown()
        ...
        self.assertIn("Round-103", summary_md)
        self.assertIn("Do not apply these R11 Loop-5 v5 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round103_r11_loop5_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r11_loop5_round103.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round103_r11_loop5_v5.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND103_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round103_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round103_r11_loop5_policy_geopolitical_event", text)
