...
from e2r.cli.build_round161_r3_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round161_r3_loop10_battery_ev_green import (
    ROUND161_BASE_SCORE_WEIGHTS,
    ROUND161_CASE_CANDIDATES,
    ROUND161_PRICE_FIELDS,
    ROUND161_SCORE_TARGETS,
    ROUND161_SCORE_STAGE_PRICE_ALIGNMENT,
    ROUND161_STAGE_CAPS,
    render_round161_green_guardrail_markdown,
    render_round161_price_validation_plan_markdown,
    render_round161_risk_overlay_markdown,
    render_round161_score_stage_price_alignment_markdown,
    render_round161_summary_markdown,
    round161_base_score_weight_rows,
    round161_case_candidate_rows,
    round161_case_records,
    round161_price_field_rows,
    round161_score_stage_price_alignment_rows,
    round161_score_profile_rows,
    round161_stage_cap_rows,
    round161_stage_date_rows,
    round161_summary,
    round161_target_for,
    write_round161_r3_loop10_reports,
)


class Round161R3Loop10BatteryEVGreenTests(unittest.TestCase):
    def test_round161_targets_cover_r3_loop10_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND161_SCORE_TARGETS}
    ...
    def test_ess_and_waste_are_green_possible_while_materials_solar_and_health_are_guarded(self):
        ess = round161_target_for("ESS_LFP_GRID_STORAGE")
        waste = round161_target_for("WASTE_RECYCLING_ENVIRONMENT")
        materials = round161_target_for("BATTERY_MATERIALS_CAPEX_OVERHEAT")
        solar = round161_target_for("SOLAR_TARIFF_SUPPLYCHAIN")
        health = round161_target_for("BATTERY_SOH_SECOND_LIFE_TRANSPARENCY")
        disclosure = round161_target_for("BATTERY_CONTRACT_DISCLOSURE_CONFIDENCE")
        ev_cancel = round161_target_for("EV_CAPA_CONTRACT_CANCELLATION")
        fire = round161_target_for("EV_FIRE_BESS_SAFETY_OVERLAY")
        bess = round161_target_for("BESS_SAFETY_PERMITTING")
        graphite = round161_target_for("BATTERY_GRAPHITE_SUPPLYCHAIN_SECURITY")
        solid_state = round161_target_for("SOLID_STATE_COMMERCIALIZATION_LICENSE")
    ...
    def test_lithium_is_cycle_overlay_not_structural_green_by_default(self):
        lithium = round161_target_for("LITHIUM_ESS_DEMAND_CYCLE")
        ...
        self.assertIn("sodium_ion", lithium.loop10_penalty_axes)

    def test_required_round161_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round161_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND161_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop10_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND161_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round161_weight_table(self):
        rows = {row["target_id"]: row for row in round161_score_profile_rows()}

        self.assertEqual(rows["BATTERY_MATERIALS_CAPEX_OVERHEAT"]["eps_fcf"], "12")
        self.assertEqual(rows["BATTERY_MATERIALS_CAPEX_OVERHEAT"]["structural_visibility"], "8")
        self.assertEqual(rows["BATTERY_MATERIALS_CAPEX_OVERHEAT"]["valuation"], "5")
        self.assertEqual(rows["BATTERY_EQUIPMENT_PARTS"]["eps_fcf"], "18")
        self.assertEqual(rows["BATTERY_EQUIPMENT_PARTS"]["structural_visibility"], "16")
        self.assertEqual(rows["BATTERY_EQUIPMENT_PARTS"]["capital_allocation"], "8")
        self.assertEqual(rows["ESS_LFP_GRID_STORAGE"]["eps_fcf"], "23")
        self.assertEqual(rows["ESS_LFP_GRID_STORAGE"]["structural_visibility"], "22")
        self.assertEqual(rows["ESS_LFP_GRID_STORAGE"]["capital_allocation"], "10")
        self.assertEqual(rows["ESS_TESLA_MEGAPACK_SUPPLY_CHAIN"]["eps_fcf"], "22")
        self.assertEqual(rows["ESS_TESLA_MEGAPACK_SUPPLY_CHAIN"]["structural_visibility"], "24")
        self.assertEqual(rows["ESS_TESLA_MEGAPACK_SUPPLY_CHAIN"]["information_confidence"], "10")
        self.assertEqual(rows["ESS_AI_DATA_CENTER_STORAGE"]["bottleneck_pricing"], "16")
        self.assertEqual(rows["EV_TO_ESS_CAPACITY_REDEPLOYMENT"]["structural_visibility"], "19")
        self.assertEqual(rows["EV_TO_ESS_CAPACITY_REDEPLOYMENT"]["capital_allocation"], "10")
        self.assertEqual(rows["EV_BATTERY_JV_RESTRUCTURING"]["eps_fcf"], "15")
        self.assertEqual(rows["EV_BATTERY_JV_RESTRUCTURING"]["structural_visibility"], "14")
        ...
        self.assertEqual(rows["SOLAR_TARIFF_SUPPLYCHAIN"]["eps_fcf"], "12")
        self.assertEqual(rows["SOLAR_TARIFF_SUPPLYCHAIN"]["structural_visibility"], "9")
        self.assertEqual(rows["SOLAR_TARIFF_SUPPLYCHAIN"]["information_confidence"], "10")
        self.assertEqual(rows["RENEWABLE_ENERGY_PROJECT_ECONOMICS"]["eps_fcf"], "12")
        self.assertEqual(rows["RENEWABLE_ENERGY_PROJECT_ECONOMICS"]["structural_visibility"], "9")
        self.assertEqual(rows["WASTE_RECYCLING_ENVIRONMENT"]["structural_visibility"], "22")
        self.assertEqual(rows["CARBON_CREDIT_CBAM_COMPLIANCE"]["information_confidence"], "10")
        self.assertEqual(rows["SECOND_LIFE_BATTERY_GRID_STORAGE"]["eps_fcf"], "18")
        self.assertEqual(rows["SECOND_LIFE_BATTERY_GRID_STORAGE"]["structural_visibility"], "17")
        ...
        self.assertEqual(rows["SOLID_STATE_COMMERCIALIZATION_LICENSE"]["structural_visibility"], "20")
        self.assertEqual(rows["SPECULATIVE_BATTERY_TECH"]["eps_fcf"], "6")
        self.assertEqual(rows["SPECULATIVE_BATTERY_TECH"]["information_confidence"], "10")
        for row in rows.values():
            self.assertEqual(row["production_scoring_changed"], "false")

    def test_round161_base_weights_stage_caps_and_alignment_are_explicit(self):
        base_rows = {row["component"]: row for row in round161_base_score_weight_rows()}
        caps = {row["cap_id"]: row for row in round161_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round161_score_stage_price_alignment_rows()}
        alignment_md = render_round161_score_stage_price_alignment_markdown()

        self.assertEqual(len(base_rows), len(ROUND161_BASE_SCORE_WEIGHTS))
        self.assertEqual(base_rows["eps_fcf_opm_transition"]["weight"], "24")
        self.assertEqual(base_rows["contract_visibility"]["weight"], "22")
        self.assertEqual(base_rows["demand_durability_structural_change"]["weight"], "16")
        self.assertEqual(base_rows["capa_utilization_capital_efficiency_fcf_stability"]["weight"], "10")
        ...
        self.assertEqual(base_rows["safety_regulatory_disclosure_confidence"]["weight"], "14")
        self.assertEqual(len(caps), len(ROUND161_STAGE_CAPS))
        self.assertEqual(caps["narrative_only_cap"]["max_stage"], "Stage 1")
        self.assertEqual(caps["stage2_contract_visibility_cap"]["max_stage"], "Stage 2")
        self.assertEqual(caps["stage4c_hard_redteam"]["max_stage"], "4C")
        self.assertEqual(len(alignment), len(ROUND161_SCORE_STAGE_PRICE_ALIGNMENT))
        self.assertEqual(alignment["tesla_lges_megapack3_lansing_case"]["score_stage"], "Stage 2")
        self.assertIn("ESS OPM", alignment["tesla_lges_megapack3_lansing_case"]["normalization_adjustment"])
        self.assertEqual(alignment["quantumscape_vw_solid_state_license_case"]["score_stage"], "Stage 2 + 4B-watch")
        self.assertEqual(alignment["ford_lges_ev_contract_cancel_case"]["score_stage"], "Stage 4C")
        self.assertIn("Stage 2 visibility and Stage 3 operating confirmation", alignment_md)

    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round161_stage_date_rows()}
        fields = {row["field"] for row in round161_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND161_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r3_loop10_guardrails(self):
        summary = round161_summary()
        summary_md = render_round161_summary_markdown()
        guardrails = render_round161_green_guardrail_markdown()
        overlays = render_round161_risk_overlay_markdown()
        price_plan = render_round161_price_validation_plan_markdown()

        self.assertEqual(summary["target_count"], 26)
        self.assertEqual(summary["source_target_count"], 23)
        self.assertEqual(summary["helper_overlay_target_count"], 3)
        ...
        self.assertIn("R3 Loop 10", summary_md)
        self.assertIn("Do not apply R3 Loop-10 v10.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round161_r3_loop10_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r3_loop10_round161.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round161_r3_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND161_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round161_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round161_r3_loop10_battery_ev_green", text)
