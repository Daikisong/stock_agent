...
from e2r.cli.build_round255_r12_loop11_report import build_parser
from e2r.sector.archetypes import E2RArchetype, archetype_definition
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector
from e2r.sector.round255_r12_loop11_agri_life_service_misc_price_validation import (
    ROUND255_CASE_CANDIDATES,
    ROUND255_DEFAULT_STAGE3_BIAS,
    ROUND255_GREEN_FORBIDDEN_PATTERNS,
    ROUND255_GREEN_REQUIRED_FIELDS,
    ROUND255_HARD_4C_GATES,
    ROUND255_PRICE_VALIDATION_FIELDS,
    ROUND255_REQUIRED_TARGET_ALIASES,
    ROUND255_SCORE_ADJUSTMENTS,
    ROUND255_SHADOW_WEIGHT_ROWS,
    ROUND255_STAGE4B_WATCH_TRIGGERS,
    render_round255_green_gate_review_markdown,
    render_round255_stage4b_4c_review_markdown,
    round255_audit_payload,
    round255_case_records,
    round255_case_rows,
    round255_deep_sub_archetype_rows,
    round255_shadow_weight_rows,
    round255_summary,
    write_round255_r12_loop11_reports,
)


class Round255R12Loop11AgriLifeServiceMiscPriceValidationTests(unittest.TestCase):
    def test_round255_targets_map_to_existing_canonical_archetypes(self) -> None:
        canonical_values = {item.value for item in E2RArchetype}

        self.assertEqual(len(ROUND255_REQUIRED_TARGET_ALIASES), 10)
        self.assertTrue(set(ROUND255_REQUIRED_TARGET_ALIASES.values()).issubset(canonical_values))
        self.assertEqual(
            ROUND255_REQUIRED_TARGET_ALIASES["HOME_LIVING_RENTAL_RECURRING"],
            E2RArchetype.HOME_LIVING_RENTAL_RECURRING.value,
        )
        self.assertEqual(
            ROUND255_REQUIRED_TARGET_ALIASES["EDUCATION_POLICY_MEDICAL_QUOTA"],
            E2RArchetype.EDUCATION_POLICY_MEDICAL_QUOTA.value,
        )
        self.assertEqual(
            ROUND255_REQUIRED_TARGET_ALIASES["FOOD_SERVICE_CELEBRITY_EVENT_PREMIUM"],
            E2RArchetype.FOOD_SERVICE_CELEBRITY_EVENT_PREMIUM.value,
        )
        self.assertEqual(
            ROUND255_REQUIRED_TARGET_ALIASES["POLICY_HEADLINE_NOT_GREEN"],
            E2RArchetype.POLICY_HEADLINE_NOT_GREEN.value,
        )
        self.assertEqual(
            ROUND255_REQUIRED_TARGET_ALIASES["PRICE_MOVED_WITHOUT_EVIDENCE"],
    ...
    def test_case_records_validate_and_keep_non_production_guardrails(self) -> None:
        ...
        self.assertEqual(summary["analyst_round_id"], "round_183")
        ...
        self.assertEqual(summary["r12_default_stage3_bias"], ROUND255_DEFAULT_STAGE3_BIAS)
        self.assertFalse(summary["production_scoring_changed"])
        self.assertFalse(summary["candidate_generation_input"])
        self.assertFalse(summary["full_ohlc_complete"])
        self.assertTrue(summary["shadow_weight_only"])

    def test_round255_archetype_definitions_are_available(self) -> None:
    ...
    def test_coway_is_recurring_candidate_but_not_green(self) -> None:
        by_id = {case.case_id: case for case in ROUND255_CASE_CANDIDATES}
        coway = by_id["r12_loop11_coway_recurring_rental_insufficient"]

        self.assertEqual(coway.primary_archetype, E2RArchetype.HOME_LIVING_RENTAL_RECURRING)
        self.assertIsNone(coway.stage3_date)
        self.assertEqual(coway.round_score_price_alignment, "success_candidate_but_insufficient_evidence")
        self.assertEqual(coway.round_stage_failure_type, "structural_candidate_not_green")
    ...
    def test_education_and_edtech_policy_cases_are_not_green(self) -> None:
        by_id = {case.case_id: case for case in ROUND255_CASE_CANDIDATES}
        quota = by_id["r12_loop11_medical_quota_private_education_watch"]
        edtech = by_id["r12_loop11_edtech_ai_textbook_rollback_phone_ban"]
    ...
    def test_childcare_pet_and_input_cost_cases_are_policy_or_4c_watch(self) -> None:
        by_id = {case.case_id: case for case in ROUND255_CASE_CANDIDATES}
        childcare = by_id["r12_loop11_childcare_fertility_policy_watch"]
        kimchi = by_id["r12_loop11_kimchi_cabbage_input_cost_watch"]
        feed = by_id["r12_loop11_feed_wheat_livestock_input_cost_watch"]
        dog = by_id["r12_loop11_dog_meat_ban_pet_welfare_transition"]
        ...
        self.assertEqual(childcare.extra_price_metrics["seoul_fertility_growth_pct"], 8.9)
    ...
    def test_celebrity_food_event_is_price_moved_without_evidence(self) -> None:
        by_id = {case.case_id: case for case in ROUND255_CASE_CANDIDATES}
        chicken = by_id["r12_loop11_kyochon_cherrybro_neuromeka_jensen_event"]
    ...
    def test_green_gate_4b_4c_shadow_and_deep_rows_are_explicit(self) -> None:
        required = set(ROUND255_GREEN_REQUIRED_FIELDS)
        forbidden = set(ROUND255_GREEN_FORBIDDEN_PATTERNS)
        review = render_round255_green_gate_review_markdown()
        stage_review = render_round255_stage4b_4c_review_markdown()
        fields = set(ROUND255_PRICE_VALIDATION_FIELDS)
        axes = {item.axis for item in ROUND255_SCORE_ADJUSTMENTS}
        shadow_rows = {row["archetype"]: row for row in round255_shadow_weight_rows()}
        deep_rows = round255_deep_sub_archetype_rows()

        self.assertIn("recurring_revenue_or_repeat_purchase_confirmed", required)
        self.assertIn("paid_enrollment_conversion_confirmed", required)
        self.assertIn("utilization_rate_confirmed", required)
        ...
        self.assertIn("celebrity_food_event_plus_20_to_30pct_without_sales", ROUND255_STAGE4B_WATCH_TRIGGERS)
        self.assertIn("feed_cost_shock", ROUND255_HARD_4C_GATES)
        self.assertIn("celebrity_event_fade", ROUND255_HARD_4C_GATES)
        self.assertEqual(len(ROUND255_SHADOW_WEIGHT_ROWS), 8)
        self.assertEqual(shadow_rows["HOME_LIVING_RENTAL_RECURRING"]["recurring_revenue"], "+5")
        self.assertEqual(shadow_rows["CHILDCARE_DEMOGRAPHIC_POLICY_EVENT"]["utilization_rate"], "+5")
        ...
        self.assertIn("r12_loop11_kyochon_cherrybro_neuromeka_jensen_event", stage_review)

    def test_summary_audit_cli_and_writer_outputs(self) -> None:
        audit = round255_audit_payload()
        self.assertEqual(audit["source_round"], "docs/round/round_255.md")
        self.assertEqual(audit["analyst_round_id"], "round_183")
        ...
        self.assertIn("do_not_use_round255_cases_as_candidate_generation_input", audit["what_not_to_change"])
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            paths = write_round255_r12_loop11_reports(
            ...
            rows = round255_case_rows()
            self.assertEqual(len(records), len(ROUND255_CASE_CANDIDATES))
            self.assertEqual(len(rows), len(ROUND255_CASE_CANDIDATES))
