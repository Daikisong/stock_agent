...
from e2r.cli.build_round127_r8_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round127_r8_loop7_platform_content_sw_security import (
    ROUND127_CASE_CANDIDATES,
    ROUND127_PRICE_FIELDS,
    ROUND127_SCORE_TARGETS,
    render_round127_green_guardrail_markdown,
    render_round127_price_validation_plan_markdown,
    render_round127_risk_overlay_markdown,
    render_round127_summary_markdown,
    round127_base_score_axis_rows,
    round127_case_candidate_rows,
    round127_case_records,
    round127_price_field_rows,
    round127_score_profile_rows,
    round127_stage_date_rows,
    round127_summary,
    round127_target_for,
    write_round127_r8_loop7_reports,
)


class Round127R8Loop7PlatformContentSwSecurityTests(unittest.TestCase):
    def test_round127_targets_cover_r8_loop7_archetypes(self):
        labels = {target.target_id for target in ROUND127_SCORE_TARGETS}
    ...
    def test_green_possible_targets_are_repeat_revenue_guardrailed(self):
        b2b = round127_target_for("B2B_SAAS_ERP_WORKFLOW")
        enterprise = round127_target_for("ENTERPRISE_AI_ONTOLOGY_WORKFLOW")
        government = round127_target_for("GOVERNMENT_AI_PROGRAM_OF_RECORD")
        cloud = round127_target_for("CLOUD_AI_SOFTWARE_INFRA")
        edge = round127_target_for("EDGE_AI_CLOUD_INFRASTRUCTURE")
        legacy_reinforcement = round127_target_for("LEGACY_SAAS_AI_REINFORCEMENT")
        cyber = round127_target_for("CYBERSECURITY_AI_THREAT_DEMAND")
        ai_networking = round127_target_for("AI_NETWORKING_SOFTWARE_INFRA_CROSSOVER")
        ...
        self.assertIn("ai_cost", cloud.loop7_penalty_axes)
        ...
        self.assertIn("capex", edge.loop7_penalty_axes)
    ...
    def test_ai_game_and_ugc_themes_are_not_green_by_default(self):
        ai = round127_target_for("AI_SOFTWARE_APPLICATION")
        game = round127_target_for("GAME_CONTENT_IP")
        single_ip = round127_target_for("SINGLE_IP_RELEASE_EVENT_PREMIUM")
        ugc = round127_target_for("UGC_GAME_PLATFORM_SAFETY")
        nft = round127_target_for("METAVERSE_NFT_THEME")
    ...
    def test_overlay_targets_are_gate_only(self):
        for target_id in (
            ...
            target = round127_target_for(target_id)
            assert target is not None
        ...
        trust = round127_target_for("SECURITY_OPERATIONAL_TRUST_OVERLAY")
        ad_trust = round127_target_for("PLATFORM_AD_TRUST_OVERLAY")
        privacy = round127_target_for("PLATFORM_PRIVACY_YOUTH_SAFETY_OVERLAY")
        observability_risk = round127_target_for("OBSERVABILITY_GUIDANCE_RISK")
        legacy = round127_target_for("LEGACY_SAAS_AI_DISRUPTION_OVERLAY")
    ...
    def test_disclosure_confidence_cap_is_cap_only(self):
        cap = round127_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round127_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round127_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND127_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round127_guardrails(self):
        records = round127_case_records()

        self.assertEqual(len(records), len(ROUND127_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round127_weight_table(self):
        rows = {row["target_id"]: row for row in round127_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round127_stage_date_rows()}
        fields = {row["field"] for row in round127_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND127_PRICE_FIELDS))

    def test_base_score_axes_match_round127_weight_table(self):
        rows = {row["axis_id"]: row for row in round127_base_score_axis_rows()}
        ...
        self.assertEqual(rows["market_mispricing_rerating_gap"]["weight"], "10")
        self.assertEqual(rows["valuation_room_4b_margin"]["weight"], "8")
        self.assertEqual(rows["operational_trust_legal_privacy_disclosure"]["weight"], "12")
        self.assertIn("security_outage", rows["operational_trust_legal_privacy_disclosure"]["hard_redteam"])
        for row in rows.values():
            self.assertEqual(row["production_scoring_changed"], "false")

    def test_summary_and_markdown_explain_r8_loop7_guardrails(self):
        summary = round127_summary()
        summary_md = render_round127_summary_markdown()
        guardrails = render_round127_green_guardrail_markdown()
        overlays = render_round127_risk_overlay_markdown()
        price_plan = render_round127_price_validation_plan_markdown()
        ...
        self.assertIn("Round 127", summary_md)
        self.assertIn("Do not apply R8 Loop-7 v7.0 weights", guardrails)
        ...
        self.assertIn("R8 v7 Base Score Axes", summary_md)

    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round127_r8_loop7_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r8_loop7_round127.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round127_r8_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND127_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round127_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round127_r8_loop7_platform_content_sw_security", text)
